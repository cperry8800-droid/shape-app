# film2.py -- the one-shot film assembly, v2: two looks (LOOK=print | hl), activities only, crossfade seams, caption bands, the globe close.
...
OPTS={'fig_radio.mp4':dict(thresh=40,fade=260)}
class Figure:
    ...
    def frame(self,f):
        ...
        if LOOK=='hl':
            mm=blur_mask(m,4)>0.35; bm=blur_mask(mm,6); stroke=((bm>0.12)&(bm<0.88)).astype(np.float32)
            ...
            a=np.clip(glow*0.6+stroke*0.95,0,1)[...,None]
...
def lockup_layers():
    ...
    mk=tri.resize((int(300*ASP),300),Image.LANCZOS); cy=470
...
class RuleFinder:
    ...
    def find(self,f):
        ...
        return self.cols
PLAN=json.load(open(f'{D}/plan2.json'))
...
class Page:
    ...
    def render_globe(self,t,k):
        pg=self.pg; u=t-pg['t0']; f=self.reader.read(); img=Image.fromarray(f).convert('RGBA'); mark_on(img,k,False)
        al=sstep((u-4.0)/0.5)
        if al>0:
