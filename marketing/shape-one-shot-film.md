...
**Style frames, 2026-09-08 — submitted as `nano_banana_pro`, reported by the service as `nano_banana_2`; params
`aspect_ratio 9:16 · resolution 2k · count 1 · use_unlim false` (credits, the account's established path); each prompt ends
with the literal line `resolution: 2k`.** ⚠ Not visually inspected by the agent (this container cannot reach the
cloudfront host); the owner's look is the QA.
