...
| **The film, v3 (2026-09-09)** | the whole film re-shot on `seedance_2_5`: seven figures + the standing coach in one batch (three rate-limited or intercepted, resubmitted), two driver takes, the runner's image-to-video in `omni_reference` (five refusals first), three globe takes (the last at 1080p); the pick reused; two render leases | **13 `seedance_2_5`** |
| **The film, v4 (2026-09-09)** | no dancer; the runner re-timed onto the beat; the beams thinned in the pass; the sphere larger; the thinner outline — all pass-side, on v3's clips: two render leases per look (the first pair, then the pair after the review's fixes); **seven `nano_banana_pro` stills** in the runner's print style, approved; every `seedance_2_5` submission (the globe with eight beams ×3, the lifter's animation ×3) **refused for credits — nothing billed** | **7 `nano_banana_pro`**, 0 video |
...
10. **The Shape Radio page — CLOSED (2026-09-09, v3).** Credits returned the same evening; the bust is retired and the page is
    a driver at the wheel on Seedance 2.5 (row 17). Open in its place: **which driver take** — take 1 (through the window, the
    door slab struck, a small nod — the default) or take 2 (a head-and-shoulders at the wheel, a big swing, no car; row 28).
11. **The model.** v3 is Seedance 2.5 throughout at the owner's ruling (*"I want the top video generation models"*); the figures
    and the runner are its default 720p (the pass masks and blurs them), the driver retake and the globe 1080p. A 1080p
    re-shoot of the seven figures is eight takes if the owner wants the print look's edges sharper.
12. **The video balance.** The owner bought 1,000 credits; the image model spends them and the video model does not — every
    `seedance_2_5` submission since reads *"Out of credits on plus (monthly) plan in Private workspace"*. Which workspace the
    pack landed in, and whether video draws on the plan's monthly balance alone, is on the owner's Higgsfield account page;
    the moment a video submission is accepted, the seven animations and the globe re-prompt go (the prompts are in Sources).
    Until then the film runs on v3's clips with the v4 pass.
...
## The film, v3 — the Seedance 2.5 re-shoot: the driver, the standing coach, the rotating globe with the mark on every beam (2026-09-09)

Owner, the same evening: *"are you using seedance 2.5?"* — no; every clip to this point was `minimax_h3`, the only video model the
account had exposed when it was probed (the launch-cut record lists the ids tried, `seedance_1_pro` among them). Then: *"credits are
back, use seedance 2.5, remove the person rocking their head in the chair, have them maybe jaming out while driving a car or in
another situation. also the globe scene at the end needs to have the globe rotatining with the points coming up from majoe cities
with shape symbol"* · *"nano banan is good as well. I want the top video generation models"* · and, on v2's globe page: *"2 shape
triangles on this page, make the sphere smaller and but shape logo above it."*

**`seedance_2_5` exists on the account now, and it is the model of every clip in v3.** Probed with a real take rather than a
question: the first submission created a job. What the model accepts, measured on the way: `duration` 10 and 12 both honoured
(10.05 / 12.05 s at 24 fps); the default output is **720×1280**, and `resolution: "1080p"` is accepted (1080×1920 — the driver
retake and the final globe are 1080p; the seven figures and the runner are 720p, which the pass masks and blurs anyway);
`mode` is one of `t2v · omni_reference · video_edit · video_extension`, and a reference image is refused in `t2v` (*"mode 't2v'
does not accept reference media"*), so the runner's image-to-video runs in **`omni_reference`** with the approved still as
`image_references`; the backend rate-limits bursts (429 `rate_limit_reached` on three of eight submissions in one batch, and on
three retries of one more — each resubmitted later, nothing lost, nothing double-billed: a refused submission creates no job);
and the preset interception is model-independent (the cyclist was intercepted again by "DROWN IN MUSIC" and resubmitted with
`declined_preset_id f1821f84…`).

| The note | What v3 does |
|---|---|
| the top models | every figure page re-shot on Seedance 2.5 with the v2 prompts (the block-A page text without column rules, the figure phrases verbatim); the runner is Seedance's animation of the same Nano Banana Pro still (E4); the globe is a Seedance take |
| the person rocking their head in the chair | that is the coach on her stool (row 11: *"one foot tapping hard and her head nodding to a beat"*). Her page is re-cast as a **standing coach beside a client mid-squat** (row 25), still under *A person. Not an algorithm.* |
| jamming out while driving | the Shape Radio page is now a **driver at the wheel, seen through the side window**, nodding and drumming the beat (row 17), under ▸ *Shape Radio.* — the radio bust (row 15) is retired |
| the globe rotating, points from the major cities, with the Shape symbol | a Seedance night Earth that **turns eastward through the shot** (measured: the city-light pattern shifts 28–72 px/s, one direction, every second sampled) with **beams of teal light rising from the cities**; the pass finds every beam's tip per frame and draws **the ▸◂ mark on each one**, tracked across frames so the marks ride their beams as the globe turns |
| two triangles on the globe page, the sphere smaller, the logo above it | the globe page carries **one** mark now — the lockup's — and no corner mark; the sphere is drawn at **60 %** of the frame, centred lower (`GS 0.6 · GCY 1480`), with the mark, the wordmark and the rule **above** it (`LOCK_CY 330`) and the two end lines in their band below |

**The pages** are v2's (`plan3.json` = `plan2.json` with the driver in the radio page and the montage, and the Seedance globe as
the close, unstretched: its 12.05 s cover the six-bar page). The captions are unchanged.

**The takes, gated the same way** (mask area at luma < 90 and < 40, white objects, centroid, the page's top band) and looked at on
8-level contact strips: the riser walks in from the left edge and dances (0.4 % → 13 %); the lifter's kettlebell swings (12–15 %);
the cook tosses the pan with the vegetables in the air (5.5 %); the yoga flow in warrior (7 %); the cyclist on a spin bike, side-on
(18 %); the standing coach and the squatting client, two figures touching (25 %); the skipper under his rope (8.5 %); the runner
with the ribbon growing from her watch (teal 12 k → 76 k px, no white object above 525 px where a phone is ~17,000, the page still
at 0.01–0.8 luma of boil). **The driver took two takes.** The first (row 17, 720p) is a side view through the window with the wheel
and a hand — and the car door below the window as a **solid black slab** (rows 1841–2069 at full height), with a nod of 12 px; the
retake (row 28, 1080p, prompted for a bigger jam and no car body) came back as a **giant head-and-shoulders at the wheel** filling
the lower frame, with a real head swing (160 px) and no car at all. The first is the page: it reads as *driving*, and the slab is
struck by a per-clip `cut_below 1835` (`OPTS` in `film3.py`), leaving the figure, the wheel and the window frame. The retake is
recorded and unused; either is one line of the plan.

**The globe took three takes** (rows 18, 19, 29). A and B (720p, two phrasings) both rotate and both raise beams; B raises more
(tip groups 11 → 35 against 5 → 23) and its city-light pattern shifts one way, so B's prompt was re-run at **1080p** (row 29) and
that is the close: mean luma 14.6 → 23.6 across the clip (the beams, not a sunrise — the v7.3 globe flooded to 79), warm city
pixels 4.7 k → 0.5 k as the lit hemisphere turns away, beams' tip groups 13 → 25, and the pattern shift +28 → +72 px/s, eastward
throughout.

**The pins.** `Pins` in `film3.py`: bright, cool pixels (luma > 170, green + blue well above red — a beam of teal or white light;
a city light is warm and fails the test) grouped by column, each group's topmost pixel a tip, groups wider than 60 px dropped (the
atmospheric rim); tips matched frame to frame within 28 px, smoothed, drawn once seen twice and dropped after six frames unseen;
the mark drawn at 44 px with the corner mark's glow, pulsing on the kick. Driven on a synthetic rotating globe with moving beams
before a lease was spent (5–6 tips found and drawn per frame). ⚠ The first local run found **no tips at all** — the local rig's
plan still pointed the globe page at the old synthetic clip; the plan file is copied into the rig now before every test.

**The render.** `runFilm3.sh`, one lease per look (inputs fetched and md5-listed, the grid re-measured — `meas_d1.json`
`66671eee4dae6b0283715147fecb144b` again — three chunks in parallel, mux, upload, verify). The 72-hour host answered 500 to every
chunk checkpoint this time and to the films on the first try; the films' 72-hour links came from a retry.

| Output | md5 | bytes | frames · s | lease | verifier |
|---|---|---|---|---|---|
| **`film_v3_hl.mp4`** (the highlighter look) | `7634e5b05cda19204f4d40c5f0b64b29` | 25,778,977 | 1,442 · 60.084 | chunks 318 s · uploaded at 340 s · verified at 365 s | **25 / 27** |
| **`film_v3_print.mp4`** (the print look) | `fc3d6cf267106fe1e820619345192d75` | 39,477,346 | 1,442 · 60.084 | chunks 243 s · uploaded at 263 s · verified at 293 s | **25 / 27** |

**Verified on the renders** (`verifyF3.py`): **hl 25 PASS / 2 FAIL · print 25 PASS / 2 FAIL — all four failures are the instrument.** The seam into the runner's page is read
against her own sprint (11.5 luma levels per frame on the black look, more than the check's margin allows); the "no column rule"
probe on the lifter's page samples a column the Seedance lifter's kettlebell passes through at the top of its swing (the clip has
no rule there: its source columns dark through the top zone are none, and the only dark group over the page's height is 12 px
wide at x 517, struck as a rule); and the coach's caption check on the print look counts the two figures' legs as ink in the glyph
rows before the band lightens them. Everything else holds on both renders: every other seam, the mark on the kick and still in the
breakdown, IN SYNC on the bar-19 downbeat, the bands, the play glyph, the wordmark above the sphere, **no corner mark on the globe
page, the sphere inside the frame under the lockup, 5,245 / 5,273 teal px of pins and marks between the rule and the sphere**, the
montage cutting on the beat, the rendered audio at 119.95.

```
probe 1440×2560 · 24/1 · nb_read_frames 1442 · duration 60.084
PASS seam lifter across 7.5 within 2.5
PASS seam cook across 8.5 within 3.0
PASS seam yoga across 9.0 within 2.5
PASS seam cyclist across 10.8 within 2.5
PASS seam coach across 14.1 within 3.0
PASS seam radio across 15.9 within 2.5
FAIL seam runner across 17.1 within 11.5
PASS seam skipper across 17.9 within 3.0
PASS seam montage across 8.2 within 2.6
PASS seam globe across 21.6 within 0.2
PASS mark on kick lifter beat 12 on 3200 mid 3008 kb 0.773
PASS mark on kick cook beat 20 on 3205 mid 3004 kb 0.802
PASS mark on kick skipper beat 84 on 3255 mid 3029 kb 0.594
PASS mark still in the breakdown on 2939 mid 2939 kb 0.001
PASS IN SYNC lands on bar 19 post 6674 pre 335 TS 36.070
PASS caption band lifter start 0 mid 14248
PASS caption band coach start 0 mid 15523
PASS caption band radio start 0 mid 8287
PASS radio play glyph teal in the glyph box 1163
FAIL no column rule lifter line px 367
PASS no column rule coach line px 0
PASS globe close wordmark white px 13130 band text px 19520 mean luma 13.2
PASS globe page has no corner mark corner teal px 0
PASS sphere sits smaller, under the lockup lit px inside 215050 at the left edge 2
PASS pins above the sphere carry the mark teal px between the rule and the sphere 5245
PASS montage cut diff 6.7
PASS audio grid {'bpm': 119.95, 'P': 0.500208, 'phase': 0.055, 'score': 0.3855} dur 60.024
RESULT hl 25 PASS 2 FAIL
```

```
probe 1440×2560 · 24/1 · nb_read_frames 1442 · duration 60.084
PASS seam lifter across 22.2 within 3.0
PASS seam cook across 19.9 within 2.4
PASS seam yoga across 14.3 within 2.2
PASS seam cyclist across 29.9 within 1.8
PASS seam coach across 40.7 within 2.9
PASS seam radio across 43.8 within 2.0
PASS seam runner across 66.5 within 28.3
PASS seam skipper across 59.0 within 1.7
PASS seam montage across 16.2 within 2.6
PASS seam globe across 180.4 within 0.2
PASS mark on kick lifter beat 12 on 2513 mid 2348 kb 0.773
PASS mark on kick cook beat 20 on 2495 mid 2366 kb 0.802
PASS mark on kick skipper beat 84 on 2514 mid 2349 kb 0.594
PASS mark still in the breakdown on 2307 mid 2307 kb 0.001
PASS IN SYNC lands on bar 19 post 6645 pre 340 TS 36.070
PASS caption band lifter start 1324 mid 14350
FAIL caption band coach start 31990 mid 15662
PASS caption band radio start 0 mid 8331
PASS radio play glyph teal in the glyph box 1159
FAIL no column rule lifter line px 2610
PASS no column rule coach line px 0
PASS globe close wordmark white px 13127 band text px 293148 mean luma 13.2
PASS globe page has no corner mark corner teal px 0
PASS sphere sits smaller, under the lockup lit px inside 214835 at the left edge 2
PASS pins above the sphere carry the mark teal px between the rule and the sphere 5273
PASS montage cut diff 20.7
PASS audio grid {'bpm': 119.95, 'P': 0.500208, 'phase': 0.055, 'score': 0.3855} dur 60.024
RESULT print 25 PASS 2 FAIL
```

**Looked at, one frame per page, both looks** (checksummed 8-level contact sheets, 29 and 31 lines): the riser dancing with her
hair out; the lifter with the kettlebell; the cook with the pan and the vegetables in the air; the yoga flow in warrior; the
cyclist on the spin bike; the coach's hand on the squatting client's back; the driver at the wheel through the window frame, no
door slab, under ▸ *Shape Radio.*; the Seedance runner with her ribbon and the bar reading IN SYNC (the print itself on cream, a
teal outline with the ribbon carried as light on black); the skipper; the cook again in the montage; and the close — one mark, the
wordmark and the rule above a sphere that sits inside the frame with the beams and their marks rising off it, and the two lines in
their band. ⚠ Not watched by the agent beyond those eleven frames per look; the owner's look is the QA. Handed over as links
(gofile, and 72-hour direct links from a litterbox retry after the lease's own attempt got a 500); review copies are never
committed.


**The scripts** — files in [`marketing/film-v3/`](film-v3/), fetched by the lease from this branch: `film3.py`
`51fbd705cef6abd00c7518420a84553a` (v2's assembly plus `OPTS.cut_below`, `Pins`, the smaller sphere under the lockup) ·
`plan3.json` `7d633c24d6d93df1c876cda8c6ddc084` · `runFilm3.sh` `193ae00e4ac7906cadd2c2e98c8e40a5` (every take's id; the globe
used as delivered) · `verifyF3.py` `5727d34d8b2735bdee75f3fbcd8c2482` (v2's checks, the wordmark box moved up with the lockup, and
three globe-page checks: no corner mark, the sphere inside the frame under the lockup, teal between the rule and the sphere) ·
`beat.py` `63b18de45fe6eb1e446043583cc7c19e` · `meas_d1.py` `e7560987139aeb03854df3c8a9311aca`.

---


## The film, v4 — no dancer, the runner on the beat, the beams thinned, the sphere larger, a truer outline; and the print-style cast approved as stills while the video balance reads empty (2026-09-09)

**Owner, on the v3 renders** (the sheet of the globe page attached): *"way too many lines coming up, make the people look less
like bathroom stick figures, more human outlined. and have the runner match the beat. this applies to the highlighter one.
zoom in on globe more"* · (the approved halftone runner frame attached) *"use this style and look for the person through whole
video of the print style"* · *"make the cooking someone cutting on an island etc. remove the person dancing in the beginning"*
· then, after the first three v4 takes were refused for credits: *"how are credits out? i just bought 1000"* · on the seven
stills: *"these are much better"* · *"these images are solid"*.

| Note | What it got in v4 | Needs a generation? |
|---|---|---|
| *remove the person dancing in the beginning* | The riser's page is gone from the plan: the lifter opens on bar 1 (three bars), the driver's Shape Radio page takes bars 13–15, everything else keeps its beat count; the montage has eight slots instead of nine. | No — `plan4.json`. |
| *have the runner match the beat* | The Seedance runner's stride is measured (footfalls = the frames where the figure's centroid sits lowest; 34 of them in 10 s, mean step 0.289 s = 3.46 steps a second) and the clip is re-timed so **every footfall lands on a beat**: a 48-fps motion-interpolated copy of the first 7.2 s, sampled by a piecewise-linear map (footfall k → k·P) with a blend between the two interpolated frames either side of each instant. She runs at 0.58× — a slow-motion sprint whose feet hit the kick. `beatwarp.py`; `STEPS_PER_BEAT=2` is the double-time alternative (feet on every eighth, 1.16×), one variable. | No. |
| *way too many lines coming up* | In the pass, until the globe is re-shot: the beams are found on the page as thin cool ridges (connected components), at most **eight** stand at once — a slot is held until its beam dies, and a freed slot goes to the live beam farthest across the disc from the ones standing — and every other beam is painted out by a row-wise lerp of the page either side of it (two passes). ⚠ It cleans the early shot (three to ten beams) and only thins the late one: from ~7 s the clip raises a forest of curved streaks that the ridge test half-catches, and what it leaves is still a field. **The fix is the re-prompt** ("never more than eight lines standing at once") — drafted, refused for credits three times, in Sources. | **Yes — refused.** |
| *zoom in on globe more* | The clip's disc rows (100–1900 of 2560) are cropped and scaled to **76 %** of the frame width (v3: the whole frame at 60 %), pasted on a canvas the colour of the clip's own corners with a 60-px feather top and bottom and 80 px at the sides (late in the shot the disc outgrows its frame and the cut limbs would show as straight edges). The disc's top at the end of the shot clears the lockup's rule by ~45 px and its bottom at the start clears the end band by ~38 px — the two extremes one fixed scale has to hold. | No. |
| *less like bathroom stick figures, more human outlined* (the highlighter look) | Two things. In the pass, a thinner, truer line: the mask is smoothed by 1.5 px instead of 4 so hair, fingers and folds survive, the stroke is 4–5 px with a softer glow. And in the source: a **photographic** figure gives the outline pass a human outline for free — the Seedance runner (a halftone press photograph) already reads that way on the black page — so the print-style cast below serves BOTH looks, and the v3 silhouettes are what the hl look draws until those are animated. | **Yes** — the stills exist; their animation is refused. |
| *use this style and look for the person through whole video of the print style* | Seven stills in the runner's exact style — `nano_banana_pro` with the approved runner (E4) as the reference and one figure swapped per prompt: the lifter, the cook at the island, the yoga flow, the cyclist, the coach and client, the driver, the skipper — all seven accepted and rendered (Sources rows 30–36), and approved by the owner in the gallery. Their animation (`seedance_2_5`, `omni_reference`, the runner's own route) is the next step and is refused. | **Yes** — stills done, video refused. |
| *make the cooking someone cutting on an island* | Row 31: a cook behind a kitchen island chopping on a board, the island a low slab in the bottom third. Approved. | **Yes** — still done, video refused. |

⚠ **THE VIDEO BALANCE AND THE IMAGE BALANCE ARE NOT THE SAME BALANCE — MEASURED, NOT ASSUMED.** In the same minutes the seven
`nano_banana_pro` stills were accepted and rendered, every `seedance_2_5` submission (the globe re-prompt three times, with
`use_unlim false` and with it omitted; the lifter's animation twice) was refused with *"Out of credits on plus (monthly) plan in
Private workspace"*. The owner had bought 1,000 credits minutes before. So either the pack landed in a workspace other than
"Private", or video models draw only on the plan's monthly balance — the message names the plan and the workspace, and the
image model does not read it. **Nothing was billed by a refused submission.** The seven animation prompts are drafted in
Sources (the lifter's verbatim; the others follow its shape with the figure's own motion) and go the moment a video submission
is accepted. ⚠ The seven stills were **measured, not viewed, by the agent** (page luma 241 on every one; the figure spanning rows
334–2219 and most of the width; teal 19–45 k px each; the column rules 2–3 px at x 385 and 1077, which the pass strikes; the
driver's sheet carrying solid shapes at the right edge that the animation prompt will have to keep out of the frame) — the
owner looked at them in the gallery, twice, and that look is the approval.

### What the pass does now (`marketing/film-v4/`, committed as files; every lease fetches them from the branch)

- **`plan4.json`** — ten pages on the same 119.95 grid: the lifter bars 1–3 · the cook 4–5 · the yoga flow 6–7 · the cyclist
  8–9 · the coach 10–12 · the driver (▸ *Shape Radio.*) 13–15 · the runner 16–20 (IN SYNC on the bar-19 downbeat) · the
  skipper 21–22 · the montage 23–24 (eight one-beat slots) · the globe 25–30. No riser.
- **`beatwarp.py`** — the runner onto the beat. Footfalls are the frames where the figure's centroid sits lowest on the page
  (luma < 60 at 720×1280, rows 125..1110, a 3-frame smoothing, peaks closer than 4 frames dropped): **34 in 10 s**, at source
  frames 3, 10, 18, 25, 33, 41, 48, 54, 62, 69, 76, 82, 90, 97, 104, 111, 119, 125, 132, 140, 146, 153, 161, 166, 172, 180,
  186, 193, 198, 207, 213, 219, 225, 232 — mean step **0.289 s**. Output frame n at t = n/24 maps to source time by the inverse
  of the piecewise-linear footfall map (k → k·P) and is sampled from a **48-fps motion-interpolated copy** of the clip's first
  7.2 s (`minterpolate` mci/aobmc/bidir, 86 s in the lease — the whole clip at 96 fps would have outrun it) with a linear blend
  between the two interpolated frames either side of the instant. 289 frames = **12.04 s** written before the copy runs out; the
  page uses 10.0 s of it, the montage slot 6.0–6.5 s. `footfalls.json` records the frames, P, the step and the copy's rate.
- **`film4.py`** — v3's assembly with: the thinner outline (`blur_mask(m,1.5)` then a 4-px band between 0.25 and 0.75 of a
  second blur, glow ×0.45, stroke ×0.95); the globe page's crop, scale and feathered paste (`GS 0.76 · GCY 1400 · GCROP 100–1900`);
  `beam_mask` (a thin cool line: brighter by 35 than the page 6–10 px away along x or y, cool, luma > 90, on rows 480..2220),
  `components` (run-based union-find, 8-connected with 1 px of tolerance; a component of ≥ 20 px is a beam, its top pixel the
  tip), `Pins.update` (tracks matched within 28 px, drawn once seen twice; at most `MAXPINS=8` chosen, a slot held until its beam
  dies and a freed slot given to the live beam farthest in x from the ones standing) and `Pins.erase` (the ridge dilated ~5 px ∩
  cool pixels above luma 45, minus the kept beams' own components dilated the same way; every horizontal run replaced by a lerp
  between the page pixels just outside it; two passes, the second on the filled page). One `beam_mask` per frame feeds both.
- **`verifyF4.py`** — v3's checks re-keyed to the new page starts, plus: at most eight beam column groups in rows 700–1000 (wide
  runs dropped first so an arc cannot bridge the count), and the runner's footfalls on the beat (centroid lows over 72 frames
  from bar 16 + 0.3 s against the grid). ⚠ **And the seam check re-derived after the hl verify:** v3's instrument asked only that
  the page after a seam differ from the page before it by twice the page's own one-frame motion plus 2, and the thinner outline
  made two mostly-black pages read alike (across 4.0–8.5 against within 1.7–5.0) on five seams that ARE crossfades. It now tests
  what "crossfade, not a wipe" means — **continuity**: the first frame after the seam differs from the last frame before it by no
  more than that one-frame motion plus 2 (a cut would jump by the whole page) — and a change of page (across > within + 1).
- **`runFilm4.sh`** — the inputs (v3's seven figures, the runner, the 1080p globe, the pick, the logo, Newsreader), the grid, the
  interpolated copy and the warp, `STAGE=prep` to stop there, then three chunks at a time per look (`WINDOWS` at 20.0633 and
  40.0716, the bar-11 and bar-21 downbeats), a `-c copy` concat, one mux with the fade, gofile + litterbox, the verifier.

### The render — two leases per look, the second pair after the review

| | hl, first pair | print, first pair | hl, after the review | print, after the review |
|---|---|---|---|---|
| lease | chunks 362 s · uploaded 381 s · verified 429 s | chunks 337 s · uploaded 357 s · verified 425 s | chunks 387 s · uploaded 406 s · verified 473 s | chunks 361 s · uploaded 381 s · verified 445 s |
| frames | 482 + 480 + 480 = 1442 / 60.084 s | 1442 / 60.084 s | 1442 / 60.084 s | 1442 / 60.084 s |
| md5 · bytes | `c9cfb8a5df6771a9a582d6c2737ef100` · 28,767,600 | `5a2018460c77078ae1a38e9a1ee360df` · 39,698,116 | `b6e1affe3d7bffe81fe683369f66e7f5` · 28,700,723 | `505415f7a5643c769872086c30c145ca` · 39,674,029 |
| verify (final instrument) | **27 / 28** | **26 / 28** | 473 sIFY | **27 / 28** |

Every failure in every column is the same one, and it is not the instrument: **"at most eight beams above the sphere"** reads 36–38
beam column groups in rows 700–1000 at the close, the late-clip field the pass thins and does not clear (the re-prompt is refused).
The first pair's counts are on the instrument of that hour (`verifyF4.py` `128e6a75…`) re-run over the same files; on the instrument as
first written they read 22 / 28 and 23 / 28, and the six extra failures were all the instrument — the seam check (three
re-derivations, below), the print look's caption check on the coach page, and one footfall low per stride. The final instrument
re-derived the print look's caption check twice more: edge ink read a halftone figure as all edge, and the band's cream floor at
200 read a lightened figure as not-cream (the fill leaves ~193 in the blue over ink); it reads the band's fill above the glyphs
at a floor of 180 and the glyph rows' ink. ⚠ And the final instrument's own tally caught a defect in it: the review's fix to the
close's line count put its note on the same line after a semicolon, which commented out the check that followed — the print run
reported 27 lines against the black look's 28, the count was diffed, and the check is live again. The sandbox ran the final
logic from a copy that differs from the committed file by that comment alone (md5 `3ba3b9f9367c0b6e0d8839d2b1707595` against
`c466d993…`); both films read **28 checks, 27 passed**, on it.

**What the checks say on the corrected pair:** every seam a crossfade (the jump at the seam inside one frame of the old page's
motion, the page changed by T + 0.40); the mark brighter on the kick than between kicks on the lifter's, the cook's and the
skipper's pages and still in the breakdown; IN SYNC on the bar-19 downbeat; the caption bands lit mid-page and absent at the
start; the ▸ glyph teal in its box; no column rule on the lifter's or the coach's page; the globe close with the wordmark, the
band's two lines, no corner mark, the sphere inside the frame under the lockup, the marks between the rule and the sphere; **the
runner's footfalls on the beat** (six lows in three seconds, mean offset 0.063 s on the black look, six lows at 0.032 s on the print); the
montage cutting on the beat; the rendered audio re-measuring 119.95.

### The review round (a background reviewer over the four scripts, before the second pair)

Twelve findings, ten acted on, two recorded. **Would have changed the picture:** (1) a chunk window opened on a cold page — the
ribbon's 18-frame trail empty, the struck rules derived from the window's first frame instead of the page's — so at 20.06 s the
coach page's ribbon popped off and regrew, and at 40.07 s the runner crossfaded out with no trail: the 20 frames before a window
are now rendered and discarded; (2) a kept beam missed for one frame lost its slot's protection — its beam painted out and its
mark gone for that frame — and now coasts at its last position for up to six frames; (3) the rim's near-vertical sides are thin in
x and could pass the ridge test as one ring-shaped "beam" (the `narrow` helper had been left unused): components wider than any
beam are rejected; (4) a montage slot opened mid-slot began at its start (latent with these windows): it opens where the slot is.
**Instruments:** the continuity sample was the crossfade's second frame (`-ss` returns the first frame at or after t), so it is
`frame(T)` now with a 4 % allowance instead of 16 %; the pre-sync margin sat 45 px above the teal dot's own area (1,200 now); the
close's line count used ink on the print look and counted the black band itself (cream on both). **Lease:** each chunk is checked
for `FILM2-DONE` before the concat; the warped runner's length is asserted ≥ 10.4 s. **beatwarp:** the source's own rate is probed.
**Recorded, not acted on:** the verifier's ~125 seeks per look (it finished 40–70 s after the upload in every lease); `-shortest`
against a 60.024-s track (1442 frames were written in every render). The reviewer also confirmed, by driving them on synthetic
inputs: the chunk boundaries fall between frames with no gap or duplicate; eight crossfade frames per seam; the montage's slot
samples precede slot 1; the runner's clip lands 16 ms off bar 16; `src_time` monotonic and exact at every footfall; `components`
8-connected with no row-edge wrap; the run fill touching only the beam's own pixels.

### Looked at

Twelve frames per look came across as checksummed 8-level contact sheets (the lines with a per-line md5 and a file md5, joined
from two prints where a sheet ran past the output cap) and were looked at: on black, the lifter, the cook, the yoga flow and the
cyclist as thin outlines under their captions; the coach pair; the driver through her window under ▸ *Shape Radio.*; the runner
with the bar at 84 / 120 and then IN SYNC; the skipper; the montage's first slot; the globe at 2 s with a few beams and their marks
and at 11.7 s with the late field under the lockup and the band. On cream, the same pages as halftone silhouettes, the runner as
the approved print with her ribbon, the globe close. Before the renders, the globe page alone at 3 s and 9 s on three detectors
(the clip's luma test: every beam standing; the ridge test: clean at 3 s, a field at 9 s; the lower thresholds: no better). ⚠ Not
watched by the agent beyond those frames; the owner's look is the QA.

### The scripts (`marketing/film-v4/`, md5 at the second pair)

`film4.py` `0931c0c2269fbdaa10a19e4fb04ba314` · `plan4.json` `5463919d7934c5ef27caf8c9b82be7f3` · `runFilm4.sh`
`ee3ef6d7c51b5f03132aa264300282fa` · `beatwarp.py` `17203fdbbea9d1d0083a88cd90364ea8` · `verifyF4.py`
`c466d9936d291bdcd3abe414ab987a0b` · `beat.py` `63b18de45fe6eb1e446043583cc7c19e` · `meas_d1.py` `e7560987139aeb03854df3c8a9311aca`.
The first pair was rendered from `film4.py` `b1c34496999dbc037936ce9ece8fa6c7` and verified on `verifyF4.py`
`128e6a75cb0b0b75238f90eeb517b6ff` (the counts above are that instrument's; the review's three instrument fixes are in the final one).
...
| 11 | The coach · block A, one take (`minimax_h3` · 9:16 · 10 s · 2K · `use_unlim false` · `declined_preset_id 24bae836-…`) — 2026-09-09 18:11 UTC — ⚠ **not in v3** — the owner's "person rocking their head in the chair"; re-cast standing, row 25 | `c739d3a7-3538-4748-a8f1-5bed0265097c` | `hf_20260909_181112_c739d3a7-3538-4748-a8f1-5bed0265097c.mp4` · md5 `76fc4cc0b70c260b561c3a18bb8f844f` · 2,629,840 B · 243 frames | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a woman seated on a stool, one foot tapping hard and her head nodding to a beat in the middle of the page, pure black with hard clean edges and no interior detail, no face. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
...
| 15 | The radio chooser · block A, one take (same params) — 2026-09-09 19:20 UTC. ⚠ A bust: head and shoulders fill 43–46 % of the page with the model's grey pseudo-text behind her; used as it is, at luma < 40 with the bottom dissolved (see "The film, v2") — ⚠ **not in v3** — the driver (row 17) is the Shape Radio page | `0a0efbdd-f348-4cec-98f0-e207f2d9a938` | `hf_20260909_192018_0a0efbdd-f348-4cec-98f0-e207f2d9a938.mp4` · md5 `0fba566e850893a61117ff3b5d8fb6eb` · 2,823,200 B | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a woman standing at the start of a workout, putting a small wireless earbud into one ear and then the other, then tapping the fitness watch on her wrist and starting to nod and bounce to a beat, in the middle of the page, pure black with hard clean edges and no interior detail, no face. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
...
| 17 | **Seedance 2.5** — the driver, take 1 (USED, with `cut_below 1835`) — 2026-09-09 21:50 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`). The first submission on the model; it created a job, which is how the id was learned | `c2d4d7aa-92cf-4589-a67c-2d3b1e9ccd57` | `hf_20260909_215042_c2d4d7aa-92cf-4589-a67c-2d3b1e9ccd57.mp4` · md5 `97168b387dcf49c13f07cb6a0677123d` · 1,322,353 B · 720×1280 · 241 frames / 10.05 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure: a person seen from the side through the open side window of a car, sitting at the steering wheel, one hand on the wheel and the other hand drumming the beat on the wheel, head nodding and shoulders bouncing to music, jamming out while driving, a fitness watch on the wrist. The person and the steering wheel are solid pure black with hard clean edges and no interior detail, no face; the car is drawn only as a thin black outline of the door and window frame, so the figure sits in the middle of the page with plain cream space around it. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 18 | Seedance 2.5 — the globe, take A (superseded by 29) — 2026-09-09 21:51 UTC (`seedance_2_5` · 9:16 · 12 s · 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `45bcbeff-9c9d-48f0-a334-dc5c089bfffb` | `hf_20260909_215122_45bcbeff-9c9d-48f0-a334-dc5c089bfffb.mp4` · md5 `9a5b5ad38b6b9f3716b048f0b17f0c80` · 3,627,595 B · 720×1280 · 289 frames / 12.04 s | *Vertical 9:16. A photoreal planet Earth at night seen from space against pure black, the globe large in the middle of the frame with black space above and below it. The Americas face the camera at the start and the globe rotates steadily eastward through the whole shot, so new continents keep coming into view; city lights glow warm on the dark land. Thin vertical beams of bright teal light, hex 34D6C5, rise straight up from the major cities into space, each beam ending in a small bright teal point of light, and more beams switch on as the globe turns. Night side only: no sunrise, no daylight, no sunlit clouds. No text, no logos, no labels.* |
| 19 | Seedance 2.5 — the globe, take B (superseded by 29, its prompt re-run at 1080p) — 2026-09-09 21:51 UTC (same params as 18) | `a03a7855-6a47-4cb0-8986-94599b267339` | `hf_20260909_215122_a03a7855-6a47-4cb0-8986-94599b267339.mp4` · md5 `7d9a9263466634467f385049f1aefc02` · 720×1280 · 289 frames / 12.04 s | *Vertical 9:16. A photoreal planet Earth at night seen from space against pure black, the globe large in the middle of the frame with black space above and below it. The Americas face the camera at the start and the globe turns about a quarter turn eastward over the shot at a smooth, visible pace; city lights glow warm on the dark land. Thin lines of bright teal light, hex 34D6C5, rise straight up from the major cities one after another in a rhythm, like pins dropping onto a map, each line ending in a small glowing teal dot above the atmosphere, until dozens of them stand up around the globe. Night side only: no sunrise, no daylight, no sunlit clouds. No text, no logos, no labels.* |
| 20 | Seedance 2.5 — The riser · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `d633fe7a-e831-4930-8ca4-37eac1685a7a` | `hf_20260909_215442_d633fe7a-e831-4930-8ca4-37eac1685a7a.mp4` · md5 `5fd5a8e1de6574e652650dcc971ebd69` · 1,149,878 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a woman with her hair flying, walking in from the left edge and breaking into a dance, in the middle of the page, pure black with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 21 | Seedance 2.5 — The lifter · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `2ebb9cb6-6fdf-4fda-a507-c08826d10e95` | `hf_20260909_215442_2ebb9cb6-6fdf-4fda-a507-c08826d10e95.mp4` · md5 `3180bb0e320185c31004bfb932714539` · 1,687,234 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a broad-shouldered man, swinging a kettlebell from between his legs up to eye level and back in steady rhythmic swings, in the middle of the page, pure black with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 22 | Seedance 2.5 — The cook (the first submission was rate-limited, 429; resubmitted) · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `7d43a9f8-79ab-42f6-9428-cdb35661c89b` | `hf_20260909_215531_7d43a9f8-79ab-42f6-9428-cdb35661c89b.mp4` · md5 `c4155d5251193a288226b1d9850e75c8` · 1,333,423 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a cook in an apron, tossing a frying pan so chopped vegetables hang in the air as black shapes, then catching them, again and again, in the middle of the page, pure black with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 23 | Seedance 2.5 — The yoga flow · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `a09bbd21-1e5f-4460-8877-bfc75d3a76ec` | `hf_20260909_215442_a09bbd21-1e5f-4460-8877-bfc75d3a76ec.mp4` · md5 `0c3bfcea273255ce4c70fb7244666308` · 1,254,443 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a woman in a yoga flow, moving from a deep lunge into a warrior pose and back, her arms sweeping up overhead and down, slow and continuous, in the middle of the page, pure black with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 24 | Seedance 2.5 — The cyclist (the first submission was intercepted by the "DROWN IN MUSIC" preset and created no job; resubmitted with `declined_preset_id f1821f84-945b-4cd1-9085-1f479db0028e`) · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `1fa776d0-1152-433c-bd25-7d0ab2c5681a` | `hf_20260909_215531_1fa776d0-1152-433c-bd25-7d0ab2c5681a.mp4` · md5 `46762d93cfd090340a0481ab46e56046` · 1,240,282 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a man on a stationary exercise bike seen side-on, pedalling hard, rising out of the saddle and sitting back down, in the middle of the page, the bike and the man one pure black shape with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 25 | Seedance 2.5 — The standing coach — replaces row 11 (the first submission was rate-limited, 429; resubmitted) · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `a51833aa-0ff6-417a-913d-9f620f786ba9` | `hf_20260909_215531_a51833aa-0ff6-417a-913d-9f620f786ba9.mp4` · md5 `951a57d84c7bb3d3e50f1ced4cb3c8af` · 1,549,975 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. Two jet-black figures in the middle of the page: a coach standing beside a client who is mid-squat with a barbell across the shoulders, the coach leaning in with one hand flat on the client's upper back and the other hand raised counting the rep on her fingers, the client rising and sinking in steady reps, both figures pure black with hard clean edges and no interior detail, no faces. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 26 | Seedance 2.5 — The skipper · block A, one take — 2026-09-09 21:54–21:55 UTC (`seedance_2_5` · 9:16 · 10 s · default 720p · `use_unlim false` · `declined_preset_id 24bae836-…`) | `bb8f5cfd-7e87-4cff-aa2e-e46ae4a0c248` | `hf_20260909_215442_bb8f5cfd-7e87-4cff-aa2e-e46ae4a0c248.mp4` · md5 `b3406264a15f85a461c456695ca1e076` · 1,687,501 B · 720×1280 · 241 frames / 10.04 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure, a man jumping rope in steady rhythmic jumps, the rope a thin black arc over his head, in the middle of the page, pure black with hard clean edges and no interior detail, no face. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 27 | Seedance 2.5 — the runner, image-to-video of E4 (USED) — 2026-09-09 22:00 UTC (`seedance_2_5` · **`mode omni_reference`** · 9:16 · 10 s · 720p · `use_unlim false` · `declined_preset_id 24bae836-…` · `medias` role `image` → `image_references cbff7e80…`). ⚠ Two submissions without `mode` were refused — *"mode 't2v' does not accept reference media"* — and `mode: "i2v"` was rejected as not in `[t2v, omni_reference, video_edit, video_extension]`; three more were rate-limited (429) before this one took | `ab1df882-c1b0-401a-9801-883c855f8596` | `hf_20260909_220042_ab1df882-c1b0-401a-9801-883c855f8596.mp4` · md5 `85fc624a08e527d4e95a3df5888ad0f0` · 8,140,281 B · 720×1280 · 241 frames / 10.05 s | *Animate this exact frame without changing its style: a flat 2D editorial newspaper animation. The page of cream newsprint, the masthead rule, the grey column blocks and the ticker strip stay perfectly still; the halftone dots stay locked to the page and do not shimmer. The runner sprints in place in full stride, knees driving high, arms pumping, ponytail streaming, her limbs streaking with motion, her hands open and empty, and the ribbon of teal light flows out of the fitness watch on her wrist and around her body in a continuous wave. The watch stays on her wrist in every frame, its teal face visible as her arm swings. Locked-off camera, no camera movement, no zoom. No new text, no logos.* |
| 28 | Seedance 2.5 — the driver, take 2 (recorded, unused: a head-and-shoulders at the wheel filling the lower frame, no car) — 2026-09-09 22:00 UTC (`seedance_2_5` · 9:16 · 10 s · **`resolution 1080p`** · `use_unlim false` · `declined_preset_id 24bae836-…`) | `10643761-3feb-40bc-afe0-6181f95a7bac` | `hf_20260909_220041_10643761-3feb-40bc-afe0-6181f95a7bac.mp4` · md5 `136dc1ce3f519b04fa35abdaf79d2e75` · 1080×1920 · 241 frames / 10.05 s | *Vertical 9:16. A flat 2D animation in an editorial newspaper style unique to a brand called Shape. The whole frame is a static page of cream newsprint that never moves: a bold double hairline rule across the top like a masthead, and a black ticker strip along the bottom with a teal left border, hex 34D6C5. One jet-black figure: a person seen from the side at the steering wheel of a car, really jamming out to music while driving — the head bobbing hard up and down on every beat, shoulders bouncing, one hand slapping the beat on top of the wheel, big joyful movement, a fitness watch on the wrist. The person and the steering wheel are solid pure black with hard clean edges and no interior detail, no face. Only a thin black line suggests the window and the dashboard; there is no solid car body, no door, nothing else black on the page, so the figure sits in the middle of the page with plain cream space all around. The cream page is plain and flat with no texture, no dots, no halftone, no pattern. Locked-off camera, no camera movement. No readable text, no logos, no phone.* |
| 29 | **Seedance 2.5 — the globe, B's prompt at 1080p (USED, the close)** — 2026-09-09 22:06 UTC (`seedance_2_5` · 9:16 · 12 s · **`resolution 1080p`** · `use_unlim false` · `declined_preset_id 24bae836-…`) | `9e7a3719-87d9-4363-b05b-8897168bf8b9` | `hf_20260909_220639_9e7a3719-87d9-4363-b05b-8897168bf8b9.mp4` · md5 `05eea2273dc676c505ee149793ef095d` · 8,919,101 B · 1080×1920 · 289 frames / 12.05 s | *Vertical 9:16. A photoreal planet Earth at night seen from space against pure black, the globe large in the middle of the frame with black space above and below it. The Americas face the camera at the start and the globe turns about a quarter turn eastward over the shot at a smooth, visible pace; city lights glow warm on the dark land. Thin lines of bright teal light, hex 34D6C5, rise straight up from the major cities one after another in a rhythm, like pins dropping onto a map, each line ending in a small glowing teal dot above the atmosphere, until dozens of them stand up around the globe. Night side only: no sunrise, no daylight, no sunlit clouds. No text, no logos, no labels.* |
| 30 | **Nano Banana Pro — the lifter, in the runner's print style (STILL, approved: *"these are much better"* · *"these images are solid"*)** — 2026-09-09 23:06 UTC (`nano_banana_pro`, served as `nano_banana_2` · 9:16 · 2k · `use_unlim false` · `medias` role `image` → `image_references cbff7e80…` (E4, the approved runner)) | `4f790eca-420e-43a4-be84-7b0171168c99` | `hf_20260909_230635_4f790eca-420e-43a4-be84-7b0171168c99.png` · md5 `3a63b113300dc7b33ec8793cb10e7c4a` · 3,439,949 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, energetic and high contrast, limbs streaking with motion; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch around the body, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a man at the top of a kettlebell swing, side-on, hips locked out, both arms straight and the kettlebell at eye height, his whole body one line from heels to bell, filling the page from the masthead to the ticker. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 31 | Nano Banana Pro — the cook at the island, print style (STILL, approved; owner: *"make the cooking someone cutting on an island"*) — 2026-09-09 23:06 UTC (same params as 30) | `c722b2da-2214-4590-98fc-c179f5c300c8` | `hf_20260909_230635_c722b2da-2214-4590-98fc-c179f5c300c8.png` · md5 `679fb4b6bc30ecb7b34b5bdb1e3a9205` · 4,966,008 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, energetic and high contrast; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a cook standing behind a kitchen island, seen from the front across the island, chopping vegetables on a wooden board with a chef's knife, a heap of sliced peppers and greens beside the board, the island a low simple slab in the same halftone dots taking up only the bottom third of the page, the cook's head and shoulders reaching up toward the masthead. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 32 | Nano Banana Pro — the yoga flow, print style (STILL, approved) — 2026-09-09 23:06 UTC (same params as 30) | `9a68e3fd-d070-4b9d-afd4-a5e946c2cdae` | `hf_20260909_230635_9a68e3fd-d070-4b9d-afd4-a5e946c2cdae.png` · md5 `c4b2bf07755368a383a861c936e1aa31` · 3,177,033 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, high contrast; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch around the body, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a woman in warrior two yoga pose, side-on, front knee bent deep, both arms stretched wide at shoulder height, gaze over the front hand, calm and still, filling the page from the masthead to the ticker. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 33 | Nano Banana Pro — the cyclist, print style (STILL, approved) — 2026-09-09 23:06 UTC (same params as 30) | `d17a6d82-4c6a-4481-ab86-1bd5e76c0141` | `hf_20260909_230635_d17a6d82-4c6a-4481-ab86-1bd5e76c0141.png` · md5 `db09bef0996115e09098f2a3c465c64c` · 3,806,669 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, energetic and high contrast, limbs streaking with motion; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a rider on an indoor spin bike, side-on, standing out of the saddle and leaning over the handlebars in a hard sprint, the bike a simple frame in the same halftone dots, the pair filling the page from the masthead to the ticker. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 34 | Nano Banana Pro — the coach and client, print style (STILL, approved) — 2026-09-09 23:06 UTC (same params as 30) | `a0bc085d-eacf-47a8-8e12-eb7c5b8938f7` | `hf_20260909_230635_a0bc085d-eacf-47a8-8e12-eb7c5b8938f7.png` · md5 `5b9bba2e5b4ba1d5fea8ec29c251b07d` · 3,830,687 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figures drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, high contrast; the client wears a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch, the only colour on the page apart from the ticker's border. But the figures are different people doing a different thing: a coach standing beside a client who is at the bottom of a barbell back squat, the coach leaning in with one hand flat on the client's upper back and the other hand raised counting the rep on her fingers, the two figures side by side filling the page from the masthead to the ticker. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 35 | Nano Banana Pro — the driver (Shape Radio), print style (STILL, approved) — 2026-09-09 23:06 UTC (same params as 30) | `37020ffd-d3c7-45b4-88e4-8094f170e4a9` | `hf_20260909_230636_37020ffd-d3c7-45b4-88e4-8094f170e4a9.png` · md5 `515ea19156815dafa3fc6aa008c0b3e6` · 3,982,456 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, high contrast; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a woman at the wheel of a car seen from beside the car through the open driver's window, one hand on the top of the wheel, the other hand up at her ear tapping a wireless earbud, head tilted back and mouth open singing along, hair loose; the car drawn only as the window frame, the wheel and the edge of the seat in the same halftone dots, no door panel, no solid shapes below the window, the page showing below. No phone. No readable text, no logos.\n\nresolution: 2k* |
| 36 | Nano Banana Pro — the skipper, print style (STILL, approved) — 2026-09-09 23:06 UTC (same params as 30) | `9237aa45-4702-475f-a661-ae41d13e1474` | `hf_20260909_230636_9237aa45-4702-475f-a661-ae41d13e1474.png` · md5 `e146fd4352fdb9b75780ecc67f912510` · 3,628,191 B · 1536×2752 | *Same page, same style, same technique as the reference image in every way — a vertical 9:16 animation frame in an editorial newspaper style unique to a brand called Shape: the whole frame a page of cream newsprint; a bold double hairline rule across the top like a masthead, thin vertical column rules with empty grey blocks and no letters, a black ticker strip along the bottom with a teal left border, hex 34D6C5; the figure drawn as a coarse halftone-dot illustration in black ink, like a press photograph screened into big dots, energetic and high contrast, limbs streaking with motion; a slim fitness watch with a flat teal face, hex 34D6C5, crisp and unscreened on the wrist; a single ribbon of teal light trailing from the watch, the only colour on the page apart from the ticker's border. But the figure is a different person doing a different thing: a boxer skipping rope, side-on, caught mid-jump with both feet off the ground, the rope a thin blurred arc over the head and under the feet, filling the page from the masthead to the ticker. No phone. No readable text, no logos.\n\nresolution: 2k* |
| — | ⚠ **REFUSED, NOT BILLED — Seedance 2.5, the globe re-prompt with at most eight beams** — submitted 2026-09-09 ~23:05, 23:08 (`use_unlim false`) and 23:09 (`use_unlim` omitted) UTC (`seedance_2_5` · 9:16 · 12 s · `resolution 1080p` · `declined_preset_id 24bae836-…`); every submission answered *"Out of credits on plus (monthly) plan in Private workspace"* — while the seven `nano_banana_pro` stills above were accepted in the same minutes, so the balance the video model reads is not the one the image model reads | — | — | *Vertical 9:16. A photoreal planet Earth at night seen from space against pure black, the globe very large, filling almost the whole width of the frame with black space above and below it. The Americas face the camera at the start and the globe turns about a quarter turn eastward over the shot at a smooth, visible pace; city lights glow warm on the dark land. Only a few thin lines of bright teal light, hex 34D6C5, rise straight up from the biggest cities one at a time, like pins dropping onto a map, each line ending in a small glowing teal dot above the atmosphere: never more than eight lines standing at once, well spaced around the globe, each one clean and separate with black space between them. Night side only: no sunrise, no daylight, no sunlit clouds. No text, no logos, no labels.* |
| — | ⚠ **REFUSED, NOT BILLED — Seedance 2.5, the lifter still (row 30) animated** — submitted 2026-09-09 23:12, 23:16 and 23:31 UTC (`seedance_2_5` · `mode omni_reference` · 9:16 · 10 s · `use_unlim false` · `declined_preset_id 24bae836-…` · `medias` role `image` → `4f790eca…`); all three answered *"Out of credits on plus (monthly) plan in Private workspace"*. The other six stills were not submitted: the same balance refuses them | — | — | *Animate this exact frame without changing its style: a flat 2D editorial newspaper animation. The page of cream newsprint, the masthead rule, the grey column blocks and the ticker strip stay perfectly still; the halftone dots stay locked to the page and do not shimmer. The man swings the kettlebell in a steady rhythm, hinging at the hips and snapping them forward so the bell rises to eye height and falls back between his legs, again and again, his arms straight, the ribbon of teal light flowing from the fitness watch on his wrist around his body in a continuous wave. Locked-off camera, no camera movement, no zoom. No new text, no logos.* |
