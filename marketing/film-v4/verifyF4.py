...
f=frame(bar(31)-0.3); wm=int((f[480:720].min(-1)>200).sum()); lines=cream(f[2000:2260,90:1350]);   # the end band is black in both looks and its lines cream, so 'ink' on the print look counted the band itself (review finding) chk('globe close',wm>6000 and lines>3000,f'wordmark white px {wm} band text px {lines} mean luma {float((0.299*f[...,0]+0.587*f[...,1]+0.114*f[...,2]).mean()):.1f}')
