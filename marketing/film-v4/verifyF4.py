...
for nm,T in starts.items():
    a00=frame(T-2/24); a0=frame(T-1/24); a1=frame(T); a=frame(T-0.15); b=frame(T+0.40); c=frame(T+0.45)   # ffmpeg -ss returns the first frame at or after t, so frame(T) is the crossfade's FIRST frame (e = 0.017), one frame after a0; frame(T+1/24) was its second (review finding)
    w0=float(np.abs(a0-a00).mean()); jump=float(np.abs(a1-a0).mean()); d1=float(np.abs(a-b).mean()); d2=float(np.abs(b-c).mean()); chk(f'seam {nm}',jump<max(w0,d2)+2+0.1*d1 and d1>d2+1,f'jump at the seam {jump:.1f} (old page motion {w0:.1f}) across {d1:.1f} within {d2:.1f}')   # one frame of the old page's motion, plus up to sstep(1/8) = 4 % of the page difference (a cream page into the black globe page is ~200 levels apart), with margin
...
# the end band is black in both looks and its lines cream, so 'ink' on the print look counted the band itself (review finding) -- and the first fix put this note on the line itself, after the semicolon, which commented out the check that followed it: one line fewer in the print run's tally, caught by counting the lines against the black look's
f=frame(bar(31)-0.3); wm=int((f[480:720].min(-1)>200).sum()); lines=cream(f[2000:2260,90:1350]); chk('globe close',wm>6000 and lines>3000,f'wordmark white px {wm} band text px {lines} mean luma {float((0.299*f[...,0]+0.587*f[...,1]+0.114*f[...,2]).mean()):.1f}')
