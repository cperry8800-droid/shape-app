...
⚠ **v7.2 — THE CARD IS NOW THE CUT'S HR LAYER AND THE PHYSICAL WATCH IS GONE.** Owner,
2026-09-03: *"this should be the beatmatching, remove the watch"*. So `in/A2.mp4` is no
longer the watch close-up clip — it is **Scene A continuing from frame `f12`**, which makes
the beat-12 cut invisible and puts no wrist on screen at all; this card is drawn across the
whole of Scene A at the wide inset and **replaces** `mk_watch.py`'s output. `meas_watch.py`
and `mk_watch.py` are out of the pipeline; `watchface()` and `card()` are both dead.

⚠ **AND THE FIRST v7.2 RENDER SHIPPED A FABRICATED READOUT — CAUGHT HERE, NOT BY A REPORT.**
`BPM` was **hardcoded 140**, carried over from the standalone preview below, which rides a
*separate* 140 BPM bed. But `render6.sh` scores the launch cut to **`in/t3.m4a`, measured
119.45 BPM** — so that render reads **`140 / 140 · IN SYNC` over a track playing 119.45**.
It is the exact thing this file's own v6 note forbids one section away (*"the station line
reads 120 BPM because that is the track's real tempo"*), and the honest-data doctrine does
not stop at the app's surfaces. **Fixed at the root rather than by retyping the number:**
`BPM` is now **derived from `meas_<track>.json`** — the same measurement the cut's grid comes
from — so the card cannot disagree with the music again, whichever track a future cut rides.
*A number typed beside a measurement will eventually contradict it.*
...
import math
S=480; FPS=24; N=243
P_BEAT=0.428571; PHI=0.055; MERGE_BEAT=16      # the MEASURED grid of THIS cut's own track
tM=PHI+MERGE_BEAT*P_BEAT; t0=1.0               # 6.912 s
# ⚠ DERIVED, NEVER TYPED. BPM is the measured tempo of the track this cut is scored to --
# read from meas_<track>.json, the same file the grid above comes from. Hardcoding it is how
# the first v7.2 render came to claim 140 over a 119.45 BPM track.
#   m = json.load(open('meas_t3.json')); BPM = int(round(m['bpm'])); P_BEAT = m['P']; PHI = m['phase_used']
BPM=int(round(json.load(open('meas_t3.json'))['bpm'])); HR0=BPM-36
