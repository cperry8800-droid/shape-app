# film3.py -- the one-shot film assembly, v3 (the Seedance 2.5 re-shoot; v2's script with the driver's cut and the globe's pins): two looks (LOOK=print | hl), activities only, crossfade seams, caption bands, the globe close.
...
GS=float(os.environ.get('GS','0.6')); GCY=int(os.environ.get('GCY','1480')); LOCK_CY=int(os.environ.get('LOCK_CY','330'))
def lockup_layers():
...
PIN_LUM=float(os.environ.get('PIN_LUM','170')); PIN_H=int(os.environ.get('PIN_H','44'))
class Pins:
    """Per frame: bright COOL pixels (a beam of teal or white light; a city light is warm and fails the test) grouped by column, each group's topmost pixel a tip. Tips are tracked across frames (matched within 28 px, position smoothed, drawn once seen twice) so the mark rides its beam as the globe turns."""
    def __init__(self): self.tracks=[]   # [x, y, frames unseen, frames seen]
    def tips(self,f):
        g=f.astype(np.float32); lum=0.299*g[...,0]+0.587*g[...,1]+0.114*g[...,2]
        cool=(lum>PIN_LUM)&((g[...,1]+g[...,2])>(2*g[...,0]+40)); cool[:120]=False; cool[H-340:]=False
        cols=np.nonzero(cool.any(0))[0]; out=[]
        if len(cols)==0: return out
        groups=[]
        for x in cols:
        ...
        return out
    def update(self,f):
        tips=self.tips(f); used=[False]*len(tips)
        ...
        for i,(x,y) in enumerate(tips):
            if not used[i]: self.tracks.append([x,y,0,1])
        self.tracks=[tr for tr in self.tracks if tr[2]<=6][:80]
        return [(tr[0],tr[1]) for tr in self.tracks if tr[3]>=2 and tr[2]==0]
...
class RuleFinder:
    ...
    def find(self,f):
        ...
        return self.cols
PLAN=json.load(open(f'{D}/plan3.json'))
...
class Page:
    ...
    def render_globe(self,t,k):
        pg=self.pg; u=t-pg['t0']; f=self.reader.read()
        small=Image.fromarray(f).resize((int(round(W*GS)),int(round(H*GS))),Image.LANCZOS); img=Image.new('RGBA',(W,H),(0,0,0,255)); img.paste(small,(W//2-small.width//2,GCY-small.height//2))
        fs=np.asarray(img.convert('RGB')); pts=self.pins.update(fs); self.pins.draw(img,k,pts)   # the pins are found on the page as drawn, so they sit on the smaller sphere
        if os.environ.get('PIN_DEBUG'): print('pins',round(t,3),'tips',len(self.pins.tips(f)),'drawn',len(pts),flush=True)
        al=sstep((u-4.0)/0.5)
        if al>0:
