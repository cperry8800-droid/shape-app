// Live program templates for the newdesign Trainer "Programs" page.
// Read-only over coach_program_templates (+ assignment counts). RLS scopes
// every row to the signed-in owner. Per-program "sold" / playlist metrics
// are intentionally absent — nothing in the schema tracks them.
...
export async function GET() {
  ...
  if (!trainerRow) {
    return NextResponse.json({ isTrainer: false, programs: [] });
  }
  ...
  return NextResponse.json({ isTrainer: true, programs });
}
