...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    {
      section: 'Access-control audit — COMPLETE 2026-07-30 (#1851 → 789a950cf; all 3 migrations APPLIED + VERIFIED LIVE)',
      items: [
        ...
        { label: '⚠ P2 open — /api/consultation is unauthenticated and puts the coach\'s private auth.users email into the ICS ORGANIZER line of a mail sent to an ATTACKER-CHOSEN address; iterate providerId to harvest every coach\'s login email. CLOSED — ORGANIZER is now a no-reply platform identity. ⚠ Moving it also took the coach OFF their own booking (neither organizer nor attendee = nothing to RSVP to), caught by Codex; there are now TWO invitations, same UID, each naming its own recipient as the attendee. The unauthenticated sessions-row insert that blocks a slot is UNCHANGED and still open', status: 'done' },
        ...
        { label: '⚠ P2 open — shared_client_acks: POST /api/me/shared-clients/[clientId]/ack never checks that the caller coaches that client, and the RLS policy pins only coach_user_id. Any authenticated user could write attacker-chosen rows, and the FK to auth.users made it an account-EXISTENCE oracle (200 for a real uuid, 500 on the FK violation for a fake one). CLOSED with is_coach_on_client + a counterpart-must-own-a-provider-row check. ⚠ REGISTERED: that counterpart check is the WEAKER of two options and deliberately so — subscriptions RLS is client-reads-own + provider-reads-own ONLY, so a request-scoped read of the counterpart\'s subscription returns NOTHING. Proving on-this-client membership needs a definer. Same fact means the shared-clients ROSTER may not work at all — worth checking', status: 'done' },
    ...
    {
      section: 'Search-term hardening — SHIPPED 2026-07-31 (#1853 → ed6c52e45; migration APPLIED + VERIFIED LIVE)',
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'P3 (OWNER/dashboard, unchanged): make "Secret scan (gitleaks)" a REQUIRED status check on main (GitHub -> Settings -> Branches). It runs on every PR today but is advisory, not gating', status: 'done' },
      ],
    },
    {
