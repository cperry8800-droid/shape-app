  ...
  return [
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER — AUTHOR the first published nora_sets rows via service-role SQL (title · dj · starts_at timestamptz · duration_min 10-360 · published=true): v1 ships no editor UI by design, so every schedule surface stays honestly empty ("Schedule lands with the first broadcast") until ops writes rows; a set covering now pins with the NOW tag but stays "broadcast coming soon" until a real stream is configured', status: 'manual' },
        ...
        { label: 'BUILDS 7-9/9 — The Cycle B→D in order: B member surfaces (settings/consents · cycle calendar · Today chip · Progress card · neutral Home lever · i18n ×13, disclaimer key PRIORITY for human review) → C coach (Case File CYCLE station + shared-overview leg; absence, never a padlock) → D website parity', status: 'pending' },
