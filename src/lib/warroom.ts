  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'OWNER, IN ORDER: (1) create the Sentry org + THREE projects — one per runtime, since Next.js/Capacitor/browser are three different SDKs with three different release streams; (2) supply SENTRY_DSN, NEXT_PUBLIC_SENTRY_DSN, VITE_SENTRY_DSN, SENTRY_ORG, SENTRY_PROJECT, SENTRY_AUTH_TOKEN; (3) REDEPLOY — Vercel injects env vars at BUILD time, not request time, so an already-running deployment never picks up the new values no matter how long it stays live', status: 'manual' },
