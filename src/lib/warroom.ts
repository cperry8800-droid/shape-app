  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'C1a — per-DAY menus · UNBLOCKED (the per-day storage contract is written — its own spec + the build, PRs #1839/#1840). The prerequisite for C: in iosAppBroadsheetPros.jsx, BSProAssignPage\'s apply() builds its week as `Array.from({ length: 7 }, (_, i) => ({ dow: i, …, meals }))` — ONE shared meals array across all seven days — so a client on an assigned meal plan eats the identical five meals every day; stack weekly rows on that and you get four weeks of the same five meals. ⚠ What blocks it: the flat `detail.blocks` shape is read by TWO consumers, and the obvious fixes break them in OPPOSITE directions — BSProAssignPage reads it to assign, and planPreview.mjs reads the SAME flat shape to render the buyer-facing marketplace preview of a PAID plan. Nesting days makes the paid preview render NOTHING; adding day metadata alongside makes all seven days render as one undifferentiated menu. Neither is acceptable on a commerce surface. The contract must name the stored shape, the legacy flat-block fallback, and ONE shared normalizer both consumers call. Locate all of this by SYMBOL, not by line — C0\'s own merge already rotted these (3362→3391 shared-meals, 5171→5222 trainer draft, 6052→6103 nutri draft)', status: 'pending' },
