  ...
  return [
    {
      ...
      items: [
        ...
        { label: 'OWNER on-device/branch-preview pass (live-web BUILD 1): the CROSS-MEMBER RLS DENIAL vector is recorded NOT RUN — it needs two distinct authenticated sessions (coach C1 linked to member M with a followers-tier row, vs stranger S) against the real backend; assert at the QUERY/EVENT layer (S gets zero rows + zero realtime callbacks, C1 gets both) before the DOM layer. A static stub cannot exercise RLS. Also: the station on a real in-progress session across trainer + nutritionist pages', status: 'manual' },
