...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    ...
    {
      section: 'Error tracking — PARKED 2026-07-30 (owner: profiles PII first; that work is now COMPLETE, so this is the next call)',
      items: [
        { label: '⚠ THERE IS NO ERROR TRACKING ON ANY SURFACE. Server (Next API routes): nothing but console.error into Vercel logs — ephemeral, unaggregated, no alerting. Website (public/newdesign): no global handler at all. Mobile: an error BOUNDARY plus bsRecordError (iosAppBroadsheetMain.jsx:2208) writing window.__BS_LAST_ERROR and localStorage shape.errorLog, last 20 — DEVICE-LOCAL, nothing is transmitted. So a production crash reaches us only if a user reports it or we are holding their phone. This codebase has shipped render-time crashes that passed every gate (#1781\'s TDZ), which is exactly the class this would catch', status: 'pending' },
        { label: 'Scope when it resumes: NO third-party tracker is wired (zero deps — no Sentry/Bugsnag/Rollbar/Datadog/PostHog, no instrumentation.ts), so this is a BUILD, not an install: an errors table + a RATE-LIMITED service-role ingest route (never a client-writable table — that is a spam vector) + client hooks on all three surfaces + a read surface (/console already exists). PARKED by the owner 2026-07-30 in favour of the profiles PII fix', status: 'pending' },
      ],
    },
    {
