  ...
  return [
    {
      ...
      items: [
        ...
        { label: 'Registered follow-ons from the review gauntlet (declined-with-rationale, NOT started): platform-wide media orphan-GC (remove/replace leaves the old object in a public bucket — same ruling as PR C, now scoped to include the film buckets) · persisted provider-slug column (the owner_id trigger resolves the coach by scanning provider rows; a stored slug would replace the scan) · re-key the general Reviews SECTION onto coach_reviews.owner_id (the wall already uses it; the section stays slug-keyed for now) · RLS-scope the account-delete storage purge (declined — documented admin-erasure exception) · P3 render verification on Signal/Terrain · on-device pass across the new profile surfaces', status: 'pending' },
      ],
    },
    {
