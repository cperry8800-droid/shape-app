  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER MIGRATION 2026-07-20-store-reprice-150.sql then 2026-07-20-tier-rewards.sql (in that order — the canteen must exist before a Tempo member can pick it). The tier-rewards file ALSO re-applies the corrected award_tier_bonuses: the live function drifted to an older build that grants +500 at a threshold of ZERO (a free welcome bonus two-thirds of the way to Tempo). Verified harmless — score_ledger is empty', status: 'manual' },
    ...
    },
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-20-radio-song-social.sql (radio_song_reactions + radio_song_comments + radio_norm_key + get/set/add RPCs). Public read of counts/comments; writes require sign-in. NOTE: the station is still a mock provider, so until a real http stream is configured, reactions accrue on the demo now-playing string — the mechanism is built ahead of the stream (the Nora Sets pattern) and lights up on real titles automatically', status: 'manual' },
