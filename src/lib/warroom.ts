  ...
  return [
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-19-nora-sets.sql: run it, then the post-apply grant/RLS proof (anon reads PUBLISHED rows only · unpublished invisible · authenticated INSERT/UPDATE/DELETE all FAIL at the grant layer), then AUTHOR the first published rows via service-role SQL (title · dj · starts_at · duration_min 10-360 · published=true) — v1 ships no editor UI by design, so every surface stays honestly empty until ops writes rows', status: 'manual' },
        { label: 'BUILDS 6-9/9 — The Cycle A→D in order: A engine (migration + cyclePhase.mjs deterministic windows/ladder + ShapeCycle data layer + /api/account/delete purge-list) → B member surfaces (settings/consents · cycle calendar · Today chip · Progress card · neutral Home lever · i18n ×13, disclaimer key PRIORITY for human review) → C coach (Case File CYCLE station + shared-overview leg; absence, never a padlock) → D website parity. ⚠ OWNER migration 2026-07-19-cycle-events.sql lands with A', status: 'pending' },
