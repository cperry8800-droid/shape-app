  ...
  return [
    {
      ...
      items: [
        ...
        { label: 'SPEC #1816 MERGED (905d3728, 5 review rounds incl. one security finding declined-with-rationale and formally WITHDRAWN by CodeRabbit): the profile wave, ALL TEN owner-ruled items — coach P1 intro film · P2 Line · P3 prompts · P4 business card · P5 wins wall (real coach_reviews ids, resolve-or-vanish in the profile\'s own slug-keyed set); member M1 Wall · M2 Start line (strict real-calendar local-day countdown) · M3 Shelf · M4 Line · M5 Film. New keys on the existing profile_custom doc (whole-doc visitor read verified) — zero new RPCs/routes; allowlisted save normalization keeps legacy keys byte-identical', status: 'done' },
        ...
        { label: 'OWNER MIGRATIONS (ride their build PRs, raw links per convention — NOT yet written): 2026-07-23-provider-listing-media.sql (additive listing_media column ×2 tables, box PR A) · 2026-07-23-member-films-bucket.sql (dedicated bucket, VIDEO MIMES ONLY, 60MB, owner-folder RLS — deliberately NOT a community-photos widening, profile PR E)', status: 'manual' },
        { label: 'Registered follow-ons from the review gauntlet: platform-wide signed/authenticated media (public-bucket URLs stay fetchable after a visibility flip — the existing architecture for ALL media surfaces; if it moves it moves wholesale, one migration) · re-key coach_reviews to the immutable provider owner_id (the wins wall + Reviews section are slug-keyed today; slug collision commingles both identically) · P3 render verification on Signal (Opus checks whether prompts already render there before building)', status: 'pending' },
      ],
    },
    {
