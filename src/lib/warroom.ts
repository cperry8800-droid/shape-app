  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Spec #1834 (OPEN, head 65bc0f20): owner ruled C (multi-week menus) + 8 rulings, all closed. The document now leads with an explicit build-ready table — C0 shipped · C1 build-ready · C2/C3 blocked on E · E split out and NOT build-ready', status: 'pending' },
        ...
        { label: 'C1 (NEXT) — per-day menus + make the ✦ AI DRAFT real. This is THE prerequisite for C: in iosAppBroadsheetPros.jsx, BSProAssignPage\'s apply() builds its week as `Array.from({ length: 7 }, (_, i) => ({ dow: i, …, meals }))` — ONE shared meals array across all seven days — so a client on an assigned meal plan eats the identical five meals every day; stack weekly rows on that and you get four weeks of the same five meals. And BOTH plan builders await `window.ShapeAI.generatePlanDraft`, render an ✦ AI DRAFT eyebrow, then build the outline from a hardcoded template — every coach who taps it gets a template and the call costs money to produce nothing. ⚠ Locate these by SYMBOL, not by line: C0\'s own merge shifted them (3362→3391 shared-meals, 5171→5222 trainer draft, 6052→6103 nutri draft) and any later edit will again — as of 5bebd73b they are :3391, :5222 and :6103. ⚠ The AI half is NOT just deleting the template: the builders send buildType `mealplan|program|diet` (nutri) and `plan|workout|program` (trainer) while /api/ai/generate-plan accepts only `workout|program|meal_plan` and NORMALIZES anything unrecognized to `workout` — so a nutritionist asking for a meal plan gets exercises back, invisible today only because the answer is discarded. C1 must map the kinds AND specify a seven-day response shape before consuming the response. Depends on NOTHING in E', status: 'pending' },
        { label: 'Paid-plan durability #1837 (OPEN, head 98883d18): one_time_purchases.plan_id is ON DELETE SET NULL (2026-06-08-coach-plans-sale.sql:12) and get_my_purchased_plans() INNER-joins the live coach_plans row (:45-46), so a coach DELETING a plan makes a paying client\'s purchase vanish from their Library and EDITING it silently changes what they bought. Surfaced by Codex while reviewing #1834; not caused by that design, so it does not wait for E. Adds plan_snapshot written at checkout + a LEFT-join fallback — the live row still WINS, since freezing paid content at purchase is a product call that belongs to E. Review then found a worse neighbour on the same path: when the coach deletes mid-checkout the FK rejects the stale plan_id, the error is only logged and the handler acks 200, so today the paid buyer gets NO purchase row at all — a known-deleted plan now records with plan_id NULL + an id-only marker, and the Library gate moved from has-an-id to has-a-NAME so it never renders an entry the client cannot open. Production currently has ZERO paid one-time purchases (verified live), so nobody has been hit — this is latent, closed before it can bite', status: 'pending' },
