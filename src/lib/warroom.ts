  ...
  return [
    {
      ...
      items: [
        ...
        { label: '⚠ REGISTERED, NOT BUILT — split out of the ack record 2026-07-31 (CodeRabbit, correctly): the shared-clients ROSTER may not function at all, and that was recorded inside an item marked done. The reasoning: subscriptions RLS is client-reads-own + provider-reads-own ONLY, so a request-scoped read of a COUNTERPART coach\'s subscription returns nothing — which is the same fact that forced the ack gate to settle for the weaker counterpart-owns-a-provider-row check. If the roster is built on that read it returns an empty list rather than erroring, so it would fail SILENTLY. Unverified either way — this is a check-then-fix, not a known break', status: 'pending' },
