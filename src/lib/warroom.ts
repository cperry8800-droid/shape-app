  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Input hardening: proxy rejects oversized bodies on ALL /api routes (413 by Content-Length; 1MB JSON / 30MB upload); shared readJson() (413 oversized + 400 malformed/empty) on the public write routes (contact/app-waitlist/intake/consultation/apply/community/support); fields already clamped (cleanText/isEmail/isISODate); no dangerouslySetInnerHTML so all output is escaped', status: 'done' },
