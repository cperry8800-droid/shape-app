  ...
  return [
    {
      ...
      items: [
        ...
        { label: 'OWNER MIGRATIONS (ride their build PRs, raw links per convention — NOT yet written; THREE across the wave): 2026-07-23-provider-listing-media.sql (additive listing_media column ×2 tables, box PR A) · 2026-07-23-coach-reviews-owner-id.sql (additive owner_id column + index + CONSERVATIVE backfill — stamp only on a single slug→provider match whose provider row predates the review, the tenure check; a bare slug join would reattach a previous slug-holder\'s reviews after rename/reuse; multi-match/unmatched/pre-tenure rows stay NULL + can never be pinned; write path stamps owner_id going forward — the wins wall resolves by id + owner, NULL never renders; profile PR D) · 2026-07-23-member-films-bucket.sql (dedicated bucket, VIDEO MIMES ONLY, 60MB, owner-folder RLS — deliberately NOT a community-photos widening, profile PR E)', status: 'manual' },
