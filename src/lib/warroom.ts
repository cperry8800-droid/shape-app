  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-19-cycle-events.sql: run it, then the post-apply validation on a throwaway account: direct cycle_settings upsert raises · direct cycle consent insert raises · non-cycle consent kinds pass untouched · cycle_set_settings writes flag+receipt atomically · a share receipt while NOT opted in raises one_flag_per_receipt (and a tracking receipt cannot move the share flag) · a future-dated period log raises future_event_date · get_client_cycle anon=false/authenticated=true · cycle_opt_out empties EVERYTHING incl. the settings doc + writes withdrawal receipt(s) · a direct settings-doc DELETE is a zero-row no-op. Until applied every ShapeCycle write reads reason:unavailable (by design)', status: 'manual' },
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-20-cycle-coach-today.sql (build 8): run AFTER 2026-07-19-cycle-events.sql. CREATE OR REPLACE get_client_cycle returning member-local today. APPLIED + verified live', status: 'manual' },
