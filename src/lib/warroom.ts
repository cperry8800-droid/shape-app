  ...
  return [
    {
      ...
      items: [
        ...
        { label: '⚠ REGISTERED, NOT BUILT — SCHEMA DRIFT, surfaced by a CodeRabbit finding that was itself wrong. `trainers public read` and `nutritionists public read` are LIVE IN PRODUCTION (SELECT, {anon,authenticated}, USING (true)) but ABSENT FROM supabase-migrations/. The repo cannot reproduce the database\'s access rules for its two most public tables — a rebuild from migrations would produce a DIFFERENT security posture than production has. CodeRabbit asked for the real-providers listing to be deleted on the grounds that no such policy exists in the migrations: literally true about the repo, wrong about the database, and it withdrew the finding on that evidence', status: 'pending' },
    ...
    {
      section: 'Error tracking — LAYER 2 SHIPPED 2026-08-01 (#1857 → eedc78dc5; migration APPLIED + VERIFIED LIVE 2026-08-01 — ⚠ the 2026-08-06 filename is a SEQUENCE marker, not an apply date). LAYER 1 (Sentry) SHIPPED 2026-08-01 on claude/error-tracking-layer-1-sentry, NOT YET MERGED — installed across all three runtimes, COMPLETELY INERT: no Sentry org, no DSN anywhere, nothing captured',
      items: [
        ...
        { label: '⚠ REGISTERED, NOT BUILT (from Layer 1): scripts/verify-staged.sh still runs a public/m sync diff that can now only ever fail — public/m has been gitignored and deploy-built since #1470, so the check compares a fresh build against stale local leftovers; remove the step or teach it the new reality', status: 'pending' },
