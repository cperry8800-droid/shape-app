  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'OWNER, IN ORDER: (1) create the Sentry org + FOUR projects — Next.js / Capacitor / static-website browser are different SDKs with different release streams; (2) supply EIGHT env vars: SENTRY_DSN, NEXT_PUBLIC_SENTRY_DSN, VITE_SENTRY_DSN, SHAPE_SITE_SENTRY_DSN, SENTRY_ORG, SENTRY_PROJECT, SENTRY_PROJECT_MOBILE, SENTRY_AUTH_TOKEN. ⚠ CORRECTED 2026-08-01 after Codex review — this line used to list SIX vars and THREE projects, and both of the additions close a hole that made the runbook untrue: without SHAPE_SITE_SENTRY_DSN the static website could NEVER have been activated at all (nothing in the repo set window.SHAPE_SENTRY_DSN, so setting every documented var and redeploying would have left it permanently unmonitored), and without SENTRY_PROJECT_MOBILE the /m/ sourcemaps are never uploaded, so every mobile stack trace arrives MINIFIED and the verification step below cannot pass; (3) REDEPLOY — Vercel injects env vars at BUILD time, not request time, so an already-running deployment never picks up the new values no matter how long it stays live', status: 'manual' },
        ...
        { label: 'RELEASE, END TO END — three of four surfaces produce a real git SHA on a Vercel deploy, one is genuinely absent. SERVER + EDGE: bsSentryRelease(process.env) reads VERCEL_GIT_COMMIT_SHA ✓. NEXT BROWSER: no explicit release key — withSentryConfig resolves it at build time (getSentryRelease() → VERCEL_GIT_COMMIT_SHA, else `git rev-parse HEAD`) and injects process.env._sentryRelease, which @sentry/nextjs\'s init() uses as its default ✓. ⚠ It USED to pass release: bsSentryRelease({SHAPE_RELEASE: NEXT_PUBLIC_SHAPE_RELEASE}) — a var set nowhere, so it evaluated to undefined, and because the SDK spreads user options LAST over its own defaults an own key holding undefined CLOBBERS the injected SHA. Deleting the key was the fix; the file now carries a comment saying never to add it back. MOBILE /m/: scripts/build-m.sh exports VITE_SHAPE_RELEASE from VERCEL_GIT_COMMIT_SHA ✓. STATIC WEBSITE: nothing stamps window.SHAPE_RELEASE, so it degrades to no release — honestly absent, not fabricated. So the design claim that one deploy correlates across surfaces HOLDS for server, edge, Next browser and mobile, and does NOT yet cover the static website', status: 'done' },
