...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    {
      section: 'Progression guardrails (#1846 MERGED 0f7806c6 · 2026-07-28) — Deploy 2b is the open half',
      items: [
        ...
        { label: 'OWNER on-device pass (2): THE LATCHED CLOCK — finish a session UNDER A MINUTE, type a duration, and wait past the minute mark. The clock is latched when the screen opens so the question cannot change under the member mid-answer. This is the ONE perturbation survivor: time cannot pass in the harness, so it has no automated guard', status: 'manual' },
        { label: 'Deploy 2b — NOT BUILT. The history-reading RPC + the builder/publish integration. Its contract is SPEC-guardrails.md §4.1: sessions carry startedAtISO + timezone, NEVER a pre-localised dateISO (that was the same mistake as pre-aggregated loadAu — it puts untestable conversion in SQL), and the `reason` vocabulary has NINE values that copy and telemetry both key off', status: 'pending' },
