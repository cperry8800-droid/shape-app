  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'CodeRabbit review pass (commit 54ff99b): server-authoritative AI grounding (draft-message builds from get_client_stats, not caller input); audit-write failure after execute no longer throws (returns audited:false + logs); notify parallelized + dedup state persisted BEFORE delivery; UPDATE-policy cross-client reassignment locked via assignment-key-immutability triggers; self-written detail.directive stripped; mic-stream cleanup; no raw DB-error leaks. 225/225 tests · tsc · next build · public/m in sync', status: 'done' },
        { label: 'Migrations — 6 applied live (ai-audit-log, ai-proposal-nonces, nutrition-compliance, source-reconcile, notification-center, program-detail-discipline); CONFIRM 3 on Supabase, safe to re-run: 2026-06-17-coach-write-scope.sql (now incl. the assignment-key immutability triggers), 2026-06-17-review-note-delete.sql, 2026-06-17-dietitian-role.sql', status: 'manual' },
      ],
    },
    {
