  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'C1b — make the ✦ AI DRAFT real · BLOCKED on the AI contract. BOTH plan builders await `window.ShapeAI.generatePlanDraft`, render an ✦ AI DRAFT eyebrow, then build the outline from a hardcoded template — every coach who taps it gets a template and the call costs money to produce nothing. ⚠ It is NOT just deleting the template, and NOT just a rename: the builders send buildType `mealplan|program|diet` (nutri) and `plan|workout|program` (trainer) while /api/ai/generate-plan accepts only `workout|program|meal_plan` and NORMALIZES anything unrecognized to `workout` — so a nutritionist asking for a meal plan gets exercises back, invisible today only because the answer is discarded. The two `program` modes COLLIDE without resolving wrongly, so a rename cannot fix it. C1b needs a six-mode contract mapping every builder kind to a route mode AND a seven-day response shape, before anything consumes the response', status: 'pending' },
