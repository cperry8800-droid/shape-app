...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    ...
    {
      section: 'Error tracking — LAYER 2 SHIPPED 2026-08-01 (#1857 → eedc78dc5; migration APPLIED + VERIFIED LIVE 2026-08-01 — ⚠ the 2026-08-06 filename is a SEQUENCE marker, not an apply date). LAYER 1 (Sentry) still NOT STARTED',
      items: [
        ...
        { label: '⚠ ALERTS CURRENTLY REACH VERCEL LOGS AND NO HUMAN. reportAlerts() in the route is the single seam Layer 1 replaces; until then this job files findings nobody is notified about. ⚠ HEARTBEAT_PING_URL is UNSET, so the dead-man\'s switch is INERT and every run reports heartbeat:"skipped" — NOTHING NOTICES IF THIS CRON STOPS FIRING, which is the same absence-needs-a-presence-signal problem the job exists to solve, one level up. Deliberately a provider-agnostic plain GET rather than Sentry.captureCheckIn, so it is not blocked on Layer 1: Sentry Cron Monitors, Healthchecks.io and Cronitor all accept the same ping. OWNER: set the env var', status: 'pending' },
        { label: '⚠ LAYER 1 (Sentry) NOT STARTED — blocked on an owner account. NO third-party tracker is wired anywhere (zero deps, no instrumentation.ts). Scope: ONE org, THREE projects — @sentry/capacitor + @sentry/react for /m/ (⚠ NOT React Native: mobile-app is Capacitor, so the reactNative wizard matches nothing here), @sentry/nextjs covering pages AND all 156 API routes in one install, @sentry/browser for the static public/newdesign. Release = git SHA injected into scripts/build-m.sh too so mobile and web correlate. User context = roles ARRAY + is_coach (dual-role accounts are real), never email/name — restricting profiles PII in #1851 and then shipping it to a third party would be incoherent. Source maps: sourcemap:"hidden" on the /m/ Vite build (generated + uploaded, no sourceMappingURL, so nothing is publicly reachable — the owner\'s stated objection was exposure). The website needs none: build-newdesign.mjs does JSX transform only, no minification. OWNER: create the org + 3 projects, supply DSNs + an auth token, AND create the alert rules — captureMessage files an issue, it does not notify anyone', status: 'pending' },
