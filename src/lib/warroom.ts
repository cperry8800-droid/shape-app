  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'OWNER: run supabase-migrations/2026-07-18-user-activity-live.sql (additive + idempotent; raw link on the build PR). Until applied, every live-progress call is a SILENT NO-OP: the boost sheet is byte-identical to today (no row -> no line), while the coach console on a REAL client shows the neutral "live detail unavailable" line instead of the old demo grid — deliberate, and true with or without the migration, because a real client must never be shown fabricated demo sets (demo roster entries still get the demo grid)', status: 'manual' },
