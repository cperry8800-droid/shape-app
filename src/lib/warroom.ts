  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: '✓ LAYER 1 (Sentry) SHIPPED 2026-08-01 — installed, not activated. Sentry wired into all three runtimes: @sentry/nextjs across pages + all 156 API routes (one install), @sentry/capacitor + @sentry/react in the /m/ app (⚠ NOT React Native: mobile-app is Capacitor, so the reactNative wizard matched nothing here), and on the static public/newdesign a dependency-free sentryInit.js classic script that CDN-loads the Sentry browser bundle by SRI hash (no npm @sentry/browser dependency — no bundler on that surface). ONE shared PII-limited tagging module, src/lib/sentry-context.mjs, is the only user context any of the three ever attach: user id, roles as the ARRAY (dual-role accounts are real), and is_coach — never email or name, since restricting profiles PII in #1851 and then shipping it to a third party would be incoherent. ⚠ INSTALLED BUT COMPLETELY INERT — there is no Sentry org and no DSN anywhere, so every SDK sits in the documented pre-account no-op state (Sentry.init({dsn:\'\'}) and its Capacitor/classic-script equivalents construct no transport). Do NOT read this as error tracking being live', status: 'done' },
        ...
        { label: '⚠ REGISTERED, NOT BUILT (from Layer 1): scripts/verify-staged.sh still runs a public/m sync diff that can now only ever fail — public/m has been gitignored and deploy-built since #1470, so the check compares a fresh build against stale local leftovers; remove the step or teach it the new reality. Also: the static website attaches only a coarse, path-derived shape_surface tag rather than real user context, because that surface has no bundler and cannot reach the shared PII-free sentry-context.mjs module — most of the 69 pageShell.jsx pages are the SIGNED-IN DASHBOARDS, not marketing, so once a DSN exists, dashboard errors there will carry no role or tier beyond the URL-path hint', status: 'pending' },
