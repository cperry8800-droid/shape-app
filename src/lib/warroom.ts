  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: '⚠ Spec #1834 (OPEN) — NOTHING IN THIS WAVE IS CURRENTLY BUILD-READY. The owner ruled C (multi-week menus) and all 8 rulings are closed, but a closed ruling is not a build signal: the document now leads with a build-ready table reading C0 shipped · C1a UNBLOCKED (contract written, #1839/#1840) · C1b blocked on the AI contract · C2/C3 blocked on E · E split out and NOT build-ready. It also carries a standing warning, because readiness has been claimed wrongly THREE times (C1 whole → the AI route cannot express what the builders send; C1b → a rename cannot fix it, nutri and trainer `program` collide; C1a → independence from E and from the AI route is not readiness). Every time, readiness was inferred from INDEPENDENCE. A track that depends on nothing blocked is not thereby ready; it is ready when the contract it writes THROUGH is defined. Do not re-mark any track ready without naming the stored shape it produces and every consumer that reads it', status: 'pending' },
        ...
        { label: 'C1a — per-DAY menus · UNBLOCKED and BUILT (contract PR #1839 · build PR #1840, both in review). What the contract ESTABLISHED (past tense — these are decisions, not open questions): `detail.days` is ADDITIVE and optional, `detail.blocks` stays as the DEFAULT menu any unauthored day inherits, so every legacy plan behaves byte-identically with no backfill and no migration; ONE shared `bsPlanWeek(detail)` normalizer in planOutline.mjs serves BOTH BSProAssignPage (delivery) and planPreview.mjs (the buyer-facing paid preview), so the week a buyer is shown cannot differ from the week that gets installed; the paid preview shows all seven day labels FREE (structure, counts only) with meals on the existing paid allowance, locked = units - free ALWAYS; the legacy default is deliberately UNCAPPED in delivery (a cap would drop sold meals — bounds apply only to the new `days` entries); `perDay` is computed from the RESOLVED week (seven identically-authored days are ONE menu, not a per-day plan). Still open WITHIN C1a: the editor day tabs (contract §6 — find-by-dow never days[dow], authored-empty publishes {dow, blocks: []}) land as their own PR; nothing can WRITE detail.days until then, so the mechanism is inert in production', status: 'pending' },
