...
// `declined` = a closed review decision with no work planned. It is deliberately
// NOT `pending` (it would show up as an open go-live step and block its section)
// and NOT `done` (nothing was built). The client excludes it from the go-live
// counts and renders it as a non-tickable record.
export type ChecklistItem = { label: string; status: 'done' | 'pending' | 'manual' | 'declined' };
...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    {
      section: 'Coach box + profile customization wave (specs 2026-07-23 → BUILD COMPLETE 2026-07-24 · all 3 migrations APPLIED)',
      items: [
        ...
        { label: 'BUILD SHIPPED (owner go via "continue" + "merge when clean and continue build"): box PR A/B → profile PR C (member M1–M4) → PR D #1821 (coach P1–P5 + coach_reviews.owner_id trust boundary, main fc6b9e12, 10 review rounds) → PR E #1822 (member M5 film + member-films bucket + dedicated filmMember key, main 322822b2, 11 review rounds). Every build ran the full CI + CodeRabbit + Codex gate; a pre-push adversarial review caught a Critical (member film serialized only in the coach spread → never persisted) before the first push', status: 'done' },
        { label: 'ALL THREE OWNER MIGRATIONS APPLIED + VERIFIED LIVE 2026-07-24 (checked read-only against prod): 2026-07-23-provider-listing-media.sql (listing_media jsonb on trainers + nutritionists — both columns present, box PR A) · 2026-07-23-coach-reviews-owner-id.sql (owner_id column + index present; SECURITY DEFINER trigger stamps from slug, immutable on identity-stable edit, re-derived on slug/kind repoint, honors the FK on delete set null — NO BACKFILL, so pre-migration rows stay NULL forever, profile PR D) · 2026-07-23-member-films-bucket.sql (member-films bucket present: public, 60MB, video-mimes-only [mp4/quicktime/webm/x-m4v/m4v], 4 owner-folder RLS policies, profile PR E)', status: 'done' },
        { label: 'REGISTERED ENGINEERING FOLLOW-ONS — not started: platform-wide signed/authenticated media (public-bucket object URLs stay fetchable after a profile flips to friends/private — the spec\'s documented residual; it is the existing architecture for ALL media surfaces, so if it moves it moves wholesale, one migration) · platform-wide media orphan-GC (remove/replace leaves the old object in a public bucket — same ruling as PR C, now scoped to include the film buckets) · persisted provider-slug column (the owner_id trigger resolves the coach by scanning provider rows; a stored slug would replace the scan) · re-key the general Reviews SECTION onto coach_reviews.owner_id (the wall already uses it; the section stays slug-keyed for now) · P3 render verification on Signal/Terrain', status: 'pending' },
        { label: 'DECLINED IN REVIEW (written rationale, accepted residual — no work planned unless the ruling changes): RLS-scope the account-delete storage purge — /api/account/delete is service-role BY DESIGN, because several purged tables (user_goals, which holds the health profile, plus user_integrations/OAuth tokens, score_ledger, messages and the coach rows) have NO user-facing DELETE policy, so an RLS-scoped client would SILENTLY fail to erase exactly the most sensitive data while reporting success. Documented admin-erasure exception, scoped to the verified caller\'s own id on every delete', status: 'declined' },
        { label: 'OWNER on-device pass across the new profile surfaces (the wave\'s remaining owner-side item — the migrations are done): coach Signal P1–P5 (intro film · the Line · prompts · business card · the Wins wall pinning real reviews) + member Terrain M1–M5 (the Wall · Start line · the Shelf · the Line · the intro film), each on a real signed-in account across the papers; confirm a coach film and a member film coexist (per-role film/filmMember keys) and that a member film uploads to member-films, not coach-media', status: 'manual' },
      ],
    },
    {
