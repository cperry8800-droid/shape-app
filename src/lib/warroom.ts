...
function buildChecklist(config: ConfigGroup[], mobileBuild = false): ChecklistSection[] {
  ...
  return [
    {
      section: 'Coach box + profile customization wave (specs 2026-07-23 — ⚠ BUILD OWNER-PAUSED)',
      items: [
        ...
        { label: '⚠ BUILD GATE — OWNER: the explicit build go ("pause before you start build" stands; recorded in session memory + the records pass was owner-ordered). Once given, Opus builds in order: box PR A (migration + canonical module + mobile) → PR B (web) → profile PR C (member M1–M4) → PR D (coach P1–P5 + shared video player) → PR E (M5 film + bucket migration)', status: 'manual' },
        { label: 'OWNER MIGRATIONS (ride their build PRs, raw links per convention — NOT yet written; THREE across the wave): 2026-07-23-provider-listing-media.sql (additive listing_media column ×2 tables, box PR A) · 2026-07-23-coach-reviews-owner-id.sql (additive owner_id column + index — NO BACKFILL, final ruling: every heuristic mis-attaches after slug rename/reuse (bare slug join hands the previous holder\'s reviews to the current one; a provider-created_at tenure check fails because row age is not slug-tenure age) and no immutable slug-history source exists, so ALL pre-migration rows stay NULL permanently — never render, can\'t be pinned; the write path stamps every new review — the wins wall resolves by id + owner; profile PR D) · 2026-07-23-member-films-bucket.sql (dedicated bucket, VIDEO MIMES ONLY, 60MB, owner-folder RLS — deliberately NOT a community-photos widening, profile PR E)', status: 'manual' },
