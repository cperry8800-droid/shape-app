  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'C1 (NEXT) — per-day menus + make the ✦ AI DRAFT real. This is THE prerequisite for C: iosAppBroadsheetPros.jsx:3362 assigns the SAME meals array to all seven days, so a client on an assigned meal plan eats the identical five meals every day — stack weekly rows on that and you get four weeks of the same five meals. And both builders (:6052 nutritionist, :5171 trainer) already await generatePlanDraft, render an ✦ AI DRAFT eyebrow, then build the outline from a hardcoded template — every coach who taps it gets a template and the call costs money to produce nothing. Depends on NOTHING in E', status: 'pending' },
