  ...
  return [
    {
      ...
      items: [
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-19-roster-weekly-adherence.sql: run it, then verify live. Until applied the RPC is absent and every surface degrades silently to no chip / no line (by design). Validated read-only pre-merge: all 24 referenced columns exist and the full CTE chain parses/plans/executes; NOT applied by the agent', status: 'manual' },
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-19-user-activity-live-expiry-rls.sql: moves expiry INTO the audience read path so an expired row cannot be fetched by a DIRECT select before cleanup (the v1 policy left expiry to calling code, which is no longer a good enough bound now a meal TITLE rides the row). Owner leg stays unfiltered so a member can always clear their own stale row. Hardening only — nothing depends on it to function. Post-apply proof: an expired seeded row is invisible to a second account but readable by its owner', status: 'manual' },
        ...
        { label: '⚠ OWNER MIGRATION — 2026-07-19-user-activity-live-coach.sql: run it, then verify live. Until applied the table is absent and every path degrades silently to today\'s "—". Pre-verified read-only: is_coach_on_client exists + is SECURITY DEFINER, and the pg_column_size CHECK was probed on a TEMP table (pg_column_size is STABLE not IMMUTABLE and could have been rejected at apply time) — accepted, and it rejects a 20KB payload', status: 'manual' },
        ...
        { label: 'BUILD 5/9 — nora-sets-schedule (any time): nora_sets table (revoke-then-grant, published-only read) + bsSetsNow (end-exclusive windows) + COMING UP on Shape Sets + stream-gated LIVE/coming-soon on the radio screen + website list (ShapeSetsLib namespace). ⚠ OWNER migration 2026-07-19-nora-sets.sql', status: 'pending' },
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Live progress v2 candidates (registered, NOT built): a coach-only richer channel carrying loads/RPE (v1 broadcasts none because the audience can include followers/public) · a server-side trigger that re-derives `visibility` from client_settings instead of trusting the client stamp (declined for v1 with receipts — the stamp is a preference on the member\'s OWN payload, the identical trust model to community_posts.privacy #1613) · cooking detail · website surfaces', status: 'pending' },
