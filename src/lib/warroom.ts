  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'REGISTERED, NOT BUILT — route boundary-caught React crashes to Sentry. BSErrorBoundary is the mobile app ONLY error boundary and its componentDidCatch writes solely to localStorage; React 19 defaultOnCaughtError is literally console.error, so a boundary-caught render crash never fires the window error event and can never reach Sentry. There are ZERO captureException call sites in mobile-app/src. This is the single biggest gap in what Layer 1 mobile will actually catch once a DSN exists — but it is PRE-EXISTING, in a file this branch does not touch, and inert until a DSN exists, so it gets its own PR with a mount test per the render-check rule rather than riding this one', status: 'pending' },
