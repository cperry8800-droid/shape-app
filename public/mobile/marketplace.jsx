...
function MarketplaceHeroBg({ tab }) {
  const src = tab === "Trainer" ? "assets/marketplace-trainer-bg.png" : "assets/marketplace-nutrition-bg.png";
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 80, alignItems: "end" }}>
          <h1 style={{ fontFamily: serif, fontSize: tab === "Trainer" ? 112 : 96, lineHeight: 0.88, letterSpacing: "-0.035em", fontWeight: 400, margin: 0, color: INK, whiteSpace: "nowrap" }}>
            Find the <em style={{ fontStyle: "italic", fontWeight: 500, color: TEAL }}>{tab === "Trainer" ? "coach" : "nutritionist"}</em><br />who fits.
          </h1>
...
function Filters({ tab, cat, setCat, sort, setSort, format, setFormat, loc, setLoc, query, setQuery }) {
  ...
  const small = (on) => ({ padding: "7px 11px", borderRadius: 999, border: on ? `1px solid ${TEAL}` : "1px solid rgba(242,237,228,0.18)", background: on ? "rgba(10,197,168,0.12)" : "transparent", color: on ? TEAL : "rgba(242,237,228,0.75)", fontFamily: sans, fontSize: 11.5, cursor: "pointer", fontWeight: on ? 500 : 400, whiteSpace: "nowrap", letterSpacing: "0.02em" });
