...
function CoachLiveWorkoutSession({ clientId, clientName = 'client', role = 'trainer', accent = '#2ee0c4', loadRuntime = coachLiveRuntime }) {
  ...
  const button = { minHeight: 44, borderRadius: 8, padding: '10px 14px', border: '1px solid rgba(242,237,228,0.3)', color: '#f2ede4', background: 'transparent', fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, cursor: 'pointer' };
  const cell = { padding: '10px 6px', textAlign: 'left', borderBottom: '1px solid rgba(242,237,228,0.1)', overflowWrap: 'anywhere' };
  ...
  return (
    <Card style={{ marginBottom: 20, borderColor: `${accent}66`, minWidth: 0 }}>
      <section aria-label={`Live workout for ${clientName}`} className="coach-live-workout" style={{ color: '#f2ede4', minWidth: 0 }}>
        ...
        {!lp && identityValid && <p style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(242,237,228,0.8)', marginBottom: 0 }}>{status === 'connecting' ? 'Checking for an active workout.' : status === 'idle' ? 'This view updates automatically when workout progress is available.' : 'Checking the connection. You can retry while automatic recovery continues.'}</p>}
        ...
        {lp && <>
          ...
          {updated && Number.isFinite(updated.getTime()) && <p style={{ ...mono, fontSize: 11, color: 'rgba(242,237,228,0.75)', margin: '0 0 14px' }}>Last workout update {updated.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', second: '2-digit' })} · Read only</p>}
          {lp.curIdx < 0 && <p style={{ fontSize: 14 }}>No exercise selected.</p>}
          {lp.exercises.map((exercise, i) => <details key={i} open={i === lp.curIdx} style={{ borderTop: '1px solid rgba(242,237,228,0.18)', padding: '8px 0' }}>
            <summary style={{ lineHeight: 1.6, padding: '7px 0', overflowWrap: 'anywhere' }}>
              <span style={{ ...mono, color: i === lp.curIdx ? accent : 'rgba(242,237,228,0.8)', marginRight: 10 }}>{i === lp.curIdx ? 'NOW' : exercise.done === exercise.total ? 'DONE' : String(i + 1).padStart(2, '0')}</span>
            ...
            {cp && Array.isArray(exercise.sets) ? <>
              <table aria-label={`${exercise.n} logged sets`} style={{ ...mono, fontSize: 12 }}>
                <thead><tr>{['Set', cp.loadUnit ? `Load (${cp.loadUnit})` : 'Load', 'Reps', 'RPE', 'Status'].map(label => <th scope="col" key={label} style={{ ...cell, fontSize: 11, color: 'rgba(242,237,228,0.8)' }}>{label}</th>)}</tr></thead>
                <tbody>{exercise.sets.map((set, j) => <tr key={j}><th scope="row" style={cell}>{j + 1}</th><td style={cell}>{set.done && set.load ? set.load : '—'}</td><td style={cell}>{set.done && set.reps ? set.reps : '—'}</td><td style={cell}>{set.done && set.rpe ? set.rpe : '—'}</td><td style={{ ...cell, color: set.done ? accent : 'rgba(242,237,228,0.75)' }}>{set.done ? 'Done' : 'Pending'}</td></tr>)}</tbody>
              </table>
            ...
            </> : <p style={{ fontSize: 14, color: 'rgba(242,237,228,0.8)' }}>Set counts available. No load, reps or RPE readings available.</p>}
          </details>)}
          <form onSubmit={sendCue} style={{ borderTop: '1px solid rgba(242,237,228,0.18)', marginTop: 12, paddingTop: 20 }}>
            <label htmlFor={inputId} style={{ ...mono, display: 'block', marginBottom: 10 }}>Send a cue</label>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <input id={inputId} value={cue} disabled={sending || !canSend} onChange={event => setCue(event.target.value)} maxLength={500} placeholder="Type a quick cue…" style={{ flex: '1 1 220px', minWidth: 0, width: '100%', boxSizing: 'border-box', border: '1px solid rgba(242,237,228,0.35)', borderRadius: 8, padding: '12px 14px', background: 'rgba(242,237,228,0.04)', color: '#f2ede4', fontSize: 16 }} />
            ...
            <p style={{ fontSize: 12, color: 'rgba(242,237,228,0.75)', lineHeight: 1.5 }}>Cues also update your coaching focus note.</p>
