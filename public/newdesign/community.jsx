      ...
      <div style={{ maxWidth: 1320, margin: "0 auto", position: "relative" }}>
        ...
        <div className="cm-hero" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 80, alignItems: "end" }}>
          <h1 style={{ fontFamily: serif, fontSize: "clamp(56px, 8.5vw, 124px)", lineHeight: 0.86, letterSpacing: "-0.045em", fontWeight: 300, margin: 0 }}>
            Show up <em style={{ fontStyle: "italic", fontWeight: 600, color: "transparent", WebkitTextStroke: `1.4px ${INK}` }}>alone</em>.<br />Leave <em style={{ fontStyle: "italic", fontWeight: 600, color: "transparent", WebkitTextStroke: `1.4px ${INK}` }}>together.</em>
          </h1>
            ...
            <CmReveal key={i} delay={Math.min(i * 50, 200)}>
              <article style={{ padding: 30, background: "rgba(11,14,12,0.62)", borderRadius: 6, border: "1px solid rgba(242,237,228,0.1)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 18 }}>
                  ...
                  <span style={{ fontFamily: mono, fontSize: 10, padding: "4px 10px", borderRadius: 999, background: "rgba(10,197,168,0.1)", color: TEAL, letterSpacing: "0.08em", textTransform: "uppercase" }}>{e.tag}</span>
                </div>
...
function Chat() {
  ...
  const ROOMS = {
    ...
    community: {
      ...
      sub: "#shape-community",
      header: "#shape-community",
      context: "All Shape members · 18,420 in · pinned: Read first — house rules",
      channels: [
        { name: "#shape-community", members: 18420, active: true, all: true },
        { name: "#brooklyn-runs", members: 420 },
        { name: "#strength-club", members: 1280 },
        { name: "#plant-based", members: 860 },
      ...
      right: "IN THIS ROOM",
      roster: [["Maya Okafor", "Coach · online", true], ["Rae Lindqvist", "Coach · online", true], ["Priya S.", "Member · Brooklyn", false], ["Marcus J.", "Member · Austin", false], ["Ava K.", "Member · Chicago", false], ["Diego M.", "Member · LA", false], ["+ 18,414 more", "", false]],
      messages: [
        { who: "Shape", role: "Mod", init: "SH", time: "Mon 9:00", msg: "Welcome to #shape-community — every member, every coach, one room. Keep it kind, keep it useful. House rules pinned below.", pin: true },
        { who: "Priya S.", role: "Member", init: "PS", time: "9:02", msg: "First marathon block done — 16 weeks in the books, race Sunday. Anyone else running NYC?" },
        { who: "Marcus J.", role: "Member", init: "MJ", time: "9:05", msg: "Not running it but cheering from mile 18 if anyone wants a high-five 🙋" },
        { who: "Ava K.", role: "Member", init: "AK", time: "9:09", msg: "Looking for a strength coach who's good with shoulder rehab. Any recs? Open to remote." },
        { who: "Maya O.", role: "Coach", init: "MO", time: "9:14", msg: "Ava — DM me. I run a shoulder-friendly hypertrophy block with a few clients post-PT. Happy to do a free intro call.", highlight: true },
        { who: "Diego M.", role: "Member", init: "DM", time: "9:18", msg: "Anyone tried Rae's overnight oats template? Macros are 🤌 and prep is 4 minutes." },
        { who: "Rae L.", role: "Coach", init: "RL", time: "9:22", msg: "Glad it's landing. Posting a vegetarian variation in #plant-based later today.", highlight: true },
      ],
      placeholder: "Message #shape-community",
    },
  };
  const r = ROOMS[tab];
  return (
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        <div className="cm-chat" style={{ background: "rgba(11,14,12,0.55)", border: "1px solid rgba(242,237,228,0.1)", borderRadius: 6, display: "grid", gridTemplateColumns: "260px 1fr 260px", minHeight: 540, overflow: "hidden", backdropFilter: "blur(14px) saturate(1.05)", WebkitBackdropFilter: "blur(14px) saturate(1.05)" }}>
          <aside className="cm-chat-aside" style={{ borderRight: "1px solid rgba(242,237,228,0.08)", padding: "24px 0", background: "rgba(242,237,228,0.02)" }}>
            ...
            {r.channels.map((c, i) => (
              <div key={i} style={{ padding: "10px 20px", background: c.active ? "rgba(10,197,168,0.08)" : "transparent", borderLeft: c.active ? `2px solid ${TEAL}` : "2px solid transparent", display: "flex", gap: 10, alignItems: "center", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: 24, flex: 1, display: "flex", flexDirection: "column", gap: 14 }}>
              {r.messages.map((m, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "36px 1fr", gap: 12, alignItems: "start", padding: m.highlight ? "12px 14px" : 0, background: m.highlight ? "rgba(10,197,168,0.07)" : "transparent", borderRadius: 6, border: m.highlight ? `1px solid rgba(10,197,168,0.25)` : "none" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
                      ...
                      {m.role === "Coach" && <span style={{ fontFamily: sans, fontSize: 10, padding: "2px 7px", borderRadius: 999, background: "rgba(10,197,168,0.12)", color: TEAL, letterSpacing: "0.06em" }}>COACH</span>}
          ...
          <div style={{ maxWidth: 1320, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", border: `1px solid ${TEAL}33`, background: `${TEAL}0f`, borderRadius: 16, padding: "16px 20px" }}>
              ...
              <div style={{ flex: 1, minWidth: 240 }}>
                ...
                <div style={{ fontFamily: sans, fontSize: 15, lineHeight: 1.5, color: "rgba(242,237,228,0.78)" }}>
                  The activity below is a sample of what the community looks like. To actually send messages, open the <strong style={{ color: INK }}>chat bubble in the bottom-right</strong> — that's where real conversations happen (Shape members only).
                </div>
              </div>
            </div>
          </div>
