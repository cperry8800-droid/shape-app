...
// ─── The replay-rejection notice (Codex round, 2026-07-28) ──────────────────
// `bsReplayQueue` DROPS a rejected assignment from the queue, and the drain runs
// on reconnect / session resolve with no assign screen mounted. This notice is
// the only thing standing between that and a silent loss, so it has to mount —
// and it has to mount on the FIRST paint, since the drain routinely finishes
// before the shell does.

test('rejection notice: renders NOTHING when there is nothing to report', () => {
  globalThis.ShapeAssign = { rejections: () => [] };
  const { html, warnings } = render(React.createElement(MOD.BSProAssignRejects));
  assert.equal(warnings.length, 0, warnings.join('\n'));
  assert.equal(html, '', 'no rejections must not paint a banner');
});

test('rejection notice: a dropped assignment surfaces on the FIRST paint', () => {
  // Seeded lazily, not in an effect — effects do not run in this renderer, and
  // more importantly they do not run before the coach sees the first frame.
  globalThis.ShapeAssign = {
    rejections: () => [{
      owner: 'coach-A', at: 1, clientId: 'c-1',
      title: 'Upper Pull', scheduledDate: '2026-08-03', reason: 'guardrail red',
    }],
  };
  const { html, warnings } = render(React.createElement(MOD.BSProAssignRejects));
  assert.equal(warnings.length, 0, warnings.join('\n'));
  assert.match(html, /NOT DELIVERED/, 'the coach is told plainly it did not land');
  assert.match(html, /Upper Pull/, 'which assignment');
  assert.match(html, /2026-08-03/, 'and when it was for');
  assert.match(html, /guardrail red/, "carrying the server's own reason, not a generic error");
  assert.match(html, /<button/, 'and an explicit acknowledgement, not an auto-dismiss');
});

test('rejection notice: a store that throws cannot take the shell down', () => {
  // It reads localStorage through window.ShapeAssign; a throw there must not
  // crash the coach app on open.
  globalThis.ShapeAssign = { rejections: () => { throw new Error('localStorage blocked'); } };
  const { html, warnings } = render(React.createElement(MOD.BSProAssignRejects));
  assert.equal(warnings.length, 0, warnings.join('\n'));
  assert.equal(html, '');
});
