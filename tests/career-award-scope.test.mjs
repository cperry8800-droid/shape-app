...
// communityFeed.jsx), so this file gates BOTH the way
...
const WEB = readFileSync(new URL('../public/newdesign/communityFeed.jsx', import.meta.url), 'utf8');
...
test('both surfaces key each claim independently, never one shared blob', () => {
  for (const [label, src] of [['shapeBackend.js', MOBILE], ['communityFeed.jsx', WEB]]) {
...
test('a queued claim is written with a single setItem on its own key', () => {
  ...
  assert.match(WEB, /setItem\(careerKey\(uid, pid\)/,
    'communityFeed.jsx: the write is not a single own-key setItem');
});
...
test('clearing removes only the matching owner+post key', () => {
  ...
  assert.match(WEB, /removeItem\(careerKey\(uid, pid\)\)/,
    'communityFeed.jsx: clear is not scoped to one claim');
});
...
test('both surfaces refuse to replay another member\'s queued award', () => {
  ...
  assert.match(WEB, /careerPendingRead\(me\.id\)/, 'communityFeed.jsx: catch-up is not owner-keyed');
  // The web catch-up used to fire on mount with no signed-in check at all.
  assert.match(WEB, /shapeDb\.getUser\(\)/, 'communityFeed.jsx: catch-up does not resolve the current user');
});
...
test('the queue survives an unauthenticated answer and clears on a terminal one', () => {
  ...
  assert.match(WEB, /data\.reason === 'unauthenticated'/, 'communityFeed.jsx: terminal rule missing');
});
...
test('the catch-up replays every one of the queued posts we own', () => {
  ...
  assert.match(WEB, /for \(const rec of pending\)/, 'communityFeed.jsx: catch-up replays only one record');
});
...
test('the web composer takes the award owner from the post response', () => {
  assert.match(WEB, /j\.post && j\.post\.author_id/,
    'communityFeed.jsx does not read the owner from the post response');
...
test('the per-claim queue is capped, oldest first', () => {
  ...
  assert.match(WEB, /a\.at - b\.at/, 'communityFeed.jsx: cap does not evict oldest first');
