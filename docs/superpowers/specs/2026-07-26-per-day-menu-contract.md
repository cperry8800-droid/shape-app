...
`/api/nutritionist/meal-plan` stores `days` wholesale without reshaping it. Seven
days with seven different `meals` arrays is *already* a legal, storable, readable
value. Nothing in the table, the route, or the client read needs to change.
...
That narrows C1a considerably: **no migration, no route change, no change to how
a client reads their menu.** It is an authoring-shape change plus the readers of
that shape.

## 3 — Every reader of `coach_plans.detail.blocks`

Verified against the tree at `5bebd73b`. This list is the reason the naive fixes
fail; it is also the acceptance surface.
...
| 4 | `startBlockWeeks` | `iosAppBroadsheetClient.jsx` | Reads day/week LINES only, to decide whether to show the weeks stepper. Nutrition never reaches it (see below). |
...
then only *read* it — and it is registered as such below, but it is deliberately
**not** in C1a's scope: adding a nutrition start path is a new delivery surface,
and this contract is about the shape.
...
paid meal plan's preview renders nothing** the moment the reader ships, and every
new per-day plan previews as nothing. Silent, total, and on a commerce surface.

**Flatten with day prefixes** — `blocks = ["Mon — Breakfast · Oats · 500 kcal", …]`:

The preview's nutrition branch is reached (its `dayLines` split test is gated on
...
```jsonc
{
  "blocks": [ /* unchanged — the week's representative menu */ ],
  "note":   "…",
...
- **`dow` is 0 = MONDAY**, matching `BS_ASSIGN_DOW` and `bsRepeatSpec` — *not*
  the reminders table's `0 = Sunday`. The parent spec already fixed this class
  once (`2026-07-20-cycle-coach-today.sql`); it is restated here because this is
  a new writer of a dow.
...
- **`days` is advisory to storage, authoritative to the reader.** No migration:
  `detail` is already `jsonb`, and a legacy row simply has no `days` key.
...
and a coach who authors only three distinct days gets those three plus four
default days rather than four empty ones.

### 5.3 The ONE normalizer

Both readers call the same function. It lives in `planOutline.mjs` beside the
grammar it uses — the module that already exists to stop exactly this drift.
...
// can never describe a week the delivery doesn't build.
...
- Invalid/duplicate/out-of-range `dow` values are dropped, not clamped. Clamping
  would silently move a coach's Thursday menu to Sunday; dropping falls back to
  the default menu for that day, which is the honest degrade.
...
**day selector above the existing list** — the editor's block list, its step
authoring (PR E), and its publish path are otherwise untouched:

- A **DEFAULT** tab (edits `detail.blocks`) plus **MON…SUN** tabs (each edits
  `detail.days[dow].blocks`).
- A day tab with no authored blocks shows the DEFAULT menu greyed, labelled as
  inherited, with one action to start from it — making §5.2's inheritance
...
- **A purchased meal plan cannot be started by its buyer** (§3). Real gap, own
  ...
  `client_meal_plans`, plus the replay/entitlement question that belongs to
  [`2026-07-26-entitlement-layer.md`](2026-07-26-entitlement-layer.md).
- **C1b** (the ✦ AI DRAFT) stays blocked on its own contract. A per-day menu
  ...
  contract C1b lands must return a seven-day shape that satisfies §5.1.
