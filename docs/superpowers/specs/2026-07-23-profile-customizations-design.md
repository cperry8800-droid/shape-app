...
6. P5: deleting the underlying review removes it from the wall on next load; the wall never renders an id that doesn't resolve; and a hand-written id belonging to ANOTHER coach's review renders nothing (the owner-scoped resolve, tested with a cross-coach id vector).
