...
- **P5 · The Wins wall** — the coach pins ≤3 of their **real, existing `coach_reviews`** (verified: table exists, unique `(user_id, coach_slug)`, publicly rendered on coach profiles today). The doc stores review **ids only**; render fetches **the PROFILE OWNER's own reviews** through the existing public read path (the query is filtered by the owner's `coach_slug` — never an unscoped by-id lookup) and shows only pinned ids that resolve **within that owner-scoped set** — so a hand-written id belonging to ANOTHER coach's review never resolves and never renders (review round — IDOR/CWE-639). **Precise scope (post-merge Codex P2):** `coach_reviews` is keyed by free-text `coach_slug` (no immutable owner-id column), so "the owner's set" means **the profile's own slug-keyed review set — exactly the set the profile's Reviews section already renders publicly today**. The wall can never show a review that wouldn't already appear on that same profile's Reviews section; a slug collision/reuse commingles the whole Reviews surface identically and is a **pre-existing platform property of `coach_reviews`, registered as a follow-on** (re-key reviews to the immutable provider `owner_id`, migration + backfill), not a scope this wave can honestly claim to close. Quotes verbatim, `— REAL REVIEW · PINNED` attribution. **A deleted review disappears from the wall automatically; the wall can never show text that isn't a live review row.** Never fabricated stats, never edited quotes.
...
6. P5: deleting the underlying review removes it from the wall on next load; the wall never renders an id that doesn't resolve; and a hand-written id from outside the profile's own slug-keyed review set renders nothing (the owner-scoped resolve, tested with a cross-coach id vector — scope precisely the profile's Reviews-section set; the slug-keying residual is documented in P5).
