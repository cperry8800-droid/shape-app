...
**Nothing set → the coach's box carries no coach media** — every slot degrades to today's content (avatar-or-initials portrait, no cover, no strip). **Two deliberate geometry exceptions, both owner-ruled:** the featured section's full-width combo layout (E, §Mobile renders #3) and the want-ad rows' thumb column (D, §Mobile renders #5) change for everyone — within them, a media-less coach's cell/row carries today's **content** (initials thumb, today's cell anatomy) in the new geometry. Every other surface is byte-identical when nothing is set.
...
- `bsSafeMediaUrl(v)` — the base URL primitive: **requires `typeof v === 'string'` FIRST, then parses inside try/catch** (review round 2: `new URL()` throws on numbers/Symbols/objects/malformed strings — the type check + catch is what makes "never throws" true, not a promise). Requires protocol `http:`/`https:`, a non-empty hostname, length ≤ 500; anything else → null. A prefix regex is explicitly NOT the mechanism (`^https?://` accepts host-less values).
- `bsOwnMediaUrl(v, bucket, ownerUid)` — **the gate listing media actually renders through** (review rounds: SSRF/CWE-918 + IDOR/CWE-639): `bsSafeMediaUrl` PLUS **https-only · the app's OWN Supabase storage origin** (derived at runtime from the app's supabase config — never hard-coded) **· no userinfo credentials · no explicit port · path prefix `/storage/v1/object/public/<bucket>/<ownerUid>/`** — the owner-folder binding means a coach can't hotlink ANOTHER coach's objects from the same public bucket. Listing media uses `bucket='coach-media'` with the provider row's `owner_id` as `ownerUid` (renders) / the caller's own uid (the setter); a row with no `owner_id` can't validate and its media drops. The profile wave imports the same function with its per-key buckets + the profile owner's uid. The in-code DEMO constants (Unsplash covers) never pass through the normalizer — they aren't DB rows — so the allowlist costs real coaches nothing (their editors upload only into their own folder) and the demo cast keeps working.
- `bsNormalizeListingMedia(raw, ownerUid)` → `{ portrait, cover, gallery, updatedAt }` — the gate every consumer runs raw row data through. **`ownerUid` is REQUIRED context, not optional** (review round — CWE-639): without an explicit owner in the contract, a consumer could normalize raw media with no owner-folder binding and reintroduce cross-coach object references. A missing or invalid `ownerUid` (anything but a non-empty string) **fails closed to the empty result** — the function never attempts validation without owner context. Renders pass the provider row's `owner_id`; the setter passes the caller's own uid.
  - `portrait`/`cover` pass `bsOwnMediaUrl(v, 'coach-media', ownerUid)` or drop; `updatedAt` passes through only as a valid ISO string.
  - **Image-only at the normalizer, not just the editors (post-merge Codex P2):** the coach-media bucket deliberately allows video (plan clips), and a coach can write `listing_media` directly via the exposed Supabase client — so every listing URL must ALSO end in an **image extension** (`.jpg|.jpeg|.png|.webp|.heic|.heif|.gif`, case-insensitive, checked on the parsed pathname) or it drops. The editors' opaque key scheme always produces these extensions, so real uploads cost nothing; a hand-written own-folder VIDEO URL can no longer ride into an image slot. Renderers keep `<img>`/`background-image` semantics and treat a load failure as absent (never a broken-media placeholder).
  - `gallery` rebuilt **field-by-field** (the coach-channel per-item rebuild discipline — extra keys can't ride through), clamped to 6; captions coerced to plain strings, control chars stripped, clamped to 80; items whose url fails `bsOwnMediaUrl` **or the image-extension check** drop.
  - Junk shapes (non-object, arrays, Symbols, numbers) **or a missing/invalid `ownerUid`** → `{ portrait:null, cover:null, gallery:[], updatedAt:null }` — never throws (**no `Number()` on anything**).
...
- **Images only** — file-picker `accept="image/*"` AND a pre-upload mime check (`image/jpeg|png|webp|heic|heif|gif` — matching the coach-media bucket's own mime list, which explicitly allows `image/heif`; iPhone photos arrive as heic/heif); a video or unknown type is rejected with an honest editor message before any bytes move.
...
- Mobile: `window.ShapeCoachMedia.upload(file, { prefix })` — **the helper gains an optional purpose prefix** (review round 2: today it produces `<uid>/<ts>-<rand>.<ext>`, which is the plan-clip path, NOT the documented listing path). Default (no prefix) preserves the existing plan-clip behavior byte-for-byte; the listing sheet passes `prefix: 'listing'` → `<uid>/listing/<epoch>-<random>.<ext>`. Web: direct browser upload via `window.shapeDb.client.storage` under the same storage RLS (the `dashboardCommunity.jsx` precedent), same checks, same listing key scheme.
...
2. **The living Signal profile** (web coach profile): a **THE STUDIO** station in the coach blocks — fetched from the provider row selected by an **explicit provider role** (role → `trainers`/`nutritionists` table, THEN by owner uid — review round: owner uid alone is ambiguous for a dual-role coach who has BOTH rows, and THE STUDIO must never display the other listing's gallery; the same reason the data layer's `mine(role)` is role-explicit). **The role comes from the CLICKED LISTING, carried in the URL (post-merge Codex P2):** live marketplace cards today link only `MemberProfile.html?u=<ownerId>` (`marketplace.jsx:339`) and the profile then guesses from `d.role` — which can be the coach's OTHER role. The cards therefore append the listing's role (`&role=trainer|nutritionist`, the same param the demo-coach links already carry), and the profile's provider-row fetches (THE STUDIO **and** the `livingShared.jsx:589` monthly_offer resolution it rides) prefer the URL's role param, falling back to `d.role` only when no param is present (direct/legacy links). Rendered as a captioned strip in the ledger grammar. Absent → nothing. (The profile keeps its own `profile_custom` cover/portrait — this adds only the gallery.)
...
5. **The portrait ladder, per rung:** a hand-crafted `"portrait": "javascript:alert(1)"` — AND a well-formed third-party `"https://evil.example/pixel.png"`, AND `"http://127.0.0.1:9000/x.png"`, AND another coach's own-bucket URL (`…/coach-media/<other-uid>/…`) — are all dropped by the normalizer (`bsOwnMediaUrl`) and **never rendered on either surface**; resolution then falls to `c.photo/avatar` if present, else `avatarByUser`, and **initials only when no valid avatar exists at any rung** — tests cover each level plus non-string junk (numbers/Symbols/objects → null, no throw). Same origin+scheme rule for cover + gallery URLs.
