...
- `bsOwnMediaUrl(v, bucket)` — **the gate listing media actually renders through** (review round: SSRF/CWE-918 — https to arbitrary origins still lets a hand-crafted row point viewers' clients at attacker endpoints or `http://127.0.0.1:…`): `bsSafeMediaUrl` PLUS **https-only · the app's OWN Supabase storage origin** (derived at runtime from the app's supabase config — never hard-coded) **· no userinfo credentials · no explicit port · path prefix `/storage/v1/object/public/<bucket>/`**. Listing media uses `bucket='coach-media'`; the profile wave imports the same function with its per-key buckets. The in-code DEMO constants (Unsplash covers) never pass through the normalizer — they aren't DB rows — so the allowlist costs real coaches nothing (their editors only ever produce coach-media URLs) and the demo cast keeps working.
- `bsNormalizeListingMedia(raw)` → `{ portrait, cover, gallery, updatedAt }` — the gate every consumer runs raw row data through:
  - `portrait`/`cover` pass `bsOwnMediaUrl(v, 'coach-media')` or drop; `updatedAt` passes through only as a valid ISO string.
