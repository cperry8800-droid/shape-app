...
Note: the third test needs Task 5's page — within this task, comment out the `crashPageMod` lines and the third test, and re-enable them in Task 5 (its Step 1).
...
- [ ] **Step 1: Re-enable the commented `crashPageMod` loader lines and the third test** in `tests/web-error-boundary.test.mjs`. Run — expect FAIL (page missing): `node --test tests/web-error-boundary.test.mjs`
