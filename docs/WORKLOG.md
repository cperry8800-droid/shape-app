...
### 2026-09-16 — The website Radio page becomes the instrument, the second player retires, and twelve review rounds on one PR

- **The owner's pick off [`REVIEW-2026-09-15-radio-page.md`](REVIEW-2026-09-15-radio-page.md), built** — the wall and the cloud composed, hand-over on the drop — plus **N1** for the nav. #2101 → `c156576`. The web had **two pages for one station** and the app has one screen; `/radio.html` is a 47-line redirect now, Nora's booth having been ported first so nothing shipped was lost. `public/newdesign/` and the two moved modules; **no migration, no new route** (one existing route gained a third answer and one gained a field).
- ⚠ **WHAT CAME OUT IS THE POINT AS MUCH AS WHAT WENT IN.** Forty-eight bar heights rolled from `Math.random()` **at render** · a scrubber reading `28:14 / 48:30` over a stream that has no position · `◉ Live` over a station that is not broadcasting · five sections defined and mounted by nothing, carrying typed BPMs and *"1,284 listening"* · a typed `currentBpm = 120` · and a **Demo HR slider that fabricated a heart rate under a badge derived from it**. The app deleted its own `demoHr` and `BSBeatRing` on 09-14 for the same reason; this is the website catching up.
- **The modules are shared rather than copied.** `radioSignalField.mjs` and `radioTempo.mjs` move to `public/newdesign/` and the app imports them across the tree — the `noraSets` precedent, nine shared modules deep. The website cannot fetch a file outside `public/`, so the alternative was a build-time copy, i.e. **two files that drift**. `radioField.mjs` is new and holds the field's **rules** and never its drawing, so both load-bearing ones are driven in Node: the figure advances on a **phrase counted off the detector's own beat** (no measured tempo, no phrase), and the lead **delegates** to `tempoEnergyFromBins` rather than looping the same bins itself, so a widened kick band reaches the lighting with nothing to keep in step.
- **TWELVE CODEX ROUNDS ON ONE PR. 33 FINDINGS, EVERY ONE REAL, EVERY ONE FIXED**, every thread replied to and resolved. That is the most rounds this log records, and the shape of it is worth more than the count.
- ⚠ **THE SAME FALSE CLAIM HAD THREE DOORS AND I SHUT THEM ONE AT A TIME.** *"No station on the air yet"* — a **positive** claim that the station is not broadcasting — was reachable from a `fetch` that resolves on an HTTP error (round 2), from a non-ok branch that recorded the right verdict and did not stop (round 3), and finally from **the server**, where `/api/radio/station` dropped its own query error and answered **200 with `configured:false`**: not *"there is no station"* but *"we could not look"*, published with the authority of a successful response, where **no client guard can see it** (round 4). I had audited every writer **in the client**, found none, and said so in the round-4 trigger. *A boundary you have declared clean is where the next one lives.*
- ⚠ **AND THE PAGE BUILT TO RETIRE A TYPED-IN BPM WAS PRINTING AN INVENTED SONG TITLE.** `/api/radio/now-playing` returns the mock provider's fixed *Tempo Lift / Shape Radio* — and production's row is `provider='mock'`, so that is what it returns **on every call today**. The route marks a simulated payload now (derived from the selector **by identity**, `getProvider(config) === mockProvider`, rather than restating the http condition) and the page refuses it. ⚠ Its honest empty needed **reading rather than eyeballing**: the fallback is the product's own name, which is **the same string as the mock's artist**, so a leak would have looked identical to a pass.
- ⚠ **THE LAST THREE ROUNDS WERE ALL THE SAME SHAPE — A MEMBER LOCKED OUT OF THEIR OWN PLAYBACK — AND EACH FIX OPENED THE NEXT.** Round 10: `/api/me` calls `createClient()` with **no request**, so it reads the Next.js cookie and a Bearer is not even an option, while this site **also** keeps sessions in localStorage under `shape.auth`; `applyNavAuthState()` copies one into the cookie **fire-and-forget**, so nothing orders it against the page's read. A member signed in on a legacy or static page was measured **signed out — permanently**, since the effect has empty deps: the key read *"Sign in to listen"* and they were handed the visitor preview for the life of the page. Round 11: the fix captured the bearer **at mount**, and `currentUser()` gives **any** Bearer **precedence over the cookie** (`src/lib/request-auth.ts:31` short-circuits) — so an hour-old page did not merely send a dead token, it **suppressed a working cookie with it**, and every retry re-sent the same dead token. Round 12: the mount probe's `/api/me` leg can still be **in flight** when the member presses, so a session bridged in between let the press start real audio and then let the **old** probe land `{user:null}` over a member who was listening — **starting the visitor preview simulation on top of a live stream**.
- ⚠ **I HAD FLAGGED ROUND 11's DEFECT MYSELF AND STOOD IT DOWN WITH A WRONG REASON.** The round-11 trigger names token staleness and dismisses it: *"the 401 path is retryable and says so"*. It is not retryable — the retry is **identical**, because the stale bearer is sent again and keeps outranking the cookie. *A hazard you have named and excused is not a hazard you have handled*, and the excuse was one measurement away from being checked.
- **Each fix makes the failure unrepresentable rather than guarded**, which is why the chain stopped: the token is resolved **at the press** and the ref **deleted outright**, so there is nothing left to go stale; a failed resolve sends **no header at all**, which is the right asymmetry, since no bearer lets the cookie have its turn where a dead one takes it away. And it costs no gesture — the resolve sits inside the `!audio.src` branch, already behind the station fetch's own `await`.
- ⚠ **TWO RECURRING CLASSES WERE SWEPT RATHER THAN FIXED A SEVENTH TIME.** *"The node that contains what I am looking for is not the node I meant"* produced **five** findings across two rounds and now goes through one `innermost()` reducer — reverting it to `candidates[0]` fails four tests, which says how much had been resting on my remembering. *"A branch that says the right thing has not necessarily stopped"* produced **six** across rounds 3, 4, 6, 8 and 9, every fix a fresh copy of the same four lines, and now goes through one `ownScopeExits()` helper. ⚠ **Writing the round-10 fix produced a SEVENTH instance inside its own fix** — without a `return` the SDK branch records `signedIn = true` and the `/api/me` fallback below it overwrites a real member back to `false` — and **the helper caught it**, which is the whole argument for sweeping a class instead of patching its instances.
- ⚠ **AND FOUR OF MY OWN GUARDS PINNED A SPELLING RATHER THAN AN INVARIANT, THE LAST ONE PINNING THE DEFECT ITSELF.** Round 10's bearer guard asserted the ref's **write** tied to the ref's **read** — so the correct fix would have failed a test about the bearer being sent. It is an **absence plus a location** now: no ref may hold an `access_token` anywhere in the file, and the station function must resolve a session **inside its own scope**. ⚠ Round 1's guard had to be **narrowed, not weakened**: it asserted the station fetch never touches `signedIn` **at all**, which is broader than that finding was. What makes a 401 and a 403 unusable is that neither measures the **session** — the 401 is ambiguous (`currentUser()` drops the error, so an auth-server fault answers 401 exactly as a visitor does) and the 403 is a **confirmed minor, who is signed IN**. A session the press **resolves** is positive proof. The invariant is **direction, not contact**, and round 1's defect is replayed in **two** forms, a literal `false` and a `null`, both killed. *Forbidding contact outright is exactly what let the stale write land.*
- ⚠ **ONE MUTATION SURVIVED AND WAS A REAL GAP, NOT A NO-OP.** Marking the session **settled without resolving it** blocks the probe's write and then supplies none of its own, so `signedIn` stays unresolved for the life of the page — the marker suppressing the only reading that was going to arrive. The guard now requires settling and publishing **in the same breath**, at every site rather than the press's alone, because it is the same mistake wherever the marker is set.
- ⚠ **AND THE ONE REVIEWER-PROOF TECHNIQUE OF THE WHOLE PR WAS COMPARING THE RETIRED PLAYER AGAINST THE NEW PAGE.** Round 10's second finding is `navigator.mediaSession.metadata`, which the retired player set from every now-playing response (`radio.html:409`) and the rewrite silently dropped — **shipped behaviour retired by a rewrite rather than by a decision**. Restored as an effect on `nowPlaying`, so it **clears** on a simulated or unreadable poll: a stale title on a lock screen is the same false claim as one on the page, and worse, because the page's own correction is not on screen beside it. ⚠ **No invented fallback** — the retired player defaulted the title to the product's own name, which is also the mock's artist string. ⚠ **Metadata only, no action handlers, deliberately**: that file's own `play` handler is recorded in this log as the one of its six `play()` call sites carrying **neither** a station guard **nor** a `.catch`.
- **Nine more defects were found by DRIVING the page rather than reading it**, before the first review round: the frame loop read the first render's values, so the preview never ran and the rail said *"Off air"* to a visitor the page had already measured as signed out · the draw read a buffer the loop had stopped writing · every wall column indexed raw bins 0–63, the bottom quarter of the spectrum, so the meters filled 70% of the fold on ordinary material · `bandsFromBins` **already** returns a mirrored array, so the column map mirrored a **second** time and a kick showed as two humps with a trough where the bass belongs · and reduced motion threw away the **reading** as well as the drawing, so at 4 fps the detector could not resolve a beat and the tempo read `—` forever.
- **Verified on the MERGED tree** (`c156576`), re-derived rather than carried: `npm test` **3903/3903** · `tsc --noEmit` **0** · JSX parse · the newdesign precompile check **72 pages, 78 shared jsx** · the full pre-commit gate including the mobile build · and **all four required checks green** on `b96a324`, not a draft.
- ⚠ **AND THE DELTA IS ATTRIBUTED BY MEASUREMENT RATHER THAN BY ASSUMPTION, WHICH CORRECTED MY OWN FIRST READING OF IT.** The branch read **3886** and the merged tree reads **3903**, and I was about to write that the **+17** belonged to the PRs that landed underneath. It does not: the merge parent `03426ae` was measured at **3855**, so **this PR contributed 48 cases** and the branch figure was taken against an **older base** — never the same tree, and therefore never comparable. *A delta between two trees is a claim about a third one until you measure the parent.*
- **Mutation rounds throughout**, each mutation **proven to land** (anchors occurrence-counted before the edit, mutated text asserted present after, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and** on a signal, every file read and written as **bytes**. Every finding of the last three rounds is replayed as its own mutation, so the suite is proven to **catch** them rather than merely to be green after the fix.
- ⚠ **REGISTERED, NOT FIXED — AND THE FIRST ONE IS THE STANDING CAVEAT ON EVERYTHING ABOVE.** **No on-account pass**: production's `radio_station` row carries a **null `stream_url`**, so nothing on this page has played a real broadcast; every signed-in reading here is a scripted route and a synthetic stream, and the honest check is a member on a station that is actually on the air. The **mobile consumer still ignores `simulated`** and shows `Tempo Lift` on a mock station — a different surface, its own honest-data pass. The **402 join-prompt copy** is an open owner call: *"press again"* is advice that cannot help a non-member. **Web Bluetooth and the matching state** are out by the review's own ruling, which is why `kx` is pinned at 0. And **N1's Doto chip** and **L1's "SHAPE RADIO" label** each reverse something already settled and are the owner's to call.
- ⚠ **A RESIDUAL NAMED RATHER THAN CLOSED:** if `NoraStage`'s constructor throws **after** `new THREE.WebGLRenderer` (`noraStage.mjs:29`), no caller can reach that renderer — `new` yields nothing — so it leaks. Judged unreachable in practice (the remaining statements are pure THREE construction) and fixing it is a `noraStage.mjs` change the **mobile app shares**, so it is stated at the site rather than smuggled into a website PR.
