...
### 2026-09-21 — Cooking progress becomes a readout with a roadmap, and the guards that pinned where a number sat

- **Merged [#2126](https://github.com/cperry8800-droid/shape-app/pull/2126)** as `fd5ad488c`, final head `e9f0617cc`. Opened on 2026-09-20 by another session and completed on the owner's word (*"can you complete them?"*). Cook mode's progress was a small header number and a step strip; it is a prominent percentage, an accessible progress bar, a completed-step count and an expandable roadmap now — one `BSCookProgress` on the solo player, the interleaved board and the session wrap. The roadmap tells completed, skipped, current, upcoming and timer-running steps apart, shows a duration only where one was authored, and planned times on the board; opening it never moves the cursor or a timer, and the duration-weighted figure stays under 100 while work or a hold remains. Thirteen `cook:roadmap.*` keys × 13 locales, one new pure module (`cookRoadmap.mjs`, 46 lines). **No migration, no route.**
- ⚠ **THE PR SAT RED FOR A DAY ON THREE GUARDS THAT PINNED A PLACEMENT, NOT AN INVARIANT.** `tests/kitchen-allergen-surfaces.test.mjs` asserted `Step … · N%` beside the step line and `N% done` in the header; the PR moved the solo figure into the readout — for a solo cook the dish figure and the overall figure are ONE number, and the old header printed it twice a few lines apart — so the correct change failed a test about something else. Reproduced locally on the head first (**4,145 tests, the same 3 failing**), then re-anchored: the figure is read from the readout, the step line must survive, the dish figure beside the step line is required only across several dishes (where the two figures differ), and the solo case is pinned to one figure. *A guard that pins a spelling pins whatever that spelling is wrong about* — the sentence this file repeats most, paid again by a PR that had been waiting on it.
- **Review:** Codex's automatic round on `9922502` completed with no findings; on the final head it refused both the front-loaded trigger and the one permitted retry within seconds (*"You have reached your Codex usage limits for code reviews"*), so no external reviewer read `e9f0617`. **CodeRabbit was not run.** My own adversarial read of the diff found nothing to change — the two roadmap builders are read-only projections, the carried debit and the saved-progress credit agree on which steps were credited, `onPrepped` has one call site, and the i18n inventory columns are unmoved (219 / 738), so the new readout is fully keyed. The merge gate was the four required checks green on `e9f0617` and not a draft.
- **Verified:** CI on the final head **4,225/4,225** (6 new) plus web typecheck/build, mobile build, secret scan and the debug APK · locally through the pre-commit gate **4,225/4,225**, mobile build clean, JSX parse clean · **8/8 mutations killed** over the re-anchored guards and the PR's own tests (the solo duplicate restored · the readout showing the dish figure across several dishes · the readout losing its percentage · the percentage counting steps instead of minutes · a sequential handoff forgetting the dish outcomes · the unfinished cap removed · a running hold no longer holding · board holds matched by timeline slot instead of original step), each proven to land, the suite's own `# pass`/`# fail` parsed, sanity green at both ends, the tree restored in a `finally` and on a signal.
- **Owner checks, and what is registered rather than fixed:** no on-device pass — every reading is the mount harness and the PR's own browser fixtures at 390px and desktop; the disclosure's `aria-controls` names a panel that is not in the DOM until it opens (a minor a11y nit, left); and, as the PR states, sequential session recovery, native background alarms and meal-log retry are not added.

### 2026-09-21 — Nora's voice: dictation in the member's language, spoken replies, and lookups for coaches, the account and how Shape works

- **Merged [#2130](https://github.com/cperry8800-droid/shape-app/pull/2130)** as `dfb8219d5`, final implementation head `9f6b7a361` (the squash tree is byte-identical to it). Owner: *"make sure nora voice chat capabilities are the best they can be. I want you to be able to talk to nora to look up coaches, ask questions about account, how shape works, etc."* `/api/support/chat` reads `voice`, `surface` and `locale` off the body — each reduced to a known value, none used for access — and a spoken turn gets a for-the-ear prompt note (one to three short sentences, the answer first, no lists, URLs or symbols that do not read aloud), a non-English app locale a LANGUAGE note. Three new abilities: **`shape_help`**, a fifteen-entry sourced knowledge base (`src/lib/ai/shapeKnowledge.mjs`: membership, coaches optional, coach prices and fees, cancelling, Radio, credentials, switching coaches, Score tiers, habits, units, Cook Mode, Nora, health data, account) offered to every caller and backing the no-key fallback on a whole-word tag or title hit; **`get_account`**, a member-only read of the profile, the platform plan (on record or not, status, price, period end), coach subscriptions by name, the Settings preferences, Nora's voice, the app language and timezone, where every leg past the profile says when it could not be read; and **`recommend_coaches` on the live marketplace** — the `trainers` / `nutritionists` rows both surfaces list, through the caller's own client (the anonymous cookie client when signed out), mapped without the marketplace's cosmetic defaults (a missing rating is null, not 4.8), the example directory merged after the live rows on the web and never on the app, a live chip carrying `providerId` so the app opens the Listing.
- **Transcription moves to `gpt-4o-transcribe`** (`OPENAI_TRANSCRIBE_MODEL` overrides) with a one-shot fallback to `whisper-1` on a refused model; a `language` hint rides only as a valid ISO-639-1 code, a vocabulary prompt primes Shape's own words, and `keywords[]` goes only to the `gpt-transcribe` models whose API takes it. Both transcription routes read `language` and `context` (`nora` · `meal` · `grocery`) off the form and report the model that answered; `/api/health` reports the default. Clients: the composer's dictation uses the locale's Web Speech tag (22 locales; Pidgin dictates as `en-NG` and transcribes on auto-detect) instead of `en-US` for everyone; the composer, the meal note and the grocery list all transcribe through the backend client with the app's locale and the matching context — ⚠ **the root-relative fetch each of them made resolves against the WebView's own origin on the native build, so none of the three voice paths could have reached the backend there**; a released hold-to-talk transcript is sent as spoken; the website widget dictates in the page's language and its hand copy of the dictation table is held to the shipped one by a test. No migration, no new route, no i18n key.
- **Review:** Codex refused the head **twice** — on PR open and eleven seconds after the one manual trigger, both *"You have reached your Codex usage limits for code reviews"* — so the one permitted retry was spent and no external reviewer read `9f6b7a3`. **CodeRabbit was not run**: it bills $0.25 a file beyond the included allowance, this diff is 21 files, and the one round on #2128 ran on the owner's word; a round is one word away. The review is my own adversarial read of the full diff plus the mutation round, and the PR says so in as many words. The merge gate was the four required checks green on the final head and not a draft.
- **Verified:** CI on the final head **4,219/4,219** (39 new across eight suites) plus web typecheck/build, mobile build, secret scan and the debug APK · `tsc --noEmit` 0 · newdesign precompile 73 pages, 0 errors · **37/37 mutations killed** (each proven to land, the suite's own `# pass`/`# fail` parsed, sanity green at both ends, the tree restored in a `finally` and on a signal): the voice note dropped, a truthy voice claim accepted, the raw locale reaching the prompt, `shape_help` withdrawn, the app handed examples, a failed live read not unavailable, the provider id dropped from the chip, the fallback answering on a stray body word, a fabricated account, English getting a LANGUAGE line, a nutritionist read on the session price, a defaulted rating, an example standing level with a live coach, standing alone making a hit, a live coach linking to the static page, every plan active, a malformed date thrown into the loop, canceled coach subscriptions listed, an unreadable plan read as none, substring tags, `minScore` ignored, keywords sent to every model, any language string riding, no transcription fallback, every error falling back, the raw locale from `speechLangFor`, an unknown context passing through, Pidgin told it is English, the language hint dropped in the route, the meal route priming Nora's vocabulary, a truthy voice claim sent, the surface unnamed, the context missing from the form, dictation back to `en-US`, the chip not opening the Listing, hold-to-talk sending as typed, the widget table drifting · the live catalog checked for every column the coach read names and for the RLS policies the account read relies on. Production deployment `dpl_6CErfZHgeNZEnTJZf6fFKwepLYtv` is READY for `dfb8219`.
- **Owner checks, and what is registered rather than fixed:** no on-account voice pass — no real device or session has exercised the mic path here, so the honest check is a member holding the mic and asking for a trainer, about their plan, and what Shape Radio is; if the account is not enabled for `gpt-4o-transcribe`, every transcription falls back to `whisper-1` and the route's `model` field says so; the website FAQ's *"we verify licenses on application"* still contradicts Terms clause 04, and Nora follows the Terms; sixteen other root-relative `/api/` fetches remain in the client module (score, subscription, checkout, billing portal, recipe reviews, meal-note, account actions, Spotify, analytics, export, delete) — the same shape the three voice paths had, not audited for native reachability here; realtime speech-to-speech (a WebRTC session in place of record → transcribe → reply → TTS) is a separate build.

### 2026-09-21 — Nora looks things up for members and coaches, and the AI layer moves to GPT-6 Astra

- **Merged [#2128](https://github.com/cperry8800-droid/shape-app/pull/2128)** as `c81fa74c1`, final implementation head `bb02cfa3c`. A verified member's Nora carries seven read tools on the caller's own RLS client (today's and this week's plan with meals, recent workouts with each top set in the unit the log stated, the last 7 days in numbers, habits, coaching, reminders, points); a coach additionally gets `find_client` over their own active roster and `get_client_snapshot` through the gated definer RPCs. A failed read answers unavailable, never empty; replies are flattened to plain text because both chat bubbles render them raw; the facts block finally carries today's habit completion.
- **The AI layer defaults to `gpt-6-astra`** (`OPENAI_MODEL` still overrides): reasoning-model request shaping (sampling params stripped, effort clamped to `low` on Astra), `store:false` with encrypted reasoning echoed through the tool loop, `prompt_cache_key` per route, a one-shot fallback to `gpt-5.4-mini` on a refused model, and model tiering on the open support route — an unauthenticated caller rides the public model, a verified member the pin, and a round that fell back names the fallback for the rest of the request. No migration, no mobile change, no i18n key.
- **Review:** Codex refused both triggers (usage limit); on the owner's word one CodeRabbit round ran on `3af41c4` and returned two findings, both real and fixed in `bb02cfa` — coach access now follows the shared membership verdict (a dual-role account whose primary role is `client` was denied the coach lookups), and a failed past-sessions read reports `recentUnavailable` instead of an absent list. Both replayed as mutations and killed, both threads resolved; the Codex retry was cancelled so the two reviewers never ran together. The merge gate was the four required checks green on the final head.
- **Verified:** CI on the final head **4,180/4,180** tests plus web typecheck/build, mobile build and secret scan; 31 mutations killed before the review round (each anchor occurrence-counted, proven to land as bytes, the tree restored in a finally and on a signal); every column and RPC the reads use checked against the live Supabase catalog, which found `load_unit` read without being selected — the fake now projects its select list so that class fails in tests. A coach owning two listings of one role is a measured production shape, so the roster read is a list read.
- **Owner checks:** if Vercel pins `OPENAI_MODEL`, the new default takes effect only after it is unset or changed and the project redeployed (the env could not be read here); if the account is not enabled for Astra every call logs one `model_fallback` line and answers on `gpt-5.4-mini`; the `store:false` tool loop is designed from the API contract and not exercised live (`OPENAI_STORE_RESPONSES=1` reverts). No on-account pass — the honest check is a signed-in member asking what is on today, and a coach asking about a client by name. **Next:** Nora's voice chat (locale-aware dictation, spoken replies, account and how-Shape-works lookups, live coach lookup) is in progress on the same branch. ⚠ **SHIPPED THE SAME DAY as [#2130](https://github.com/cperry8800-droid/shape-app/pull/2130) — the entry directly above; the branch is resynced to `main` and nothing is in progress on it.**
...
- **Solo cooking now autosaves steps, completed/skipped state, checklists, elapsed start and timer deadlines.** Resume is account-bound, same-day and fingerprinted to the unchanged recipe. Browser verification forced a reload mid-cook and recovered step 5 plus the correct remaining timer time. Expired timers remain available for acknowledgement. Recovery failures are visible and exit copy states that alerts need the app open. Multi-dish prep recovery and native background timer notifications remain follow-ups.
- **The owner's Radio screenshot was reproducible at Small text size:** the canvas was 329.4px inside a 366px wrapper because viewport dimensions were scaled twice by CSS zoom. Canvas sizing now uses local layout dimensions, fills the wrapper, and converts measured figure coordinates into drawing space. Small and Large settings both rendered a centered 366px visualizer in the 366px surface after the fix; geometry tests cover all three scales across several widths.
- **4,083/4,083 CI tests passed on the final head**, plus website typecheck/build/precompile, mobile build/assets, Android build and secret scanning. The 272 targeted tests and local mobile build also passed. Codex reviewed `d61168b`, found an obsolete saved draft could reappear after a later website edit, and the fix plus mounted regression landed in `ed4128acd`; the thread is resolved. No second external review was requested. New messages cover all 13 locales.
- **Production verified:** Vercel deployment `4gDHKZuL5BMz1R9iuwXiBtFZxfuq` succeeded for `fde5fea2d`, and the reloaded production `/m` app's Settings displayed `0.1.0 · FDE5FEA`. No migrations are needed. Native source/Android build are updated, but no store release was performed. Live signed-in note saves, actual media upload and native-device background behavior still need device/account checks. See [HANDOFF-2026-09-19.md](HANDOFF-2026-09-19.md).

### 2026-09-18 — Coach workout parity, simpler client sessions, and durable assignment history

- **Merged [#2119](https://github.com/cperry8800-droid/shape-app/pull/2119).** Website and mobile coach libraries now use the same structured workout document, draft/publish behavior, conflict protection, exercise videos, single-day/multi-week editing, and selective future updates. Client Quick Log pre-fills prescriptions, advances sets/exercises and supersets, starts rest, and retains account-bound drafts with atomic retry-safe saves. The coach roster, progress states, private notes, and client drawer were improved.
- **Final implementation head `6790bef72`: 4,071/4,071 CI tests; website, mobile, Android, and secret-scan checks passed.** Codex reviewed `3a1e242` and found three issues, all fixed before merge: Adjust must archive retired assignments; coach drafts must join every sign-out scrub; future updates must preserve program notes. All three review threads are resolved. No second external review was triggered. PostgreSQL verification exercised both migrations twice, session retries/rollback/auth, assignment preservation, stale-content rejection, and Adjust retirement/save/retry/audit behavior.
- **The owner confirmed both 09-18 workout migrations were applied before merge.** Production verification is still pending: the website release marker remained `a0615fd22` after merge, and Vercel required sign-in. Do not equate the merge with a verified live deployment. Native source is updated; an App Store/Play release was not performed. Signed-in coach flows and an actual media upload still need an authenticated live check.
- **i18n:** all 13 locales have the new controls. The partial-surface inventory changed from 220 to 219 because the client Open session action now uses its translated key. Local browser checks covered coach multi-add, drawer focus return, direct Begin session, one-tap logging, automatic rest, and phone layout. See [HANDOFF-2026-09-18.md](HANDOFF-2026-09-18.md).

### 2026-09-17 — Two owner picks turned into build briefs: the App page as One Day, and the app's Radio spectrum in the website's two tones

- **Records plus one design-independent swap — the builds themselves are handed to a build session.** Owner: *"lets go with one day"* (the App page pick, off the 09-17 review) · *"can you make the look of shape radio on app match the colors on website, regarding this part. i like the 2 different color schemes"* · *"make sure when you adjust colors on the settings app in still applies to the app on shape radio, both color sections"* · *"let me know when i can switch to opus for build once you have everything together"*. So this session assembled rather than built: two code-level briefs with every `path:line` machine-checked against `main` = `f52db62`, the review marked with the pick, and the two stale captures replaced. **No PR.**
- **[`BUILD-2026-09-17-app-page-one-day.md`](BUILD-2026-09-17-app-page-one-day.md)** — Concept B of [`REVIEW-2026-09-17-app-page.md`](REVIEW-2026-09-17-app-page.md) (the review recommended C; the pick supersedes it and the review's §5 says so). What is deleted from `GetApp.html` with line references; the chrome the page never had (the shared `<Header active="App" />` and `<Footer />` mounted into two roots, the font link byte-identical to `index.html:44`); the eight-beat SCREENS table with the figure read off each capture and the file each beat shows; the doors with the waitlist `platform` fix (`route.ts:13` defaults a missing platform to iOS, so every Play sign-up has been filed as App Store); the guards to write and the three that change (`tests/site-footer.test.mjs` reads GetApp's own footer table today); and **Appendix A, the board's CSS and markup generated from the same generator that rendered the approved preview, with the image paths already pointed at the site's captures** — not retyped. ⚠ **The three rulings the pick did not settle are DEFAULTED so the build can start, each one line to reverse**: the `/m/` door yes, the two unbacked copy clauses dropped, the member app only.
- **[`BUILD-2026-09-17-radio-two-tone.md`](BUILD-2026-09-17-radio-two-tone.md)** — the website's rule read from `radioInstrument.jsx`: a lit column is teal, **its top fifth is hot amber** (`fromBottom > level * 0.8`, `:644`), the kick flood and the cloud's loud dots go hot too. The app draws one tone (the accent) with a cream tip, which is why the owner's screenshot was violet. ⚠ **THE HOT TONE IS DERIVED FROM THE ACCENT, NOT PICKED, AND ON TEAL IT REPRODUCES THE WEBSITE EXACTLY.** `hotFor(accent)` rotates the hue by the offset that turns the website's teal into its amber (−138.5°), takes amber's saturation and the same lightness step — `hotFor('#34d6c5') === '#e0a24a'` byte for byte, verified by running it. So one Settings picker moves both tones, which is what *"both color sections"* asks for. **Every pair was rendered as a mini-spectrum on the Black paper and on Cream and looked at before the rule was chosen** — blue→lime, amber→violet, rust→cobalt, green→rose, violet→mint, rose→sky; a fixed-lightness alternative was rendered beside it and read harsher on blue and violet. The mono accents and the contrast flip collapse to one tone. Six draw sites change, each cited; the matching rows, the heart's rust and the website itself do not.
- ⚠ **THE TWO STALE CAPTURES ARE REPLACED, AND THAT IS THE ONLY CODE THIS SESSION SHIPPED.** `getapp-home-v5.png` and `getapp-radio-v3.png` — the pre-Signal-Field Radio page and a Home still printing a listener count — replaced in place with the 09-17 re-shots (600×1387, the signed-out preview, the clock pinned like the other eight), `?v=20260917` on `GetApp.html:300`, `:307` and the homepage's price phone at `index.html:886`. The review's §6 called it *"the first fix whichever design wins"*; it needs no design. Suite **3951/3951** on the swapped tree, the precompile check 72 pages / 0 errors.
- ⚠ **AND THE FIRST SUITE RUN DIED AT TEST 3164 AND REPORTED EXIT 0.** A background task whose command ended in its own `&` orphaned the inner job when the outer shell returned — the log stopped mid-file with no summary and the wrapper called that success. *A green exit from a shell that did not wait is a green exit from the shell.* Re-run without the inner fork: the count above is that run's.
- ⚠ **THE BOARD'S OWN CITATIONS DRIFTED BEFORE THE BRIEF WAS AN HOUR OLD.** Machine-printing every uncertain `path:line` corrected fourteen of them, off by one to six lines — a CSS block cited at the `a {}` rule beside it, a widget cited at its closing div, the website's tile rule cited at the flood line under it. Every one corrected from the printed line, none from memory. *A citation nobody re-derived is a claim.*
- **Open, for the build session:** both briefs; the review's §6 as marked; and the branch `claude/great-edison-dntp3q`, rebased onto `f52db62` with the 09-17 review, the gallery records and this swap on it.

### 2026-09-17 — The App page reviewed and re-imagined three ways: the one nav tab where the site's nav disappears

- **Records only — a review with previews, not a build.** Owner: *"review the app page on shape website, give me 3 new designs for that page"* → *"i want to see previews before"*. The review is [`REVIEW-2026-09-17-app-page.md`](REVIEW-2026-09-17-app-page.md); the previews are a live concept board — https://claude.ai/artifact/CoRssAAAWmSX8jSo1KfMZQ — **thirteen artboards in four rows**: **Today · as shipped** (the page rendered in its real typefaces at 1440 and 390, plus the step-2 bezel and the step-8 Radio capture as evidence), then **A · The Front Page**, **B · One Day** and **C · The Panel**, each as a whole desktop page, a whole phone page and a frame that opens to scroll. **Recommended: C.** The owner reported the canvas not loading on first open; every artboard was verified under the type's own runtime (zero errors, every image and typeface present), and the same previews were published as a plain page — https://claude.ai/artifact/GAUZQXSvviKwSCepwsRywj — four tabs with the full-page renders as images, which loads like the earlier concept boards. **No code changed, no migration, no PR.**
- ⚠ **THE APP TAB IS THE ONE NAV DESTINATION WHERE THE SITE'S NAV DISAPPEARS.** Measured across the ten nav targets: `GetApp.html` is the only one with **zero `pageShell.jsx` references** — it draws its own two-link bar (the logo and *Notify me →*) — and the only one still on Fraunces / Space Grotesk / JetBrains Mono. A visitor who clicks App loses the eight tabs, the Radio pill and *Get started*; the ways out are the logo and the footer.
- ⚠ **IT IS A TEN-CLICK CAROUSEL AND NINE TENTHS OF IT IS INVISIBLE.** `render()` writes one step into the DOM at a time — measured **1 of 10 titles present**; with JS off the stage is an empty title, an empty body and `01 / 08`, a hardcoded total two steps stale, with no image at all. No hash per step, so no step can be linked or found.
- ⚠ **TWO OF THE TEN CAPTURES SHOW SCREENS THE APP RETIRED ON 09-14.** `getapp-radio-v3.png` is the pre-Signal-Field Radio page — `ON AIR · 3,472`, the `132 BPM` ring, the `0:00 / -0:00` scrubber, the keyframe EQ — and `getapp-home-v5.png` still prints `3,472 LISTENING`. Both carry `?v=20260910`; the retirements (#2072 · #2076 · #2088) landed 09-14/15; the Home file is also the homepage's price phone. **Both re-shot from today's build for the board** through the real signed-out preview flow; the site still serves the old pair, and the swap is the first fix whichever design wins.
- ⚠ **THE TEAL EYEBROWS READ AT 1.75:1 ON CREAM.** `#0ac5a8` is the accent for the app's black paper, where it measures 8.9:1; on `#efe5cd` the page eyebrow and the four footer heads compute to **1.75:1**, the step counter and © line at ink 0.40 to **2.59:1**, the waitlist's orange eyebrow to **3.20:1**. Computed, not eyeballed.
- ⚠ **FOUR STEPS PUT A CREAM BEZEL AROUND A BLACK SCREEN.** Every capture is the app's Black paper, but `dark:false` on TRAIN · EAT · GROCERY · COMMUNITY flips the frame to `#f4eedf` (measured `rgb(244,238,223)` on steps 2, 3, 4 and 9), so the bezel changes colour five times across a ten-step walk.
- **Also measured:** the ten dots are **9×9px** targets and the nav link 17px tall; the copy block is 616×279 of a 1440×900 fold, the phone is cut at the fold by 47px and the only conversion sits one fold down; **all ten PNGs (1.82 MB) load at once** — `loading="lazy"` does nothing on images stacked at `inset:0` inside the visible phone; the page never states the $5 price and carries no *Get started*; and **a Google Play sign-up is recorded as an App Store one** — the form never sends `platform` and the route defaults a missing one to `ios`.
- ⚠ **AND THE APP RUNS IN A BROWSER TODAY, WHICH THE SITE NEVER SAYS.** `vercel.json` builds it into `public/m` on every deploy and the app's own paywall carries *Preview the app first →* — and a grep across `public/*.html` and `public/newdesign/` finds **zero** links to `/m/`. Registered as an owner ruling rather than assumed: concept C shows the door, A and B would take the same line.
- **Copy claims, checked against the app:** Instacart, move video, starter templates and swaps are real; *"Pick friends to be notified when you miss"* has **no code path** (only the member's own OS reminders exist), and *"One live channel"* describes a station that is not broadcasting. The concepts drop both clauses; the wording is the owner's to settle.
- **The backbone every option carries:** the shared header and canonical footer · the site's type system · every screen visible in one scroll with every word in the DOM · real captures, one dark bezel, each region labelled *Example member · captured from the app's signed-out preview* (the homepage's own convention) · one Doto figure per screen, read off the screen · the $5 price and the two honest doors in the first viewport · the accent chosen for the paper (`#0a6560` on cream, 5.5:1; the app's `#34d6c5` on the dark grounds, 10.9:1).
- **The three.** **A · The Front Page** — the app calls its Home *The Shape Daily* and carries a masthead, so the page becomes the app edition of that paper: cream, a dateline, the Home screen as the lead story with the doors, nine column stories with photo crops and hairline column rules (**3,821px at 1440**). **B · One Day** — a dark hour-by-hour timeline, `06:45 WAKE` to `SUNDAY`, each hour's phone sticky beside its copy and a sky glow warming and cooling down the page (**7,087px**, the tallest). **C · The Panel** — the app's black paper: one directive plate with the three doors, then all ten screens as instrument plates in a five-wide board (**2,938px**, the shortest). **Pick: C** — the whole app in two folds, the doors in the first viewport, and the page looks like the product it sells; A the fallback for the cream register; B the best story and the longest page.
- ⚠ **THE FIRST RE-SHOOT BOOTED NOTHING, AND IT WAS THE CLOCK.** `page.clock.install()` installs fake timers that never advance, so the app's splash never cleared — zero controls at every stage, which reads exactly like a broken build. `clock.setSystemTime()` pins the date and leaves the timers real. And the static server read its `/m/` root from the environment **at import time**, so a script that set it after importing served a 404 for the app's own index. *A harness that reports nothing is reporting on itself.*
- ⚠ **AND A WIDTH PROBE CANNOT SEE A MONOSPACE FONT.** JetBrains Mono and every monospace fallback advance 0.6em per glyph, so the probe read them identical and reported *not loaded* on a page that had fetched the file — the 2026-09-15 lesson, paid for again; the fetch log is what settles it. The display face is proven by its width axis (Anybody: 340px at width 50 against 1,518 at 150), which a fallback cannot do.
- **Verified:** docs-only (the pre-commit hook skips the code gates) · every `path:line` re-read from `main` = `c62f0ba` · the shipped page driven in Chromium at 1440 · 1280 · 1024 · 860 · 390 · 320 in its real faces, stepped through all ten steps with the bezel and the active image recorded, the waitlist form driven, one render with JS off · the three concepts rendered at 1440 and 390: **zero horizontal overflow, zero page errors, zero controls under 24px** on all six renders · both re-shots at **600×1387**, the Home one without a listener count and the Radio one carrying the app's own `PREVIEW · EXAMPLE SIGNAL`.
- ⚠ **OWNER RULINGS NEEDED before a build** (review §6): the pick · the browser door · the two copy clauses · the re-shoot of the two stale captures (design-independent) · whether this page carries the coach apps at all.

### 2026-09-17 — The fold's eyebrow drops a chip that was labelling a heading, and the label that survives gets bound to its figure

- **One owner note on `public/newdesign/index.html`, one PR.** *"remove 'example' on index/home page on shape website"*, on a crop of the fold's eyebrow reading **YOUR CLIMB · SHAPE SCORE EXAMPLE**. It reads **YOUR CLIMB · SHAPE SCORE** now. #2116 → `4d1f9bb`. `public/newdesign/` and one test file; **no migration, no route, no data change, no mobile change, no i18n key.**
- ⚠ **READ AS THE CHIP THE SCREENSHOT POINTS AT, NOT EVERY EXAMPLE LABEL ON THE PAGE — and the other reading would have made the page claim four things nobody measured.** The page carries five `.exlabel`s and its own footer promises *"every figure on this page is measured or labelled an example"*: the other four mark the **ON AIR chip** on a station that is not broadcasting (the 2026-09-11 ruling — *"just have on air showing on demo mode which is fine"* — where the label is precisely what makes the chip allowed), the **demo coach cast**, and the **two captured screens**. *When two readings of an instruction differ, the one that loses nothing is the one to ship and write down.*
- ⚠ **AND THE CHIP THAT WENT WAS THE ONE LABEL NOT ATTACHED TO A FIGURE AT ALL.** The eyebrow names the SUBJECT of the fold; the drawn score two lines below carries its own `Shape Score · example` caption, and the journey dial carries a third in canvas. So the fold said *example* twice about one number, and the copy that came out was the one sitting over a heading rather than over a reading.
- ⚠ **WHICH MAKES THE SURVIVING CAPTION LOAD-BEARING, SO ITS GUARD IS BOUND TO THE FIGURE RATHER THAN TO THE PAGE.** The old assertion was a page-wide `visibleText(SRC)` match on `Shape Score · example` — satisfied by those words turning up **anywhere**: a footer line, another section, a caption moved off the figure, while the counting number itself sat unlabelled. **Proven in both directions rather than argued:** moving the caption out of the summit block **passes the old guard 18/18** and **fails the new one**. This is the lesson the ON AIR guard in the same file had already paid for (*"the label has to be bound to the thing it labels"*), applied to the label that just became the only one.
- ⚠ **AND IT LEAVES NO SCREEN-READER ASYMMETRY, WHICH IS WORTH CHECKING RATHER THAN ASSUMING** — the eyebrow is NOT `aria-hidden` and the summit block IS, so the removal takes a label off a row a screen reader reads and leaves the surviving one on a figure it never receives. That is sound rather than backwards: the whole decorative block is hidden together, so no unlabelled figure is exposed to that reader at all — there is no claim there to label. The sighted reader is the one who sees the number, and the number is captioned.
- ⚠ **AND A LAZY `[\s\S]*?</div>` CANNOT MATCH THAT BLOCK, WHICH IS WHY THE DEPTH WALK IS SHARED NOW.** `.sc` (the figure) and `.lb` (its caption) are **siblings**, so the first `</div>` closes the figure and the fragment ends *before* the caption — the guard would fail on correct code. `airCard`'s existing walk is extracted as `element(html, openRe)` so the two callers cannot drift, and the walked block must still contain `id="summit-score"`, or a broken walk passes on a fragment instead of failing.
- ⚠ **TWO COMMENTS WENT FALSE IN THE SAME DIFF, AND BOTH SAT WHERE THE NEXT READER DECIDES WHETHER TO RESTORE THE CHIP.** One read *"the figure is labelled an example **above**"* — that was the chip. The other counted **THREE PLACES** and named the eyebrow first. Both corrected at their sites, with the ruling recorded beside them, because *a comment that counts a list it does not own is a false claim with a fuse on it.*
- ⚠ **HEADLESS `--window-size` WOULD NOT GO BELOW ~500px HERE, SO THE 320/390/430 RUNS ALL MEASURED THE SAME 500px LAYOUT.** Stated rather than reported as five widths — the *"a guard that thinks it changed zone and did not"* class. The narrow-width question is answered by **intrinsic** size instead: the eyebrow's content goes **256px → 199px** (199 + 10 gap + 47 chip), so the change can only ever reduce that row and cannot introduce a wrap at any width. ⚠ It also explains the clipped 390px screenshot, which read exactly like page overflow: the **window** was 390 and the **layout** 500.
- **Verified on the MERGED tree** (`4d1f9bb`), and the merged tree was first proven **byte-identical** to the reviewed branch tree on both changed files: `npm test` **3951/3951** · the newdesign precompile check **72 pages, 0 errors** · the full pre-commit gate (`verify: all checks passed`) · **5/5 mutations killed**, each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal** — the caption deleted · the caption moved out of the block · the figure's id dropped · the walker replaced by a lazy `</div>` match · and **the Radio card's own label dropped**, which is what proves the refactor still guards the ON AIR case rather than only the new one.
- **Driven in Chromium on BOTH builds** (this branch and `origin/main`, same probe, same server): the headline **does not move** (`h1Top` identical at all three layouts measured — 205 / 236 / 149), the eyebrow row **keeps its 14px height**, **zero horizontal overflow either side**, and the summit caption still reads `Shape Score · example`. ⚠ **The served file was confirmed to carry 4 labels rather than 5 before any reading was trusted** — the stale-listener-on-a-reused-port control this file records, so the render is of the edited page rather than a cached one.
- ⚠ **AND `pkill -f "http.server 8731"` MATCHED ITS OWN INVOKING SHELL AND KILLED THE COMMAND (exit 144) — TWICE, the second time through `pgrep`.** The self-match this file already records for `http.server`, `pgrep`, `serve.mjs` and `serve-main.mjs`, in a **sixth** costume — and the ordinal was derived rather than guessed, which caught my own first instrument: `grep -oE "in a (third|fourth|fifth) costume"` returns **two** hits, because the file writes `in a **fifth** costume` and the asterisks split the phrase. *A count run with the wrong instrument is the same claim wearing a receipt* — the `<em>`-through-the-middle-of-a-phrase defect this file already post-mortems, one punctuation mark over. What works is a pattern that cannot match itself (`http[.]serv''er`), and the tell is that the cleanup had in fact completed — reading `git status` rather than the exit code is what said so.
- ⚠ **AND ONE PRECISION ERROR IN MY OWN NEW COMMENT, FOUND BY READING THE DIFF RATHER THAN BY A TEST.** It read *"A FOURTH LABEL SAT IN THE FOLD EYEBROW"* while the page carried **five** and now carries four — so as written it implies the vacuity floor directly beneath it had just gone from four to three. Comment-only, and worth the CI re-run: *the count is the whole subject of the assertion it sits above*, and a wrong one is what the next reader edits.
- **Review:** none, and the record says so rather than implying a layer ran. Owner, this session: *"no code review necessary. merge when green."* So no Codex trigger and no CodeRabbit; the round is my own adversarial read of the diff plus the mutation round. The merge gate was the four required checks green on the final head and not a draft.
- ⚠ **NO ON-ACCOUNT PASS.** Every reading here is the signed-out render against a static server; no signed-in member has opened the page.


### 2026-09-17 — The Radio hero stops changing with the monitor, and the third reason was one no bounding box could see

- **Two owner notes on `public/newdesign/`, one PR.** *"remove this entire section from shape radio page"* (the **IN THE APP / The same instrument, on your phone.** block) and *"the shape radio here needs to look cleaner and the same on every monitor size"*. #2113 → `7e6af9a`. `public/newdesign/` only; **no migration, no route, no data change, no mobile change** — `radioSignalField.mjs` and `radioTempo.mjs` are the app's own files and are untouched.
- **`RadioInApp`, its mount and its `.rd-two` media query are deleted**, 37 lines, with no orphans: nothing else referenced the component, the class or the copy.
- ⚠ **THE HERO READ DIFFERENTLY ON EVERY MONITOR FOR THREE INDEPENDENT REASONS, AND EACH BECAME VISIBLE ONLY ONCE THE ONE BEFORE IT WAS FIXED.** That is the shape of the whole change: two of the three were found by measuring the page, and the third by a review round after the first two had made it the only thing left moving.
- ⚠ **(1) THE WORDMARK'S BUDGET WAS A FRACTION OF A GRID THAT GROWS WITH THE SCREEN.** `wallCols(W) = floor(W/16)`, and `buildMask` fitted the word to `cols * 0.88` with a font step off `rows * 0.42` — **both fractions OF THE GRID**. Measured on the real page: **1120×128 at a 1280 fold and 1712px wide at 3440**, i.e. 87.5% of the fold collapsing to 44.2%. `wallWordFit` caps it at a **tile count** (`WALL_WORD_MAX_COLS = 70`) while the fold's own share still binds **below** that width, so a narrow fold shrinks the word rather than clipping it. Measured after: **1120×128 at every desktop width from 1280 to 3840**.
- ⚠ **AND THE GRID DELIBERATELY DOES NOT BECOME FIXED-COLUMN.** The wall's 16px cell pitch is what makes it read as a wall; pinning the column count would rescale the tiles with the viewport and turn a dot-matrix into a stretched bitmap. **The budget is capped; the grid still tracks the fold** — the residual is a `[1264, 1280)` band costing one font step, where before the change the same word ran 1120 → 1712px across the desktop range.
- ⚠ **(2) THE CHROME WAS PINNED TO THE VIEWPORT, NOT TO THE SITE'S MEASURE.** `.rd-top` / `.rd-bottom` ran full-bleed, so on a 3440 monitor the NOW PLAYING block and the key sat **~1000px outside the header's own content column** — the fold was the one page on the site ignoring the site measure. Both are staged to `RD_STAGE = 1440`, **read from `pageShell.jsx` rather than typed**, and step the header's own 32/24/18px gutters; `.rd-hero` neutralises the site-wide `section { padding-left: 22px !important }` at ≤900. The cloud is staged with them, so it stops drifting to 62% of a 3840 screen while the word stands still.
- ⚠ **(3) AND THE WORD STILL LOOKED DIFFERENT AFTER BOTH — THE FINDING A BOUNDING BOX CANNOT SEE, AND THE REASON THE CODEX ROUND WAS WORTH RUNNING.** `wallBand` stretches all 32 bands across the grid, so a word held at a constant ~70 cells covers a **different slice of the spectrum** as the monitor widens. Measured by driving the shipped rules: bands **2..29 at 80 columns, 7..25 at 120, 9..22 at 160, 11..20 at 240**. A word tile only turns hot when its own column is near saturation — the word sits 0.46 down, so `fromBottom` ≈ 0.54 against a 0.55 span, i.e. **`meters[c] > 0.982`** — and **bass reaches that on a kick where treble does not**. So the **whole** word flashed on a wide monitor and only its **middle letters** flashed on a narrow one. The word takes the centre column's meter at every width now; that column **is** the bass, because `bandsFromBins` is already mirrored and `wallBand` puts bands 15/16 in the middle.
- ⚠ **IT ADOPTS THE WIDE-MONITOR BEHAVIOUR EVERYWHERE RATHER THAN INVENTING A THIRD ONE**, and the **ROW** term is deliberately kept, so the word still fills from its baseline up exactly as it did. Only the column dependence is removed, and only for the word — the wall's own tiles are untouched and still read their own band.
- ⚠ **A CACHE KEY WOULD HAVE SHIPPED AN EMPTY HERO, AND THE GUARD CAUGHT ITS OWN AUTHOR FOUR TIMES.** `scripts/build-newdesign.mjs` content-hashes `<script type="text/babel">` tags **only** (its `BABEL_TAG` pattern), so a `.mjs` import keeps whatever `?v=` a human last typed: a new export in `radioField.mjs` behind a stale key means `rdLib` bails, the field effect returns, and **the fold draws its chrome over an empty canvas with no error anywhere**. All four module imports are content-hashed and `tests/radio-module-cache-keys.test.mjs` recomputes them from the **bytes**, printing the exact remedy line. It failed four times on this branch — every time because I edited the module after hashing. *A guard whose failure message is a paste-ready fix is a guard people actually use.*
- **THE CODEX ROUND ON `71602f2` RETURNED THREE P2s, ALL VERIFIED AT THE SOURCE, AND TWO OF THEM LANDED IN AREAS THE TRIGGER COMMENT HAD ASKED IT TO ATTACK.**
- ⚠ **THE SECOND FIX IS THIS FILE'S OWN LESSON TURNED ON ITSELF.** The cache-key guard's vacuity floor was **derived from the page**, which is what its comment claims — but derived **with the same assumption as the thing it checks**: `ANY_MODULE` and `VERSIONED` were both anchored on a **double quote**, so a single-quoted import fell out of the versioned set **and** out of the floor *together*, `ALL.length === IMPORTS.length` still held, and that import could ship with no content hash and no failure. *A control that reimplements the thing it controls is not a control.* It parses the `<script type="module">` bodies with `@babel/parser` now and reads `ImportDeclaration` nodes, so quotes, line breaks and an import-shaped string inside a comment cannot move the answer; a body that stops parsing **throws** rather than reading as an empty page.
- ⚠ **AND THAT FIX IS PROVEN IN BOTH DIRECTIONS, BECAUSE ONE DIRECTION ALONE IS A SPELLING PIN POINTED THE OTHER WAY.** The exact case — single-quoted **and** the hash dropped, which the old guard passed — is **KILLED**; single-quoted with the hash intact, i.e. correct code, **still passes**.
- ⚠ **THE THIRD FINDING IS CONFIRMED IN MECHANISM AND REFUSED IN REMEDY, ON A MEASUREMENT.** Snapping the word to a whole column leaves it up to **8.0px** off the fold's centre and steps it a full **16px** at W = 1296, where `cols` crosses 80 to 81 — all exact, W included. But measured across 1280..3840, **dropping the round takes that to 7.5px and 8px**: half a pixel of centring, paid for with the **16px SIZE jitter** the snap was added to remove (1104 ↔ 1120). **The quantisation is not removable** — the mask is one pixel per tile, so the word can only ever sit on a 16px grid and the choice is *which artifact to spend*, not whether to have one. This change spends position to hold size. ⚠ **Recorded at the site with both numbers, because the comment there had named only the size cost** — the reviewer supplied the half I did not have, and a residual nobody can re-derive is a residual the next reader re-opens.
- ⚠ **ONE MUTATION SURVIVED ITS FIRST ROUND AND IT WAS A CLAIM WITH NOTHING BEHIND IT.** Dropping the row term makes the whole word flash at once — which is **width-independent**, so it passed every assertion I had written — while the comment at the site promised the word still fills from its baseline. *A because-clause is a claim, and a claim with no guard behind it is the thing this suite exists to stop.* Closed and re-killed; **4/4** on that guard now.
- ⚠ **AND THE RECOLOR GUARD IS DRIVEN, NOT PINNED, WITH ITS OWN CONTROL.** The shipped `wallBand` / `wallWordFit` / `WALL_METER_SPAN` are run over a bass-heavy frame at 80 and 240 columns, **behind an assertion that the retired rule really does differ across those widths** — so a green result is the fix rather than a fixture too flat for any rule to look constant. The call site is pinned separately, or the test drives a restatement while the renderer keeps the defect.
- **A 22-AGENT ADVERSARIAL ROUND RETURNED 17 VERIFIED FINDINGS: 2 CONFIRMED, 15 REFUTED** — and **every one of the fifteen was the head-pinning trap**, filed against earlier heads that later commits on the same branch had already fixed. *A verdict is only about the head it names*, and at this fan-out that is most of what the round produces.
- ⚠ **THE ONE SUBSTANTIVE FINDING FROM IT IS REGISTERED WITH A CONTROL THAT REFUTES ITS SEVERITY.** The scrim's eyebrow contrast falls with width and this change makes it **marginally** worse — verified independently: mechanism and direction real, **magnitude materially overstated**, measured **5.24 → 5.05** at 3840 against an AA floor of **4.5**, not the 5.27 → 4.60 claimed. And the severity argument dies to a control: **the spread was already width-dependent before the change** (0.09 against 0.10, opposite sign). Both proposed remedies refused at the site with reasons.
- ⚠ **AND MY OWN CONTRAST PROBE IGNORED THE TEXT'S 0.55 ALPHA, WHICH WOULD HAVE CLEARED A REAL FINDING ON A NUMBER FOR A COLOUR NOTHING PAINTS.** It reported **13.81 → 13.35** for opaque cream on type that sits near 5:1. The shipped figures composite in sRGB and the probe self-checks (white/black = 21.00, black/white = 21.00, grey on itself = 1.00). ⚠ The eyebrow figure is also quoted as the **range five runs produce (6.76–6.84)** rather than the point value I first wrote (**6.93**) that five runs never returned — *a measurement nobody re-derives is a claim*.
- ⚠ **FOUR MORE INSTRUMENTS REPORTED ON THEMSELVES BEFORE THEY REPORTED ON THE PAGE.** A probe **reimplemented** `buildMask`'s retired rule and therefore reported the fix as not landed (it lifts the shipped `wallWordFit` now). A control server's `grep -c` read **0 out of a 404 body** and I called that *"confirmed the pre-fix build"* — a **false confirmation** from a stale listener on a reused port; the control asserts the served file contains a known marker now. An overlap probe took the worst hit **by area**, named a wrapper spanning the whole strip, and had me believing the primary key was covered — it asks `elementFromPoint` at each control's own centre now, and the real dead control is the NORA chip. And a `Monitor` reported the commit **"STILL STAGED"** while it was mid-flight. *An instrument that reports a pass is broken until something proves it can fail, and one that reports a failure is broken until the failure is proven to be the code's.*
- ⚠ **FOUR OF MY OWN GUARDS FAILED A CORRECT TREE BEFORE THEY GUARDED ANYTHING.** One demanded a media-query rule at exactly ≤1020 that the ≤1100 rule already covers — it compares **effective computed padding** now rather than rules. One carried a vacuity floor of `>= 3` against **four** imports, so a mutation survived; derived from the page. One restated `W * 0.62` and reported the staged cloud as unchanged. And the `<style>` TemplateLiteral guard was widened from `pageShell.jsx` alone to **all 80** newdesign `.jsx` (allowing the `StringLiteral` `dashToday.jsx` legitimately uses), with a floor of 15 so it cannot pass on an empty walk.
- **Verified on the MERGED tree** (`7e6af9a`), re-derived rather than carried — and the merged tree was first proven **byte-identical** to the reviewed branch tree: `npm test` **3943/3943** · `tsc --noEmit` **0** · JSX parse on every changed module · `radioField.mjs` imports clean · the newdesign precompile check **72 pages, 78 shared jsx, 0 errors** · **all four cache keys reconciled against file bytes** · **28 mutations killed across four rounds** (16 · 5 · 1 · 6), each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal**, every file read and written as **bytes** — **both fixed Codex findings replayed as their own mutations**, so the suite is proven to catch them rather than merely to be green after the fix.
- **Driven in Chromium at 13 widths from 320 to 3840**, with React, ReactDOM and Babel served from **npm-packed bytes that satisfy the page's pinned SRI exactly** (the proxy blocks unpkg, and a hand-rolled substitute is correctly rejected) and the fonts served locally from **absolute** URLs: the wordmark measures **1120×128 at every desktop width from 1280 up** and shrinks below, **zero controls under the 24px floor**, zero horizontal overflow, zero page errors. ⚠ **The faces are proven real by the `wdth` axis, never by `document.fonts.check()`**, which answers **true for a fallback** and so cannot settle the question at all.
- ⚠ **REGISTERED, NOT FIXED — each checked against a control build on `main` rather than assumed:** the **chat-bubble / NORA chip overlap at 1280×720**, a site-wide fixed overlay measured **identical on both builds**, so it is not this change's; the fold naming **"Shape Radio" three times**, which is a copy call and where the logo raster is the hero's only accessible name; the `[1264, 1280)` scrollbar band; the **un-hashed inner imports** (`radioField.mjs` → `radioSignalField.mjs` / `radioTempo.mjs`), harmless **today** because module identity is per-URL and **both shared modules declare zero top-level `let`/`var`** (measured), so there is no state to diverge; and the **two pre-existing wordmark/chrome touch conditions at 26 and 27 rows**, recorded as a measured table at the site because the 26–30 row band had never been swept.
- **Review:** Codex, **one front-loaded round** as the standing rule requires — **3 findings on `71602f2`, all real, two fixed and one refused with a measurement**, each verified against the source before acting, each replied to on its own thread, the two fixed threads resolved and **the refused one deliberately left open so its reasoning stays visible**. Per the one-round rule the fix head was **not** re-triggered, so that diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not run** (owner's standing ruling); its comment is the automatic under-10-stars skip notice and is not a review. The merge gate was the four required checks green on `e703b78` and not a draft; the squash-merge pinned `expectedHeadSha`, so it could only ever have landed the commit that was verified.
- ⚠ **NO ON-ACCOUNT PASS.** Every reading here is the signed-out render against a static server; no signed-in member has opened the page.

### 2026-09-16 — Radio stops selling movement, Nora's booth had been dead since a dependency bump, and the wall's wordmark was sized by the monitor

- **Three owner reports on one page, one commit** — `333a30e` on `claude/awesome-meitner-klhllm`; **no PR opened, none was asked for**. *"remove the 'mixed for movement' on the shape radio page from shape sets description. I dont want shape radio to be marketed as 'built for movement.' I just want it to be a radio station with great music that users have accesss to with membership"* · *"also the nora preview is not working"* · *"also the look and clarity of this changes with the monitor size"*. `public/newdesign/`, the app's Radio module and 13 catalogs; **no migration, no route, no data change.**
- **THE COPY: the clause is deleted in each locale's OWN sentence, never re-translated.** Shape Sets reads *"DJs and live acts, captured on the floor and streamed through Shape Radio."* on both surfaces. Every catalog keeps its own remaining words — **1 insertion / 1 deletion per file**, 82 keys, parity clean 13/13 — because removing a clause is a deletion job, not a translation job, and re-authoring thirteen sentences to delete three words is how a catalog acquires new wording nobody asked for.
- ⚠ **AND MY FIRST SWEEP MISSED THE HOMEPAGE, BECAUSE A TAG RUNS THROUGH THE MIDDLE OF THE PHRASE.** `public/newdesign/index.html:836` reads **`<h2>Music built for <em>movement.</em></h2>`** — the banned phrase, verbatim, on the front page — and a raw `grep` for *"built for movement"* **cannot match it**: the `<em>` splits the string. This file already post-mortems the identical defect on #2069 (*"the timing sweep stripped tags and not entities or JSX containers"*), and I walked into it. The sweep **renders first** now — tags, entities, `<br>`, and `{" "}` JSX containers — and finds **45 lines** where the literal grep found 7. *A sweep over raw markup is a sweep over a string the reader never sees.*
- ⚠ **AND THE PITCH WAS THE FIRST THING THE APP EVER ASKED ABOUT RADIO.** `radio:prompt.titleAccent` — the ask-prompt mounted in the client shell **and both coach shells** — read **"Want music *while you move?*"** in all 13 locales. Retired to *"while the app is open?"*, each locale composed from its **own** `prompt.body` clause for exactly that (de *wenn die App geöffnet ist* · ru *пока приложение открыто* · tr *uygulama açıkken*, which is that catalog's own `uygulama açık` inflected). Same for the onboarding tour (`onboarding:tour.radio.body`, *"Ad-free workout mixes, curated by BPM"* × 13) and the in-app pricing card and FAQ.
- ⚠ **THE TOUR EDIT KEEPS EACH LOCALE'S HEAD NOUN, AND THAT IS WHAT PROTECTS THE GRAMMAR.** Deleting the modifier and the BPM clause while leaving the noun means every agreement downstream still holds — es *Mezclas … **Incluidas***, fr *mixes … **Inclus***, pt-BR *Mixes … Grátis*. Re-authoring the noun to "music" would have silently broken the adjective in at least three of the thirteen, and nothing in the suite checks agreement. **Verified per locale that the second sentence is carried byte-verbatim.**
- **Also cleared on the canonical live set:** the homepage Radio card (headline, body **and the meta description**, which is the search snippet), `pricing.jsx`'s feature line and FAQ (Pricing is main-nav), `clientOverview.jsx`'s *"Curated stations for lifting, running… auto-matches the tempo of your session"* (Client.html, main-nav), and `dashTour.js`, which all three dashboards load.
- ⚠ **AND THE BANNED PHRASE WAS LIVE VERBATIM TWO SURFACES OVER.** `GetApp.html`'s RADIO step read **"Music built for movement."** over *"Each track's BPM syncs with your wearable — when the two lock in, your pace becomes the song"*, and the legacy `public/mobile/radio.jsx` — **reachable, linked from `Mobile.html`, `Index.html` and `ClientTrain.html`** — carried *"Mixed for movement"* and *"Synced to your program"* as feature headings. Fixing only the sentence the owner screenshotted would have left the page that sells the app saying the exact words they asked to retire. A repo-wide sweep returns **no "mixed/built for movement" in any code or markup**.
- ⚠ **THE BPM MATCHING IS NOT REMOVED, AND THAT DISTINCTION IS THE WHOLE READING OF THE RULING.** The Signal Field's heart-rate match is a **shipped feature**; the owner asked to stop *marketing Radio as movement music*, not to delete it. So it survives, described as an option (*"there if you want it, easy to ignore if you don't"*) rather than as the reason to listen. *A ruling about a pitch is not a ruling about the product it is pitching.*
- **THE NORA BOOTH: it threw on every open, and had done since a dependency bump.** Driven in Chromium against the page's own pinned versions, **"Watch Nora (preview)" answered "The booth could not start on this device" with WebGL healthy** (SwiftShader present, `webgl.ok: true`). `RadioNora`'s `catch` swallows the error, so it reached no console and no log; reproducing the open sequence with the error visible gave **`TypeError: VRMUtils.combineSkeletons is not a function` at `noraStage.mjs:53`**.
- ⚠ **ONE MODULE, TWO DEPENDENCY PAIRS — AND THE APP WAS FINE THROUGHOUT, WHICH IS WHY NOBODY SAW IT.** `noraStage.mjs` is bundled for the app by Vite against `mobile-app/node_modules` and resolved for the web by `Radio.html`'s import map off esm.sh. A bump moved the app to **three 0.185.1 / three-vrm 3.5.5** and left the map on **0.169.0 / 3.1.6**. `combineSkeletons` was added to three-vrm **after 3.1.6** — measured, **0 occurrences in 3.1.6's bundle, 5 in 3.5.5's** — so the call existed on the phone and not on the web. **And a missing static method is a runtime `TypeError`, not a bundler resolution error**, so no build, no typecheck and no test could see it.
- ⚠ **A COMMENT IS WHAT HID IT, AND IT IS THE CLASS THIS FILE POST-MORTEMS MOST.** `mobile-app/vite.config.ts` asserted these were the *"same versions as the web import-map — three@0.169.0 + @pixiv/three-vrm@3.1.6"* while `package.json` said 0.185.1 / 3.5.5. Anyone who read it would have believed the two were in sync and looked elsewhere. Corrected at the source; `tests/nora-stage-version-parity.test.mjs` is what enforces it now.
- ⚠ **THE GUARD IS TWO ASSERTIONS THAT CHAIN, AND NEITHER IS SUFFICIENT ALONE.** (a) every `VRMUtils` member `noraStage.mjs` calls exists in the **installed** three-vrm — which on its own only proves the APP is safe, since that is mobile's `node_modules`; (b) the import map pins the **same versions the app installs**. Together they carry the result across to the web. Drop either half and the web side is unproven again. Both carry vacuity checks, so a `package.json` whose keys moved or a sweep that stops matching **fails** rather than sailing past on two undefineds.
- **Fixed by aligning the map to the app's pins**, because that pair is what production traffic already runs. **Proven by A/B in a real browser rather than by reasoning**: with 3.1.6 the booth throws; with 3.5.5 it loads and renders the dot-matrix hologram — on three 0.169.0 **and** 0.185.1, so the bump is not load-bearing and the drift was.
- ⚠ **"THE NEWDESIGN PAGES CANNOT BE DRIVEN HERE" IS FALSE A SECOND TIME, AND THIS ONE EXTENDS IT TO esm.sh.** #2089 already refuted it for unpkg; the same trick reaches the module graph. `npm pack` **is** available through the proxy even though `esm.sh` and `unpkg` both answer **000** — so `three@0.169.0` and `@pixiv/three-vrm@3.1.6`, the exact production pins, were fetched and served locally, and React/ReactDOM/Babel's `npm pack` bytes **match the page's own `integrity` attributes byte-for-byte** (all three verified), so the page loads unmodified except for host. *A blocked CDN is not an unobtainable dependency, and the version a page pins is testable.*
- ⚠ **AND THE BOOTH NOW SAYS WHY IT FAILED, WHICH IS THE REASON THIS ONE COULD SIT BROKEN.** `RadioNora`'s catch swallowed the error entirely: a member, and anyone reading a console, got *"The booth could not start on this device"* — which names no cause — while the real answer was a one-line `TypeError`. Finding it took reproducing this function's own sequence by hand with the error made visible. **The app's copy of this flow has always warned** (`console.warn('[nora] stage failed', e)`); the web's did not. It does now, and the member-facing message is deliberately unchanged — it is honest, and there is nothing useful to say to them. Driven with the VRM aborted: the console carries `[nora] booth failed to start TypeError: … at NoraStage.load (noraStage.mjs:50)` with a full stack, and the on-screen line is untouched. *A swallowed cause is how a dead feature stays dead.*
- **THE WALL'S WORDMARK: two independent causes, both measured, neither the one that looked likely.**
- ⚠ **THE MASK WAS RASTERISED AT ONE PIXEL PER TILE.** `buildMask` rendered "SHAPE RADIO" into a canvas of `cols × rows` **pixels** — at 1440 that is **90 × 41 for the whole wall** — and each tile took its state from a **single sample** thresholded at half of 255. Stems that fell between sample points dropped out, counters filled in, and the same letter came out differently at two widths. Supersampled **8× per axis** now, with a tile lighting on **coverage**.
- ⚠ **AND MY FIRST THRESHOLD MADE IT WORSE, WHICH ONLY LOOKING SHOWED.** Supersampling at the inherited half-tile threshold rendered the word **thinner and more broken** than before — a 600-weight face at 10–11 tiles tall draws stems about **one tile wide**, so demanding half a tile of coverage erases them. Rendered at 128 / 64 / 40 side by side and read: **64 is where the stems survive and the counters stay open**. `WALL_MASK_INK`. *A supersample is not automatically an improvement; the threshold is half the rule and it was inherited unexamined.*
- ⚠ **`WALL_MASK_MIN_COLS` WAS MEASURED AT ONE END OF THE RANGE AND ASSUMED AT THE OTHER, AND THE MIDDLE IS WHERE EVERYONE IS.** The wall is a **fixed 660px tall** — 41 rows always — while its columns are `floor(W / TILE_PX)`, so the fit loop shrinks the type until the word fits 88% of the **columns** and the glyph height is whatever is left. Measured across the range: **6 tiles at 768px · 8 at 1024 · 10 at 1280 · 11 at 1440 · 15 at 1920**, capping at 17 only past **~2240**. So the wordmark was drawn nearly **three times bigger on a wide monitor than on a small laptop** — the owner's report, in one table. At 44 (≈704px) the long form was allowed at **six tiles tall**, the exact noise the rule exists to prevent. It is **80** (1280px), chosen from the render rather than the arithmetic; below it the wall says SHAPE and gets the whole width.
- ⚠ **AND EVERY PRE-EXISTING ASSERTION ABOUT THAT RULE STILL PASSES UNCHANGED**, which is the tell that the rule was right and only its number was wrong: 1440 is still the long form, 390 and 320 are still the short one, and the two guards that pin the constant are written against the constant.
- ⚠ **A RULE THE PAGE CAN BYPASS IS HALF A FIX.** The threshold shipped as a bare `> 128` **inside the draw loop**, where nothing in Node could drive it — the third opinion about one rule, the shape `radio-instrument-rules.test.mjs` exists for. Both the threshold and the word form are read from `radioField.mjs` now, and a guard asserts the page carries **no literal**; the mutation that reintroduces one is killed.
- ⚠ **FOUR OF MY OWN INSTRUMENTS WERE BROKEN BEFORE THEY MEASURED ANYTHING, AND EACH FAILED IN THE DIRECTION THAT LOOKS LIKE A FINDING.** (1) A leak probe counted `webglcontextlost` — but `NoraStage.dispose()` calls `renderer.dispose()` and **never `forceContextLoss()`**, so it could not fire and read `0` on a working fix. (2) The replacement counted GL draw calls and told Playwright to click mid-load — **Playwright waits for a control to become ENABLED**, and the button carries `disabled={state === "opening"}` for exactly that window, so it landed a **normal** close after the load and measured the path that already worked. (3) A `readPixels` check re-called `getContext` with `preserveDrawingBuffer` — **the attributes of an existing context are ignored**, so it read zeros off a canvas with a figure plainly on it. (4) The width sweep ran without a **measured** signed-out `/api/me`, so `signedIn` stayed `null`, the visitor preview never armed, and every capture was of the honest off-air state rather than the state the owner screenshotted.
- ⚠ **AND THE GUARD FOR ITS OWN SPAN CAUGHT ME: `MASK_SS` IS DECLARED ABOVE `buildMask`, NOT INSIDE IT**, so the first version read `null` out of the body and failed — which is the right failure, and the only reason it was visible is that the assertion names the value it read.
- ⚠ **`pkill -f serve.mjs` MATCHED ITS OWN INVOKING SHELL AND KILLED THE COMMAND (exit 144)** — the self-match this file already records for `http.server`, `pgrep` and `serve-main.mjs`, in a **fifth** costume. And writing `vite.config.ts` back through a **text-mode** rewrite normalised the one **CRLF-stored** file in the change to LF: a two-line comment edit arrived as **429 changed lines**, caught by reading `git diff --stat` rather than by the edit reporting anything. Re-applied in **bytes**: 7 insertions, 2 deletions, 217 CR intact. *The lead-boosts lesson, paid again.*
- ⚠ **REGISTERED, NOT CREDITED WITH A FIX: the booth's abandoned-load guard.** `stageRef` is assigned only after `load()` **and** `start()` resolve, so `close()` and the unmount cleanup both see `null` during a ~10.7MB VRM download — the guard the app's own copy has (`if (disposed) { st.dispose(); return; }`) and this one did not. It is added, and it is **hardening rather than a reproduced symptom**: measured, Hide is disabled for that whole window, so the only abandon path is an unmount, which on this page is a navigation that discards the context anyway. Said so at the site rather than claiming a defect.
- **Verified on the pushed head** (`333a30e`): `npm test` **3908/3908** · `tsc --noEmit` **0** · JSX parse on every changed module · the newdesign precompile check **72 pages, 0 errors** · i18n parity **13/13 at 82 keys** · mobile build clean with the new copy confirmed **in the emitted bundle** behind a positive control (`Club Shape`) **and thirteen negative ones** — every retired phrasing reads **0** · **18 mutations killed across two rounds** (8 on the version parity, 10 on the mask), each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal**, with **both shipped defects replayed as their own mutations** · and the page **driven in Chromium at 768 · 1024 · 1280 · 1440 · 1822 · 2560** in the visitor-preview state, plus the booth opened end to end: **zero page errors, zero horizontal overflow** at every width, and the wordmark **looked at** rather than only counted.
- ⚠ **NO ON-ACCOUNT PASS, AND IT IS THE STANDING CAVEAT.** Every reading here is the signed-out visitor preview against a local server with the CDN shimmed; no member has opened the booth on a real browser against esm.sh, and **esm.sh itself is unreachable from this container**, so the aligned pins are proven against the same package bytes rather than against the CDN that will serve them. The honest check is the deployed page.
- ⚠ **AND THE WORDMARK STILL SCALES WITH WIDTH BY DESIGN — 10 tiles at 1280 against 17 past 2240.** What changed is that it is **legible at every width** and the noise case is gone. Pinning it to one absolute size means it stops filling the fold on a wide monitor, which is a composition call and the owner's to make.
- ⚠ **CORRECTED — "NO PR OPENED, NONE WAS ASKED FOR" IS NO LONGER TRUE, AND THE HEAD MOVED WITH IT.** The owner asked the same day (*"open PR"*), so this work is **#2114**, opened on the merge-up of that day's `main` and closed at **`41753f1`** rather than at the `333a30e` this entry names. Marked rather than rewritten — a dated entry says what was true on its date — but this file is **auto-loaded every session**, so an unmarked *"no PR"* reads as the current state to whoever lands on it.
- ⚠ **AND A FOURTH OWNER REPORT IS RECORDED HERE BECAUSE THE ENTRY NEVER CARRIED IT.** *"also this wording is very confusing"*, on the **IN THE APP** block: *"The same instrument, on your phone."* named a thing the reader has no referent for — *instrument* is this page's internal word for the Signal Field, not a word a visitor arrives with. I reworded it to *"The same station, on your phone."* — and ⚠ **THAT REWORDING NEVER SHIPPED, BECAUSE #2113 ANSWERED THE SAME REPORT BY DELETING THE SECTION.** Two sessions took the same owner report and reached different answers; theirs merged first, so `RadioInApp` is gone and the rewrite went with it in the merge. **The deletion is the better answer to "this wording is very confusing"** — the clearest version of a confusing paragraph is often no paragraph. Recorded rather than quietly dropped, because the report was real and the entry had not carried it at all: this entry's own headline counts three reports; there were four.
- ⚠ **CORRECTED — "THE SWEEP RENDERS FIRST NOW" IS NECESSARY AND IS NOT SUFFICIENT, AND THE ROUND IS WHAT SHOWED IT.** Codex, on #2114: the same homepage Radio card's subline still read **"One channel, tuned to your session."** — the identical pitch, nine lines under the headline that had just been rewritten. **The renderer was not what missed it: the PATTERN LIST was.** That phrase is plain text, split by no tag, and a rendering sweep buys nothing against a phrase nobody banned. *A sweep has two halves, and this entry celebrated one of them.*
- ⚠ **SO THE SWEEP STOPPED BEING A SCRATCHPAD SCRIPT.** `tests/radio-marketing-pitch.test.mjs` is the standing guard the ruling never had — measured before writing it, a grep across `tests/` for the retired copy returned **zero** guards, so a second instance on the surface this entry claims to have cleaned was evidence a third was equally likely. It renders first, strips comments through the shared helper, carries a vacuity floor and a **keep-list of copy that must not fire** — and that keep-list earned itself on its first run: a bare `/while you move/` also matches *"it stays put while you move between tabs"*, a comment about tab state. Anchored on the end of the clause now. Scope is the canonical live set: **223 files, 0 hits**.
- ⚠ **AND `directionA/B/C.jsx` ARE NOT "UNREFERENCED", WHICH THE PR BODY CLAIMED AND I MEASURED WRONG.** Four pages each load them (`index-explorations.html`, `index-print.html`, `Shape Redesign*.html`). They are **retired explorations** reachable only from unlinked boards and the legacy mobile tree — this repo's own recorded position in `pageShell.jsx` — so the conclusion held and the wording did not. The guard skips them and **asserts the skip**: wire one into a live page and it fails, and a second assertion fails if those files are renamed away, so the first cannot pass for the wrong reason.
- ⚠ **CORRECTED — "A GUARD ASSERTS THE PAGE CARRIES NO LITERAL" WAS AN OVERCLAIM, IN THE BULLET WRITTEN TO CELEBRATE CLOSING THAT EXACT HOLE.** The guard asserted the **presence** of `F.WALL_MASK_INK` and of the mask sample, and **no absence assertion existed at all** — so `mask[..] > F.WALL_MASK_INK && mask[..] > 128` satisfied every line of it while the half-tile gate erased the stems again. Codex found it and its bypass is exact. The literal is rejected now, behind a guard-the-guard proving the pattern sees both shapes the defect takes and does **not** fire on the correct loop.
- ⚠ **AND "THE TWO GUARDS THAT PIN THE CONSTANT" PINNED RANGES, NOT THE CONSTANTS.** `WALL_MASK_INK` was asserted **32..127** and `WALL_MASK_MIN_COLS` **65..80** — so either could drift to a value the render comparison had already rejected, with the suite green. Neither comes from an invariant; both were chosen by looking at 128 / 64 / 40 side by side. Both are pinned by **equality** now, with the literal written **in the test** rather than read back from the source, which would have made the assertion tautological — the `dashGrid` `278` lesson, applied. *A range that admits the values the render rejected preserves nothing.*
- **Review:** Codex, **one front-loaded round** as the standing rule requires — **3 P2 findings on `cec9929`, every one real, every one fixed**, each verified against the source before acting, each replied to on its own thread and resolved, and **each replayed as its own mutation** so the suite is proven to catch it rather than merely to be green after the fix. Per the one-round rule the fix head was **not** re-triggered, so that diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not run** (owner's standing ruling; its skip-review notice is automatic and is not a review). The merge gate was the four required checks green on the final head and not a draft.
- **Verified on the final head** (`41753f1`): `npm test` **3934/3934** · `tsc --noEmit` **0** · JSX parse on every changed module · mobile build clean with `public/m` in sync · `verify: all checks passed` · **8 further mutations killed across two rounds**, each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal**, every file read and written as **bytes** — Codex's exact bypass, both constants moved **inside** their old ranges, the retired subline restored, the tag-split headline restored, the retired prompt wording restored in a shipped catalog, an exploration wired into a live page, and the walk collapsed to nothing.
- ⚠ **AND TWO COMMENTS WENT STALE INSIDE THIS PR'S OWN DIFF, FOUND BY READING IT RATHER THAN BY THE ROUND.** Retiring `radio:prompt.titleAccent`'s wording left `iosAppBroadsheetRadio.jsx` and `iosAppBroadsheetMain.jsx` quoting the prompt by a string that is now in **0 of 13** catalogs. Both name it **by key** now. *A comment that pins a spelling pins whatever that spelling is wrong about, and this copy has already been retired once.* `iosAppBroadsheetClient.jsx`'s *"stays put while you move between tabs"* is about tab state and is left alone.
- ⚠ **AND #2113 LANDED ON `main` WHILE THIS SAT UNMERGED, ANSWERING TWO OF THE SAME FOUR REPORTS FROM ANOTHER SESSION.** Two sessions, one page, the same owner. It touched the **same eight files**, so this PR went un-mergeable — and that is why no CI ran on it: **GitHub creates no workflow runs for a PR whose merge commit it cannot compute**, so a conflicted PR is indistinguishable from one whose checks have not started. This file already records that trap by name, and the tell was that `ci.yml` has **no `paths` filter**, so an absence of runs was never a skip. *Measure the mergeability before reading an empty check list as "still queued".*
- ⚠ **THE RECONCILIATION WAS READ RATHER THAN TAKEN A SIDE ON, AND EACH HALF WENT A DIFFERENT WAY.** Measured against `main` rather than assumed: the Nora import map there was **still 0.169.0 / 3.1.6**, `WALL_MASK_MIN_COLS` **still 44**, and the draw loop still `mask[(r*mw+c)*4] > 128` — so three of the four fixes here were untouched by #2113 and are carried through. The **IN THE APP rewording was not**: #2113 deleted the section, so the rewrite is gone, as the bullet above records.
- ⚠ **THE TWO WORDMARK FIXES TURNED OUT ORTHOGONAL, WHICH IS WHY BOTH ARE KEPT.** `wallWordFit` decides how big the word is **in tile units**; the supersampling decides how accurately a tile's coverage is **measured**. Neither subsumes the other. The units are the one thing they had to agree about: `wallWordFit` returns cells and columns, and on `main`'s one-pixel-per-tile canvas those coincide with px so nothing is scaled — here a column is `MASK_SS` px, so every returned value is scaled on the way into the context. **Miss one and the word draws an eighth of its budget wide, which still RENDERS** and is simply the wrong size at every width, i.e. the defect both changes exist to remove.
- ⚠ **AND THE MERGE SURFACED A RELATIONSHIP NEITHER CHANGE COULD STATE ALONE.** #2113's cap only binds once `cols * 0.88` exceeds 70, i.e. from **80 columns** — and the long-form gate is also **80**. So the two rules partition the range cleanly: below 80 the word shrinks with the fold and is the **short** form; at 80 and above it is the **long** form at a constant capped size, and **no width is ever offered a long form that still shrinks**. That also settles whether the gate survived a fit function that no longer exists: **below the bite point `wallWordFit` IS the pre-#2113 fit by arithmetic** (`min(MAX, c*FRAC)` is `c*FRAC` while the cap does not bind), so the 768px and 1024px renders it was chosen from were never in the range #2113 changed. ⚠ **This does retire one figure above: the "15 at 1920" and the cap "at 17 past ~2240" describe the OLD fit** — the word is constant across the desktop range now. The measurements below the gate stand; the ones above it are #2113's to state.
- **Both facts are asserted rather than restated**, derived from the constants, so moving either one fails instead of quietly opening a band that gets neither rule — and `radioField.mjs`'s cache key is re-hashed **last**, as #2113's own guard instructs, because a module edited without a re-hash pairs the new instrument with a browser's **cached copy of the old module**. Merged tree: `npm test` **3951/3951**, zero conflict markers, both resolved modules parse.

### 2026-09-16 — About becomes The Note on both surfaces, and the review file describing three options nobody built

- **The owner's pick off the concept board's SECOND round, built.** Owner: *"do the about the note design - remove my picture from the about pages on both website and app"*. #2111 → `b03becab5`. **No migration, no route, no data change.**
- ⚠ **THE PICK IS NOT IN THE REVIEW FILE, AND THAT FILE SAID THE BOARD HAD EIGHT TABS.** [`REVIEW-2026-09-15-about-and-kitchen.md`](REVIEW-2026-09-15-about-and-kitchen.md) §3 describes **A1 · The Brief · A2 · The Dossier · A3 · The Chapters** and recommends A2 — and the owner had read those three, ruled them *"too much going on, too much wording"*, and been shown three much shorter options in a second round the file never records. The board has **eleven** tabs. So *"the note design"* resolves to **A5 · The Note**, which the review does not mention at all. **Found by reading the board's own tab list rather than by guessing from the three names**, which is the only reason the right page was built: *"The Brief" is the nearest synonym to "note" among the three, and it is the wrong page.* Corrected at the source — the header, §3, §6 and the build order all carry ⚠ markers, because a review file that still recommends a rejected option is an instruction to build it.
- **What the page is.** An eyebrow, one headline that is the belief the company is built on, three sentences from the letter each on its own line, signed. Then four facts and the doors. **798 words become about 80**: measured at the review's own viewports, the website goes **5.2 screens → 1.6** at 1440×900 and **7.6 → 2.1** at 390×844; the app **4.5 screens → 1**.
- ⚠ **NOTHING APPROVED WAS DELETED — IT MOVED ONE TAP DOWN.** The whole letter, verbatim with its drop cap and both pull-quotes, and the approved bio under its own label sit behind one closed line. That is what lets the note stay at eighty words without the page giving up the copy the owner approved; and it is why the drop-cap guard — the codepoint-safe derivation, the one that catches `charAt(0)` splitting a surrogate pair — still has something to guard.
- ⚠ **NO PORTRAIT ON EITHER SURFACE, ASSERTED AS AN ABSENCE.** `founder.webp` is deleted from both trees and `aboutPage.founderAlt` from all thirteen catalogs, retiring the 2026-08-28 founder-card ruling (#1945). A removal is the easiest change in the world to undo by accident, so `tests/about-note.test.mjs` fails if either surface references the file, renders an `<img>`, reaches for the alt key, or if either asset comes back. The war-room section is **marked retired rather than deleted** — it describes a card that no longer renders, and its treatment and two lessons were paid for.
- ⚠ **THE APP SHIPPED ONE DOOR ON A PREMISE THAT IS FALSE, AND THE CODEX ROUND IS WHAT REFUTED IT.** The note read *"this app has nowhere to send it"* — true only of an IN-APP route, which is not what the door needs: coach signup lives on the website, and this module already opens external web destinations (`bsOpenCheckout` for Stripe, and `BSTermsPage`’s own link to `theshapecommunity.com/terms.html`). **So the board’s second door was dropped for want of a mechanism that was already there**, and a signed-out preview visitor who wanted to become a coach lost the page’s only route to it. Both surfaces carry both doors now. ⚠ The app’s goes through **`bsOpenCheckout` despite the checkout-specific name** — on native that is the Capacitor Browser hop, where a bare `location.href` takes the WebView **itself** to a marketing page with no way back into the app — and an **absolute** URL, because on iOS/Android the WebView’s origin is the Capacitor scheme and `/newdesign/Coaches.html` resolves to nothing there. Both are invisible on the web build, which is where this would be checked by hand.
- ⚠ **AND THE CREDENTIALS FACT CONTRADICTED THE APP’S OWN TERMS — WITH THE REASONING THAT REFUTES IT SITTING IN MY OWN COMMENT ABOVE IT.** It read *"Coach credentials checked"* under a note arguing that `marketplace.jsx` renders its ✓ Verified badge **per coach** (`c.verified &&`), so *"every coach verified"* would be contradicted by the first listing a reader opens. **That is right, and it refutes the weaker blanket claim for exactly the same reason**: a conditional badge IS the evidence that not every coach was checked. `BSTermsPage` clause 04 says so in as many words — *"Unless a coach shows a Verified badge, the credentials on their profile are self-reported and not independently verified by Shape"* — so the About page was contradicting a legal document one screen away, in thirteen languages. Scoped to the badge: **"Verified coaches carry a badge"**. ⚠ **REGISTERED, NOT FIXED, AND IT IS WIDER THAN THIS PAGE:** the site FAQ (`public/newdesign/shared.jsx:17`) still says *"Every coach is vetted … We verify licenses on application and re-check annually"*, which is what my fact leaned on and which contradicts those Terms. It pre-dates this page; the Terms are the operative document, so a NEW claim follows them, and rewriting marketing copy on three other pages inside a redesign PR is not this diff’s to do.
- ⚠ **AND "FREE FOR COACHES" BESIDE "$5 A MONTH FOR MEMBERS" READS AS A PRICE COMPARISON AND STATED THE WRONG HALF OF IT.** Coaches pay a **15% platform fee** on everything clients pay them (`PLATFORM_FEE_RATE`, `src/lib/platform-fee.ts`) — and every other coach-facing surface carries the qualifier this one dropped: `coaches.jsx` renders **"$0 · to join and list"** directly beside **"15% · platform fee, only on what you’re paid"**, and the pricing FAQ says *"No monthly dues, no setup fees."* The decisive detail is that **the qualifier was in the owner’s own ruling and my four-word compression is what lost it**: 2026-09-14 reads *"Its free to **join** for coaches."* Now **"Coaches join free"**, in thirteen languages.
- **The other two facts are unchanged and were right:** the shipped **$5/mo**, and a locale count **asserted against the catalogs that actually ship** rather than typed.
- **i18n: 12 new `aboutPage.*` keys × 13 = 156 values, 23 retired × 13 = 299 deletions**, each authored from **that catalog's own vocabulary** — the founder word out of `founderRole` (de *Gründer* · ru *Основатель* · vi *Người sáng lập*), the luxury sentence out of each locale's own `letter.p2`.
- ⚠ **THE NOTE'S FIRST AND LAST LINES ARE THAT LOCALE'S OWN `letter.p7`, SPLIT ONCE AT AUTHORING TIME.** A runtime split on `"."` is a guess about punctuation in thirteen languages; a split frozen in the catalog is one somebody can read before it ships. A guard requires the two to **rejoin into `letter.p7` exactly** in every locale, so the note and the letter one tap below it can never quote different sentences.
- ⚠ **THE RATCHET IS UNMOVED, AND THAT IS THE CERTIFICATION.** `partStrings` **220** and `noneStrings` **738**, byte-identical to where the Kitchen work left them — a presentation rewrite that swaps keyed strings for keyed strings must move the measurement by **nothing**, and had it hardcoded one word the column would have gone up.
- ⚠ **TWO NUMERIC PINS IN THE EXISTING GUARD WERE CLAIMS ABOUT THE PAGE'S LENGTH, AND A REDESIGN MADE THEM FALSE.** `tests/about-page-i18n.test.mjs` asserted `>= 40` catalog keys and `>= 40` `tr()` calls; both broke on a change that removed copy rather than on anything going wrong. *A number that tracks how long a page happens to be cannot tell a regression from an edit.* Replaced by a **two-way coverage check** — every key the page asks for exists, every key the catalog holds is asked for — which a redesign cannot make stale. Measured 33/33 both ways.
- ⚠ **AND THE HOUSE'S OWN LINE-COMMENT STRIPPER IS A SILENT NO-OP ON A CRLF FILE.** `l.replace(/(^|[^:])\/\/.*$/, '$1')` cannot work there: **`\r` is a line terminator in JS regex**, so `.` cannot cross it and `$` sits past it, and every comment survives. `public/newdesign/about.jsx` is the repo's one CRLF-tracked page — measured while writing the guard, **the portrait ban fired on the comment explaining that the portrait had been removed**. Both About guards import `tests/helpers/strip-comments.mjs` now (its line-oriented `.trim()` handles `\r` correctly), and the swap is replayed as its own mutation. *That file's own header says "import this, never re-derive it"; this is the third time the repo has paid for re-deriving it, and the first time the cost was a CRLF file rather than a swallowed block comment.*
- ⚠ **AND THE STANDARD OVERFLOW PROBE REPORTED ZERO ON A PAGE THAT WAS OVERFLOWING.** JSX strips the newline between two adjacent elements, so rendering the four facts and their separators as inline spans left the strip **one unbreakable 715px line inside a 280px box** at 320px wide. `document.documentElement.scrollWidth - clientWidth` read **0**; `document.body.scrollWidth` (**735**) and the strip's own `scrollWidth` are what saw it. Fixed to flex-wrap on **both** surfaces, guarded from source on both, and the guard's window scoped to the map body — a first cut took a fixed 700 characters, ran into the doors below (whose button labels are legitimately `nowrap`) and **failed on correct code**.
- **Verified on the MERGED tree** (`b03becab5`), re-derived rather than carried — and the merged tree was first proven **byte-identical** to the reviewed branch tree: `npm test` **3926/3926** (9 new) · `tsc --noEmit` **0** · JSX parse on both modules · the newdesign precompile check **72 pages, 78 shared jsx, 0 errors** · i18n parity **13/13 at 509 keys** · mobile build clean with **every translated value confirmed in the emitted bundle** — the first push's 156 behind a positive control and two **discriminating** negative ones, and the round's 39 (13 locales × the two rewritten facts and the new door) behind three more, the retired values reading **0** · **30/32 mutations killed across two rounds**, each occurrence-counted before the edit and proven to land, the suite's own `# pass`/`# fail` **parsed**, sanity green at both ends, the tree restored in a `finally` **and on a signal**, every file read and written as **bytes** so the CRLF page could not be silently normalised.
- ⚠ **AND TWO NEGATIVE CONTROLS READ "STILL PRESENT" AND WERE THE INSTRUMENT, NOT A FINDING.** `"Join the"` and `"shape"` exist elsewhere in the bundle — `onboarding:login.titleJoin` carries the first — so neither discriminates anything about a retired About key. The two that existed **only** on this page, `idea.coachHead` and `founderAlt`, read **ABSENT**. *A short common string is not a negative control.*
- **Driven in Chromium.** The website at **320 · 360 · 390 · 430 · 720 · 760 · 900 · 1024 · 1280 · 1440**: zero horizontal overflow, zero page errors, zero 4xx, the expander open and closed at every width, the facts strip wrapping **4 → 3 → 2 rows** as the column grows (131 → 106 → 82px, which reconciles exactly against its own 22.8px line box, 2px row gap and 34px of padding and rules), contrast **15.92:1**, 46px doors — and **the fonts proven real rather than the metrics fallback**, the same string measuring **224px at `wdth` 50 against 1192px at 150**, which a fallback cannot do. The app through the real signed-out preview at **320 · 390 · 430 in English, German and Russian**: one screen, **zero controls under 24px**, zero page errors, **zero English leaks** against a 13-value probe, and no founder image in the DOM.
- ⚠ **THE APP'S OWN BACK ROW WAS A 13px TAP TARGET, AND THIS PAGE OWNS IT.** 10px mono in a zero-padding button; padded with a cancelling negative margin to **29px**, so the hit area grows and the glyph does not move — past this repo's documented floor, **WCAG 2.5.8 AA at 24px**, never Apple's 44pt suggestion. The Passport's own panes were padded this way in #2087; this page has its own row and never was.
- ⚠ **AND THE APP HARNESS REPORTED THE PAGE MISSING THREE TIMES BEFORE IT REPORTED ON THE CODE.** Since the Passport (#2085) the Settings root is TILES, so *About Shape* is not on it at all and a two-step walk times out on a page that is one tap away; Playwright's `hasText` tests an element's **whole** text, and every tile is four lines, so `/^About$/` matches nothing; and a Settings **row** is a plain div with a React `onClick`, so a role-based walker finds the tile and misses the row inside it. Then `innerText` returned the **CSS-uppercased** eyebrow, facts and door, so an exact match against the catalog read them as absent — the trap this file recorded on 2026-09-16, paid again the same day.
- ⚠ **A RATIONALE COMMENT WAS DELETED WITH THE CODE IT SAT BESIDE WHILE THE RULE IT STATES STILL BOUND**, caught by reading my own diff. The *"every split-accent slot is authored NON-EMPTY in all thirteen — `returnEmptyString: false` renders the RAW KEY"* note sat above the retired hero; the letter's split slots are still rendered, so it moved to the letter rather than going with the hero.
- ⚠ **REGISTERED, NOT FIXED:** the shared header's nav links (14–16px) and the footer's (16px) on ~70 pages; the shared dark header reading as a grey band on a light page — **measured**, the link colour is `rgba(245,239,225,0.78)`, **identical to the already-shipped light Kitchen page**, so it is the site's light-page treatment rather than this page's; `pageShell.jsx`'s `!important` `h1` rule forcing this headline to **72px between 720 and 1020px**, left alone because overriding a site-wide rule from one page is the local exception that causes drift; and the About pick's sibling — **the Kitchen half of that review is done and the A4/A6 options stay on the board as the record of the round**.
- ⚠ **NO ON-ACCOUNT PASS.** Every reading here is the signed-out preview; no signed-in member has opened either surface.

- ⚠ **THE GUARD WRITTEN FOR THE CREDENTIALS FACT REQUIRED THE OPPOSITE, SO THE CORRECT FIX BROKE IT.** It banned the word *verified* (`doesNotMatch(/\bverified\b/i)`) — which pinned a spelling and forbade the only honest wording there is. Re-anchored on the invariant, with the **Terms clause as its guard-the-guard**: if that sentence ever goes, the claim has to be re-derived rather than left standing. Per locale it checks the fact against **`profile:coach.verified`** — a word the PRODUCT owns rather than one the test types — by longest common substring, which survives inflection where an `includes()` check fails correct translations (uk *Підтверджені* against the badge’s *Підтверджено*). Measured **7–13 characters against a floor of 6** in all thirteen. *A guard that pins a spelling pins whatever that spelling is wrong about* — this file’s most-repeated sentence, paid again in a guard written to enforce it.
- ⚠ **AND THAT FLOOR IS PROVEN LOAD-BEARING BY A COMBINED MUTATION RATHER THAN ITS OWN DELETION.** Dropping it from 6 to 1 alone is a **no-op** — every shipped locale clears it by margin, so the mutation survives and says nothing. What proves it is **floor 1 AND `ru`’s fact rewritten without the badge word**, which goes green where floor 6 catches it. *A floor is proven by a read that has stopped matching, never by its own deletion.*
- ⚠ **AND THE RESIDUAL IS NAMED IN THE TEST RATHER THAN LEFT UNSAID.** The derivation proves the fact is **about** verification-by-badge; it cannot prove the claim is **scoped**. Measured with the exact value the round quoted — the retired fr *"Diplômes des coachs vérifiés"*, a universal claim carrying the same word, **survives it** — which is why that string is written at the site as the known hole. Across thirteen languages "is this claim scoped" is not a textual question, and a guard that pretended otherwise would be a spelling pin dressed as a measurement.
- ⚠ **AND MY OWN NEW GUARD BLAMED THE COACHES PAGE FOR ITS OWN DEFECT.** The fee guard-the-guard matched `/\$0\b[^"]*to join/` — and the two halves are **separate array elements** (`["$0", false, "to join and list"]`), so the character class could not cross the quotes between them. It failed on correct code with the message *"the Coaches page no longer states what is free"* about a page that says it in the very next literal. *An instrument that reports a failure is as broken as one that reports a pass.* A second hollow spot went with it: the door-label comparison carried a `|| 'Become a coach'` fallback, so a regex that stopped matching would have compared the catalog against the test’s own typed copy and passed forever.
- **The doors are driven, and the wrap turns out to be load-bearing rather than decorative.** In Chromium at **320 · 390 · 430** in English and German: the two doors take **two rows at 320** (y 747 / 796) and **one row at 390+ with 18px clear** — without `flexWrap` that row is **354px of buttons in a 284px column**. Zero horizontal overflow, zero page errors, zero controls under 24px, zero English leaks in German. The website re-driven at six widths from 320 to 1440: zero overflow, zero errors, both doors resolving to pages that exist, the facts strip wrapping **4 rows narrow / 2 wide**.
- **Review:** Codex, **one front-loaded round** as the standing rule requires — **3 findings on `501496b25`, every one real, every one fixed**, each verified against the source before acting, each replied to on its own thread and resolved, and **each replayed as its own mutation** so the suite is proven to catch it rather than merely to be green after the fix. Per the one-round rule the fix head was **not** re-triggered, so that diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not run** (owner’s standing ruling). The merge gate was the four required checks green on the final head and not a draft.
- ⚠ **AND MY OWN READ OF THE FIX DIFF FOUND A FALSE PREMISE IN THE COMMENT ABOVE THE NEW DOORS — ONE THIS REPO HAD ALREADY PAID FOR.** It claimed *"About is client-only, so the client shell is the one host"*, carried forward from the one-door version. **About rides the SHARED `BSSettings`, which both coach shells embed** — and `iosAppBroadsheetPros.jsx:1356` records a shipped bug from believing exactly that: **#1795, where the coach-side door was a dead tap** until the pros shell grew its own `shape:goCommunity` listener. So the premise had already cost a dead control once, in the same spot, and I re-asserted it. Nothing about the fix changes — both shells carry the listener now, and **the coach door depends on no listener at all**, which is one thing a URL opener buys over an event — but a because-clause is a claim, and this one was checkable in one grep. Corrected at the source, comment-only, and worth the CI re-run.
- ⚠ **AND A COACH SEES "BECOME A COACH", WHICH IS A DECISION RATHER THAN AN OVERSIGHT.** The website shows the same door to everyone including signed-in coaches; this is the company’s story page rather than a personalised surface, and the parity guard exists to keep the two saying the same thing. Gating it on a role needs a signal this component does not take, and **getting that gate backwards hides the door from the one audience it is for** — which is the defect the round was opened to fix. Stated at the site so the next reader does not re-open it.

### 2026-09-16 — Shape Kitchen becomes the Menu on both surfaces, and a board that turned over at 1 a.m. all summer

- **The owner's pick off [`REVIEW-2026-09-15-about-and-kitchen.md`](REVIEW-2026-09-15-about-and-kitchen.md) §K3, built.** Owner: *"build the new kitchen page"* → *"for website and aopp"* → *"app"* → *"also need to create a link for it on nav bar on website"*. #2109 → `72deea3`. **No migration, no route, no data change.** ⚠ **The About pick is STILL OPEN and was not built** — that is a separate ruling the owner has not made.
- **What it is.** The recipe page stops being a filter drawer over a grid and becomes a **menu**: four courses by cooking time (under 15 · 15–30 · 30–60 · over an hour), a row per dish carrying its own credit and readings, a **board of three for today**, and a sticky course jump. `recipesPage.jsx` renders it on the website and `RecipeDetail` takes its palette; `BSRecipeBox` is rewritten against the same rules in the app, keyed in 13 locales.
- ⚠ **THE RULES ARE SHARED RATHER THAN DESCRIBED TWICE, AND THE GUARD EXECUTES BOTH COPIES.** The course a recipe lands in, whether a course pages, and which three dishes the board picks are all decided by the catalog's own `time` string — a surface that read it differently would put the same dish in two places. `tests/recipe-menu-parity.test.mjs` **lifts and runs** both implementations' declarations over every recipe, every course, every paging decision and the board across 366 days; a textual comparison could not have told an equivalent rewrite from a real divergence.
- ⚠ **THE ROW AND THE READINGS ARE CALLED CLOSURES, NOT COMPONENTS, AND THAT IS FORCED BY THE HARNESS.** `drive()` renders ONE component deep, so a `<Row/>` would have made every title, byline, certification and `Send to grocery` **invisible** to every mount assertion — the guards would have passed on a surface rendering nothing. The allergen eyebrow is a **JS template rather than ICU** for the same reason: the harness `tr` returns `defaultValue` **without interpolating**, so an ICU placeholder there is a guard asserting on a literal `{…}`.
- ⚠ **THE APP'S STICKY COURSE ROW MEASURES ITS OFFSET OFF THE CHROME, NEVER A LITERAL.** `BSPage`'s pinned masthead **condenses** as you scroll, so a typed number is right at one scroll position and wrong at every other. The masthead carries a `data-bs-pinned-mast` handle (an 8-line diff) and the row reads its height; measured `jumpTop 58 = mastBottom 58` at 320 / 390 / 430, and the parity guard pins the anchor so the two cannot drift apart.
- ⚠ **THE NAV'S EIGHTH TAB NEEDED THE HEADER TO TIGHTEN *ABOVE* THE COLLAPSE, NOT AT IT — AND THAT IS THE BLOCK SPLIT #2103's OWN NOTE SAID WOULD HAVE TO HAPPEN FIRST.** Shape Kitchen was footer-only, the standing `Client.html` had before the Members tab. Measured naturally on `Pricing.html`: the eight-link row wants **508px at 1021** and the flex row gave it **493**, so **`About` was CUT OFF at every width from 1044 down to the 1020 collapse** — silently, with no scrollbar and no error. Freeing 16px of side padding and 8px of each gap clears it. **The breakpoint is NOT moved**: taking 40px of desktop away would also split this bar from the homepage's, whose own row measures clean to 1021 with the same eight links.
- ⚠ **AND THE SWEEP THAT PROVED IT CARRIES ITS OWN CONTROL, BECAUSE A PROBE THAT REPORTS ZERO PROVES NOTHING.** Driven in Chromium at nine widths from 1440 to 1021: shipped, `clip = 0` everywhere with 18px clear of the auth cluster at the narrowest. With the trim reverted, **3px clipped at 1044, 5px at 1040, 15px at 1021** — the same range, from the other side.
- **THE CODEX ROUND ON `61b0180` RETURNED TWO P2s, BOTH REAL, AND NEITHER WAS IN THE SUMMARY COMMENT.** The verdict row read *Completed* with no findings beside it while both sat in the **submitted review** — the reading trap this file already records, paid for again. *A verdict is in the review, not the summary.*
- ⚠ **P1-IN-EFFECT — THE BOARD TURNED OVER AT 1 A.M. FOR EIGHT MONTHS OF THE YEAR, ON BOTH SURFACES.** `kmDaySeed` computed `(d − local Jan 1) / 86400000`, which measures **elapsed time**; a DST day is not 24 hours, so the quotient runs an hour short. **Reproduced rather than reasoned about, and it is wider than the finding stated**: not the transition day, but **every day from spring-forward to fall-back**, for the whole of local `00:00–00:59` — and it goes both ways, with `Australia/Sydney` reading the **next** day's seed at 23:00 all southern winter. Fixed by mapping the local Y/M/D onto `Date.UTC`, an ordinal in which every day is 24 hours **by construction** — the same fix this repo already made for the progress page's calendar-day window. Swept after: **504 instants across a full year in seven zones, zero mismatches**, `Pacific/Chatham` (+12:45, with DST) included.
- ⚠ **BOTH SURFACES CARRIED IT IDENTICALLY, WHICH IS EXACTLY WHY THE PARITY GUARD COULD NOT SEE IT.** A guard that only asks whether two implementations agree is blind to a defect they share; the assertion that catches it is the one comparing the seed against the **local calendar ordinal**, which is what it claims to be.
- ⚠ **AND THE FIXTURES THAT SEE IT ARE MEASURED TO BE LOAD-BEARING RATHER THAN ASSUMED.** Driven per (zone, instant) against the defect: of the six original fixtures **exactly ONE pair caught it** — `2026-06-15T23:59` in Sydney, **by accident** — and the **northern direction, where most DST-observing readers are, was caught by nothing at all**. The added `00:30` samples are what do the work, behind a second guard-the-guard: at least one sampled instant must sit at an offset other than its zone's January offset, or the whole assertion is vacuous.
- ⚠ **P2 — THE PARITY GUARD COMPARED PAGING BOOLEANS AND NEVER THE ROW ORDER, AND THE MUTATION IS WHAT SETTLED IT.** Reversing the app's tie-break **for sourced recipes only** leaves course membership identical, leaves every `paged(n)` identical, and never reaches the board (which filters to authored dishes first): measured, **the whole file went GREEN while the two surfaces listed different rows above "Show all"**. What a reader sees is the ORDER, so the sorted title array of every course is what has to agree now. It is the defect I had asked the round to attack, and it was found.
- ⚠ **A FIRST-PAGE SLICE COMPARISON WAS WRITTEN, MEASURED AND DELETED.** The finding asked for one; if the whole course arrays are `deepEqual` then **every slice of them is**, so it cannot fail where the full comparison passes — its paired mutation was **killed by the full comparison alone**. Removed with the reasoning at the site rather than kept: *a guard that cannot fire reads as a safety net to the next person and is holding nothing.* **A suggestion is a claim, and the honest form of it is the one you just ran.**
- **i18n:** 40 new `kitchen.*` keys × 13 locales — **520 values** — each **authored from that catalog's own existing vocabulary** rather than translated fresh, and each a **byte-preserving append** that keeps the authored key order (41 insertions / 1 deletion per file; the catalogs are **not sorted**, so a rewrite-and-sort showed **264 insertions for 40 keys** and was reverted). Validated before any catalog was touched: placeholder sets identical to `en`, no straight apostrophe in an ICU string, and plural category sets taken from a key **each locale's own translators already wrote** rather than from a table in the script. ru/uk carry the full one/few/many/other (`100 рецептов` renders the genitive plural); tr and vi keep the noun singular after a numeral, which is right for both.
- ⚠ **THE RATCHET IS THE CERTIFICATION, AND IT MOVED BY EXACTLY THE 19 STRINGS THE OLD COMPONENT ACCOUNTED FOR.** `noneStrings` **757 → 738** and `none.length` **91 → 90**, with `partStrings` and `part.length` **unmoved**. A whole-surface rewrite that hardcoded one word would have moved a column it did not.
- ⚠ **AND THE PR BODY AND MY OWN COMMIT MESSAGE BOTH CITED A REVIEW DOC THAT DOES NOT EXIST — `REVIEW-2026-09-16-about-and-kitchen.md`, where the file is dated `-15-`.** Corrected here rather than only in my head, because a squash-merge commit can no longer be edited and this file is what the next session auto-loads. It changes no conclusion; it is exactly the class this log post-mortems repeatedly — *a `path` nobody re-resolves is a claim, not a reference* — and the tell was cheap: one `ls`.
- **Verified on the MERGED tree** (`72deea3`), re-derived rather than carried — and the merged tree was first proven **byte-identical** to the reviewed branch tree: `npm test` **3917/3917** · `tsc --noEmit` **0** · JSX parse on all five changed modules · the newdesign precompile check **72 pages, 78 shared jsx, 0 errors** · mobile build clean with `public/m` in sync · **every one of the 13 locales' 40 values confirmed in the emitted bundle, derived from the catalogs rather than spot-checked and behind a negative control** (a value nobody wrote reads absent, so the probe discriminates) · **4/4 code mutations killed**, each proven to land, sanity green at both ends, the tree restored in a handler **and on a signal** — **both findings replayed as their own mutations**, so the suite is proven to catch them rather than merely to be green after the fix.
- **Driven in Chromium at 320 · 390 · 430:** `The menu.`, four courses **14 / 39 / 28 / 19**, 50 rows, three `SHOW ALL` doors, the sticky row landing exactly under the condensing masthead — **zero horizontal overflow, zero controls under 24px, zero page errors**. Both surfaces produce the same courses, the same Vegan **0/7/6/4**, the same +Gluten-free **0/5/5/2**, the same 11 rows for `paprika` and the same `Show all 39` → 50→77.
- **And in Russian, because a layout pass is not a copy pass:** `Меню.` · `До 15 минут` / `От 15 до 30 минут` / `От 30 до 60 минут` / `Больше часа` · `100 рецептов` · `ДОСКА НА СЕГОДНЯ` / `Завтра другая` · `В СПИСОК ПОКУПОК →` — **zero English leaks** against a 16-string probe, zero page errors, zero overflow.
- ⚠ **TWO HARNESS LESSONS, BOTH OF WHICH REPORTED THE FEATURE BROKEN BEFORE THEY REPORTED ON IT.** `innerText` returns CSS-**transformed** text, so a lowercase probe against an uppercased row reads the whole page as absent — every text probe here is case-insensitive. And the **demo banner sits OVER the tab bar and swallows its clicks**: the Russian run reported `tab-eat` as unreachable (`bs-hide-scroll intercepts pointer events`) until the banner was dismissed by **that locale's own `onboarding:preview.dismiss` value** rather than by an English word. *An instrument carrying one locale's vocabulary into another measures the vocabulary.*
- ⚠ **REGISTERED, NOT FIXED — AND CHECKED AGAINST `origin/main` RATHER THAN ASSUMED.** The diet / protein / free-from chips still render the catalog's **raw English tokens** (`VEGAN`, `GLUTEN-FREE`…). The old component rendered the same `label={d}` straight off `RECIPE_DIETS`, so it is **pre-existing**; localizing it means a display mapping over a table that `recipeMatchesDiet` and `recipeNeeds` **compare against**, which reaches the data layer and the website's parity copy too — its own change, not a side effect of a presentation PR. Also pre-existing and outside this component: the Eat page's own top tabs, the masthead kickers, and the 100 catalog recipe titles and blurbs.
- ⚠ **NO ON-ACCOUNT PASS.** Every reading here is the **signed-out preview** against a static server; no signed-in member has opened either surface.

### 2026-09-16 — The multi-dish session was reachable from two rows on one page, and "Cook this" was not one of them

- **Owner, on the cooking tutorials: *"i am supposed to be able to cook multiple recipes at once … i am not able to select more than one recipe to run simultaneously … Its not clear how I can select multiple recipes"*.** #2106 → `ad75715`. Mobile only; **no migration, no route, no data change.**
- ⚠ **THE PICKER WAS ALWAYS MULTI-SELECT. NOTHING LED TO IT.** `BSPrepSession`'s picker has been a checkbox per row over a `sel` map since PR C, and the three-mode ask the owner describes is `BS_COOK_CHOICE` on the mise. It was reachable from **exactly two** affordances — the `Prep the week` row on the menu and the same row on the shop list — while all three **`Cook this`** doors (`cook:cta`, on the Shape Kitchen recipe, the plan meal and a member's own recipe) went straight to single-dish `BSCookMode`. **Measured before a line was written**, not inferred from the report: the report was exact, because from where the owner was standing there was no route and no sign one existed. *An engine nothing leads to is indistinguishable from a missing feature.*
- **`＋ Cook with something else` now sits beside every `Cook this`**, opening the session **seeded** with that dish, already ticked, every other candidate one tap away. `Cook this` is **unchanged** — one dish still goes straight to the walkthrough; routing it through a picker to confirm a choice already made would tax every single-dish cook to serve the rarer one.
- ⚠ **THE DOOR FIRES AN EVENT BECAUSE ITS THREE HOMES ARE THREE DIFFERENT TREES.** The Shape Kitchen recipe lives inside `BSClientEat`, the meal preview inside **both** `BSClientHome` and `BSClientEat`, and a member's own recipe inside the Catalogue on another tab — so there is no common parent to thread a prop through. `shape:cookWith` is the mechanism the shell already routes for `shape:openMarket` and `shape:goWall`. ⚠ **And it carries the COOKABLE, not a lookup key**: two of the three are dishes the Eat tab cannot resolve on its own (a member recipe lives in their `client_recipes` document; a meal opened from Home belongs to that day's program), so a key would arrive with nothing to resolve it against and the session would open **empty** — the defect this door exists to fix, in a new coat.
- ⚠ **THE SEED'S KEY IS STABLE FROM THE FIRST RENDER, WHICH IS THE WHOLE REASON THE TICK NEEDS NO EFFECT.** The member's own recipes hydrate asynchronously (`myDoc`), so a key resolved **against** the candidate list would change under a tick already made and the selection would be silently dropped. The seed keeps `BS_PREP_SEED_KEY` and **the duplicate is removed from the base list instead** — one row either way, no migration, no race.
- ⚠ **AND THE DEDUPE IS NOT COSMETIC: `bsMergeMise` MERGES BY INGREDIENT.** Two rows for one dish buys and preps everything twice and has the orchestrator schedule the dish against itself.
- **THE CODEX ROUND ON `85f7ac4` RETURNED THREE P2s, EVERY ONE REAL, EVERY ONE VERIFIED AT THE SOURCE BEFORE ACTING.**
- ⚠ **AN IDENTITY ON BOTH SIDES DECIDES, IN BOTH DIRECTIONS — the title arm was overriding one.** A seeded member recipe carrying `mealId` and a program meal carrying a **different** one, sharing a title, had the program meal removed: both identities present, both unequal, so provably different dishes, and the member lost a dish from the picker they can see is not the one they arrived with. The title is a fallback for where an identity is **missing**, never a tiebreak against one that is there — **which is the rule this file already applies** when it refuses to resolve a `myrecipe:` pointer by exact title, for the reason Codex cited back at it: a member's own recipe may be called anything, so such a match is a **claim** about identity rather than proof of it.
- ⚠ **THE REPLACEMENT DISCARDED THE MATCHED ROW'S DAY, AND THE ROW IT REPLACED WAS THE ONE THAT KNEW.** `writeEntry` stamps `dayIdx: it.dayIdx, slot: it.slot` onto the prep record and the wrap reads it straight back (`BS_PREP_DAY_LABELS[e.dayIdx]`), so a seed that **correctly** replaced Tuesday's dinner recorded both as `undefined`: the session reported **no day covered at all**, and `bsPrepMatch` had nothing to match the Prepped ✓ stamp against on the menu. ⚠ The **group deliberately stays `Starting with`** rather than the matched row's day — putting the arrived-with dish at the top is the point of seeding, and that is presentation, not the record.
- ⚠ **THE FRAMING READ THE PROP, NOT THE SELECTION.** The seeded row is an ordinary toggle, so a member could untick the dish they arrived with, tick one other and continue — while `cookNow` stayed true and headed a **one-dish** session `Cook together`, asking *"What else is cooking?"* about a dish that was not going to be cooked. Codex offered two remedies and **the other one was declined with a reason**: making the seed mandatory means a row that refuses to untick, which is a worse defect than the one being fixed. Dropping the arrived-with dish drops the claim with it.
- **The session names the door it was opened by, and that is the honest-data rule applied to a frame rather than a figure.** Entered from the menu it IS the Sunday ritual and keeps every word; entered from a recipe it is two dishes now, where a finish reading *"The week is set."* after one dinner describes something that did not happen. ⚠ **The MISE title is deliberately shared** — *"One board, everything."* is true of both, and a second key for one screen is thirteen more values a translator has to keep in step for no gain.
- ⚠ **THE PICKER NAMES THE TIMING CHOICE WHERE THE DECISION IT DEPENDS ON IS MADE, AND QUOTES NO FIGURES DOING IT.** A member with one dish ticked had no way to know that ticking a second unlocked anything — the reported defect in one sentence. At one dish it says what a second buys; at two it says what is coming. The **ask itself stays on the mise**, because the three options can only be **costed** once the kitchen is known, and a row quoting minutes it had not computed would advertise a schedule it does not run. Asserted as an **absence** so a later "helpful" addition fails.
- **i18n:** 8 new `cook:*` keys × 13 locales, each composed from **that catalog's own** existing vocabulary rather than translated fresh (de *Zusammen kochen* off its `prep.soonest`; fr *Cuisiner avec autre chose* off its own `cta`; vi *Bắt đầu với*). Pure appends — **9 insertions / 1 deletion per file**, parity clean at **129 keys**.
- ⚠ **ONE MUTATION SURVIVED THE FIRST ROUND AND IT WAS A REAL GAP IN MY OWN GUARD.** The opener assertion pinned `/setPrepSeed\(s\)/` — which `if (s) setPrepSeed(s)`, **the defect itself**, also satisfies: the week's door would then keep whatever the last "Cook with something else" left behind. It **drives the shipped opener** now (lifted out of the source, run with no argument) rather than matching its text. *A guard that pins a spelling pins whatever that spelling is wrong about* — this file's most-repeated sentence, paid for again in a guard written to enforce it.
- ⚠ **AND THREE MUTATIONS WENT STALE ACROSS THE FIX ROUND — REPORTED AS SKIPS, NOT SILENTLY RELOCATED.** Rewriting the candidates memo moved their anchors, and the runner said `anchor occurs 0 times` rather than moving the edit to a nearby match. That is the right failure: *a runner that quietly relocates an edit reports a kill for a mutation that never landed.* Each was re-anchored and re-run.
- ⚠ **A BROKEN INSTRUMENT IN MY OWN TEST, EXPOSED BY THE THIRD FINDING'S GUARD.** `s.click` matches text **starting with** the label, and a ticked row's text starts with its `✓` glyph — so a title could **select** a row but never **deselect** it. The untick guard would have passed while never exercising the untick at all. It toggles by content now.
- **Verified on the MERGED tree** (`ad75715`), re-derived rather than carried — and the merged tree was first proven **byte-identical** to the reviewed branch tree: `npm test` **3855/3855** (16 new guards) · `tsc --noEmit` **0** · JSX parse · mobile build clean with all 8 keys confirmed **in the emitted bundle** at 14 occurrences each behind a positive control (`prep.soonestSub`, also 14) **and a negative one** (the renamed-away `prep.cookingNow` reads **0**) · i18n parity **13/13 at 129 keys** · **19/19 mutations killed, 0 survived, 0 skipped** across two rounds, each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal**, every file read and written as **bytes** — **all three Codex findings replayed as their own mutations**, with the day and the slot split into two because one mutation dropping both would be killed by whichever assertion fired first and say nothing about the other.
- **Driven in Chromium at 390×844** through the real signed-out preview, on the rebuilt bundle: the recipe detail carries **both** doors; the seeded session opens `COOK TOGETHER` / *What else is cooking?* / `STARTING WITH` with **1 of 31 ticked** and **no** *Prep the week* anywhere; ticking a second retires the nudge and names the choice; the mise costs all three modes — **33 min together against 48 separately**; **Close returns to the recipe** it was opened from and re-opening re-seeds. **Zero horizontal overflow, zero page errors.**
- ⚠ **THE HARNESS REPORTED THE APP BROKEN TWICE BEFORE IT REPORTED ON THE CODE.** The build is based at `/m/`, and served at root every module 404'd to `index.html` — **a MIME-type error that reads exactly like a broken bundle**; and the icon-only footer defeated a label-driven walker, so the tab bar had to be found by **geometry** (the row of buttons sharing one y inside the viewport). ⚠ A `pkill -f "serve …"` also **matched its own invoking shell** and killed the command (exit 144) — the self-match this file already records for `http.server` and `pgrep`, in a fourth costume.
- ⚠ **AND BOTH MERGE COMMITS' FOOTERS ARE MALFORMED — #2106's AND THIS ENTRY'S OWN (#2107) — WHICH IS MINE TWICE.** Each trailing `Co-Authored-By` line carries HTML entities (`&lt;`/`&gt;`) instead of angle brackets, and #2106's also carries a stray `</commit_message></invoke>` fragment from the tool call that wrote it. ⚠ **BOTH ARE MINE, AND THE FIRST DIAGNOSIS OF THEM WAS WRONG TOO.** I recorded the first as *"the tool escaping my input"* — it was not: the stray `</commit_message></invoke>` fragment could only have come from my own call body, so that call carried **pre-escaped brackets AND a malformed tail**, typed by me. The second then **copied the escaped form forward** out of the broken output. *A broken output you copy is a defect you have adopted* — and blaming the tool for it is how the adoption goes unnoticed. Corrected before the correction itself shipped, which is the only reason it was caught: a cause assigned to an instrument is a cause nobody re-derives. Neither changes anything about the code and **a force-push to `main` to tidy a footer is far worse than the footer**, so both are recorded rather than repaired. ⚠ This bullet originally said *"the merge commit"*, singular, and was made false within the hour by the very commit that shipped it — corrected here rather than left, because this file is auto-loaded and an understated count reads as the whole truth. *A merge commit is the one message you cannot correct, so it is the one worth reading twice before sending.*
- ⚠ **REGISTERED, NOT FIXED:** adding a dish **from inside** the cooking tutorial (`BSCookMode` assumes one cookable throughout, so that is a different change, not a bigger version of this one); the candidate `mine` field is carried and **read by nothing**, which mirrors the existing member-recipe candidate shape rather than adding new dead code; and there is **no on-account pass** — every reading here is the signed-out preview over the demo program, so no real member with their own saved recipes has opened the seeded picker.

### 2026-09-15 — Radio leaves the desktop nav row, and a backtick in a CSS comment that every gate but the browser waved through

- **Owner, on a screenshot of the site header: *"remove the radio tab from main middle dashboard and leave the existing radio tab next to get started. dont need 2 tabs on dash"*** — then, mid-build, *"this on the website"* and *"desktop"*. The bar carried Radio **twice**: as a middle-row link and as the bordered **▸◂ RADIO** pill in the auth cluster beside Get started. The pill is the one that stayed, on both bars, so the row goes **eight links to seven** — Coaches ▾ · Members · App · Community · Rewards · Pricing · About. #2103 → `be5612459`. `public/newdesign/` only; **no migration, no route, no mobile change.**
- ⚠ **THE DUPLICATION ONLY EVER EXISTED AT ≥1021px, WHICH IS WHY BOTH DRAWERS KEEP THEIR RADIO LINK.** At ≤1020 the collapse rule hides the **whole auth cluster** (`.shape-nav-auth`, and `.nauth .nradio` on the homepage), so the pill is not on screen and the drawer is the only nav a phone has. Dropping Radio there would not have removed a second tab — it would have removed the **only route to the page from every phone**, on all ~70 pages the shared header renders. The owner's *"desktop"* confirms the reading rather than being assumed into it.
- **The shared drawer renders from the same table the bar does**, so removing Radio from `SHAPE_NAV_GROUPS` took it out of the drawer too; it is rendered **explicitly** there now, pointed at **the pill's own href** rather than a second spelling of the same page, so the two cannot drift. Both drawers put it **after** the tabs, because it is no longer one of them.
- ⚠ **AND IT IS UNCONDITIONAL, LIKE THE PILL.** `PORTAL_NAV` has never carried Radio, so a **signed-in** phone had none at any narrow width — a gap rather than a design, since the bar renders that pill in both auth branches. Widened by one line, deliberately, and said so at the site.
- ⚠ **THE TWO BREAKPOINT COMMENTS JUSTIFIED 1020 BY AN EIGHTH LINK THAT HAS NOW LEFT, so they were re-measured rather than left describing a bar that no longer exists.** Driven in Chromium **in the real faces**, with the collapse rule overridden so each width reports whether the row **would** have fitted — the natural render says nothing below the breakpoint, where the row is `display:none`. The homepage bar **fits at 960** and puts **10 · 30 · 50 · 70px** on the page at 940 · 920 · 900 · 880; the shared header **fits to 920** with 3px clear and overlaps its auth cluster by **1px at 900**. So the binding width is **~950** and 1020 is a ceiling with ~70px of slack.
- ⚠ **THE FONT WAS PROVEN LIVE BEFORE ANY WIDTH WAS BELIEVED — 322px at `wdth 50` against 1288 at 150.** `fonts.gstatic.com` is blocked in this container, and *a width measured in the metrics fallback measures the fallback*; `document.fonts.check()` answers **true** for a fallback, so it cannot settle it. The three real woff2 are served locally and the axis is the only discriminator.
- ⚠ **AND THE BREAKPOINT IS DELIBERATELY NOT RE-TIGHTENED, WHICH IS A MEASUREMENT RATHER THAN A SHRUG.** Lowering it would hand a 960–1020 window the row back, which is a real gain and not what was asked for — **and it is not a one-number change**: the same ≤1020 block also drops `.shape-header-inner` to 24px padding / 14px gap, so the row would arrive at those widths with **less** room than the sweep gave it and the block would have to be split first. Left as a measured ceiling; the number's reason is now the measurement rather than the eighth link.
- ⚠ **A BACKTICK IN A COMMENT INSIDE `pageShell`'s `<style>` TEMPLATE IS CODE, AND EVERY GATE BUT THE BROWSER SHIPPED IT.** Quoting a selector the way every other comment in this repo does closed the template early — and the file still **parsed**, `tsc` was **0**, the mobile build was clean and **all ~3,800 tests passed**, because ``…`.shape-header-inner` to 24px`` is legal JavaScript: a member read, then a subtraction chain over the bare identifiers `header` and `inner`. The shared header then threw **`ReferenceError: header is not defined`** at render and **~70 pages lost their chrome**. Only the render pass caught it, and only because the drawer was being checked for something else.
- **The guard is structural and needs no list of forbidden characters**: every `<style>` child expression must be a plain `TemplateLiteral`. Proven **both ways before it was written** — `TemplateLiteral` on the clean file, `BinaryExpression` with the backticks restored — and the shipped defect is replayed as its own mutation, so the suite is proven to catch it rather than merely to be green after the fix.
- ⚠ **REGISTERED, NOT FIXED — THE SHARED HEADER'S MOBILE DRAWER IS CLIPPED TO THE HEADER'S OWN 72px BOX, AND IT PREDATES THIS CHANGE.** `.shape-header` carries `backdrop-filter: blur(20px) saturate(1.05)`, and a backdrop-filter makes an element the **containing block for its `position: fixed` descendants** — so the drawer's `inset: 0` resolves against the header instead of the viewport. Measured at 390×900 **on this branch and on `origin/main` alike**: the dialog's box is **390×72** with its links laid out down to **y 656**, and forcing `backdrop-filter: none` on the header takes it straight to **390×900**. It scrolls, so nothing is lost — but every nav item, **Sign out included**, is read through a 72px slot. The homepage's own drawer is unaffected: `.ndrawer` is a **sibling** of its `<nav>`, not a child. The fix moves the drawer out of `<header>`, which is a change to the chrome of ~70 pages and does not belong in a PR about which links the bar carries.
- ⚠ **AND A CONTROL IS THE ONLY THING THAT MADE THAT A FINDING RATHER THAN AN ACCUSATION.** The first reading — a 72px dialog with links at y 656 — reads exactly like *"the change I just made broke the drawer"*. A worktree on `origin/main`, its own static server on a second port, and the same probe returned the **identical box**, after which the cause was proven by removing one property rather than by reasoning about it.
- **Verified on the merged tree:** `npm test` ****3839/3839**** · `tsc --noEmit` **0** · JSX parse · the newdesign precompile check (**72 pages**, 0 errors) · **12/12 mutations killed**, each proven to land, sanity green at both ends, the tree restored in a `finally` **and on a signal**, read and written as **bytes** — Radio restored to each table · the drawer link dropped, re-pointed and duplicated · each drawer's link deleted · the homepage drawer's ordering reverted · each collapse rule's hide removed · each pill deleted · and the backtick defect replayed · and both bars and both drawers **driven in Chromium at 1440 and 390**: zero page errors, zero horizontal overflow, seven links and the pill at 1440, the pill hidden and Radio in the drawer at 390. ⚠ The suite, `tsc`, the parse and the precompile were **re-derived on the merged tree** rather than carried (the count moved 3832 → 3839 under #2102, which is theirs, not this work's); the mutation round and the browser sweep are the pre-merge figures, and #2102 touches neither the nav nor the header.
- ⚠ **THE MUTATION RUNNER'S OWN "DID IT LAND" CHECK WAS WRONG FOR EVERY INSERTION, AND IT ABORTED THE FIRST ROUND.** It asked whether the anchor had **gone** — but an insertion's replacement **contains its own anchor by construction**, so the very first mutation reported as never applied. It compares the **file** now, not the anchor. *A runner that reports a mutation it did apply as skipped is the same broken instrument as one that reports a kill for a mutation that never landed.* A second non-kill was a correctly **reported SKIP** (`anchor occurs 0 times`) after I moved that anchor mid-round — the right failure, and re-pointed rather than relocated.
- ⚠ **AND I READ A SIX-MINUTE CI RUN AS SIXTEEN, WHICH THIS FILE ALREADY POST-MORTEMS BY NAME.** A `run_in_background` bash `sleep` **returns immediately**, so four "waits" cost nothing and I was re-polling the API within seconds of each check. Caught by printing `date -u` against the checks' own `started_at` rather than by trusting the count. The honest wait is a **foreground until-loop**. *A wait that does not wait is an instrument reporting on itself* — recorded on 2026-09-14, paid for again.
- ⚠ **AND `pkill -f serve-main.mjs` MATCHED ITS OWN INVOKING SHELL AND KILLED THE COMMAND (exit 144)**, taking a `tsc` run with it — the self-match this file records for `http.server` and for `pgrep`, in a third costume.
- **Review:** Codex, **one front-loaded round** as the standing rule requires — **clean on `c8abecdc3e`**, head-pinned by its own *Reviewed commit* line. ⚠ It posts as an **issue comment**, not a review, so `get_reviews` reads empty on a PR it has answered; reading that as silence would have bought a needless second trigger. **CodeRabbit was not triggered** (owner, this session: *"dont run coderabbit for PR"*); its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `c8abecdc3` and not a draft.
- ⚠ **NO ON-ACCOUNT PASS.** Every reading here is the signed-out render against a static server; no signed-in member or coach has opened either bar.
### 2026-09-15 — The bubble's Feed drops a chip row that duplicated its own tabs, and the plate stops diverging from the app

- **Two owner notes on one screenshot of the Feed tab, one PR.** *"dont need client and community tabs here in chat bubble, already have them in chat bubble, repetitive"* and *"also this still not replicating what is on app on wall feed"*. #2102 → `4a80f25`. `public/newdesign/` and two test files; **no migration, no route, no mobile change, no i18n key.**
- ⚠ **THE "REPETITIVE" CLAIM WAS VERIFIED BEFORE TWO OF THREE CONTROLS WERE REMOVED, NOT ASSUMED.** The bubble carries **Team · Clients · Trainers · Nutri · Friends · Channels · Help** as TABS (`clientChatThreads.jsx`, plus the `Feed` tab `chatWidget.jsx` prepends), so the `Client` and `Community` chips duplicated two of them one level down. **The app's Chat has no such tabs, which is why it can afford the chips and this surface cannot** — the divergence is forced by the bubble's own tab set rather than chosen. With two gone the third is a picker with one option, i.e. a label wearing a control, so the row went with them.
- ⚠ **AND THE OTHER READING WOULD HAVE DELETED TWO POSTS FROM THE WEBSITE IN SILENCE.** Pinning the feed to the Wall channel is the literal reading of *"don't need client and community"* — and the two `SHAPE`-channel demo posts are **members' own notes** (*"Down 14 lb and running negative splits…"*, *"Race day Sunday — Brooklyn Half…"*), not Shape announcements, and exist nowhere else here (the Channels tab is a different demo thread set). Measured: the panel renders **8 cards where 6 rendered before**. *When two readings of an instruction differ, the one that loses nothing is the one to ship and write down.*
- **The three rules #2099 shipped as STATED divergences are reversed, and that is what the owner was looking at.** The pill is drawn for **every** card that measured something rather than for records only (an ordinary workout reads `TIME · 52 MIN` again, where it drew none); a PR pill falls back to the **measure** when nothing names the lift, which on a live post is always (`mapPost` sets no `lift`, the same blanking the app writes as `a.real ? '' : a.lift`); and the stat grid is the **whole** set sliced to six — the app's `detailStats` verbatim — where it held `rest.slice(2, 8)`, what the hero and facts had not already said.
- ⚠ **THE GRID ONE IS THE VISIBLE DEFECT IN THE SCREENSHOT.** A demo PR carries exactly **two** stats (Top set + Reps), so the filter left **nothing** and the record was the emptiest card on the board. Each divergence had a *"reverse it by"* note at its site, which is the only reason reversing them was a one-line change each — and each note is now replaced by why it was reversed, so the next reader does not re-derive the "improvement".
- **A demo PR's headline is composed the way the app composes it** — `` `${lift} — new PR` `` (`iosAppBroadsheetClient.jsx:18957`) — rather than left as the bare lift. The app's demo PRs carry no `title` either; it **builds** one. ⚠ **There is deliberately NO per-kind branch left** (a `tier` post falls to `New tier · Tempo`), because the app has none: one rule, every card.
- **THE CODEX ROUND ON `5b5fc1b`: TWO FINDINGS, BOTH REAL, BOTH VERIFIED AT THE SOURCE BEFORE ACTING.**
- ⚠ **P1 — DROPPING THE CHIP ROW DROPPED AN AUDIENCE BOUNDARY WITH IT.** `/api/community/feed` applies **no channel predicate at all** — it filters `privacy`, plus `author_id` in `following` mode, then `LIMIT 50` — so the chips were the **only** thing scoping channels on this surface. And the app says what they mean in its own comment: *"SHAPE = individual members, TRAINER/NUTRI/CLIENT = that role's peers only"*, enforced as `shown = posts.filter(p => p.kind === filter)`. So the removal showed a **CLIENT** the trainer- and nutritionist-**peer** channels for the first time, and let those rows consume the 50-row window ahead of Wall activity. **The control stays gone; the boundary comes back** as `cfFeedScope(role)` — exactly the three channels this member's chips covered, **merged rather than widened**, so nothing they could previously reach is hidden and nothing new is exposed. Measured after: the panel renders the **same 8 cards with the same pills and the same grids**, which is the point.
- ⚠ **P2 — `onRepost` IS THE OTHER CREATE PATH AND SENT NO CHANNEL**, so the app's mapper fell back to the reposter's ROLE and filed it under Client/Trainer/Nutri instead of the Wall — and a reposted card is the one most likely to be a record. **Pre-existing, and this PR owns it:** making `CF_POST_CHANNEL` the file's stated write-side invariant is what turned a gap into a broken invariant.
- ⚠ **THE MUTATION ROUND ON THE FIX FOUND FOUR SURVIVORS AND TWO WERE REAL GAPS IN MY OWN GUARDS.** The scope test drove `cfFeedScope`/`cfChannelOf` in isolation and **never asserted the RENDER goes through them** — so deleting the filter from the component, which is Codex's P1 exactly, left every assertion green. *A rule that is correct and tested while the page bypasses it* is the class this file post-mortems on `bsIbSetRowsFor`, walked into again in the fix for a finding about that very filter. And the create sweep matched `channel: CF_POST_CHANNEL` **anywhere in its window**, which for the composer's create was satisfied by the **optimistic row's own stamp thirteen lines above it** — so deleting `metrics.channel = …` was invisible: it was measuring a different occurrence of the same string. It reads the metrics payload the call actually sends now.
- ⚠ **THE OTHER TWO SURVIVORS WERE VACUITY FLOORS, WHICH ON A HEALTHY CORPUS ARE NO-OPS BY CONSTRUCTION.** Removing a floor changes nothing while the read still matches, so the mutation proves nothing either way. They are replaced by mutations that break the **READ** — the endpoint pattern, the `lift` name, the render anchor — each of which the floor then catches. *A floor is proven by a read that has stopped matching, not by its own deletion.*
- ⚠ **AND ONE ASSERTION OF MINE COULD NEVER HAVE MATCHED, IN THE OBVIOUS WAY.** It looked for the app's *"peers only"* sentence — **a comment** — in a comment-**STRIPPED** copy of the source. It asserts the app's own **enforcement** instead: `CHIP_KEYS = ['COMMUNITY', myRoleChip, 'SHAPE']` offers exactly one role channel, the viewer's, and the feed is filtered to the chip. *A guard aimed at a comment is aimed at the one thing its own reader deletes.*
- ⚠ **AND THE CREATE SWEEP HAD TWO MORE HARNESS DEFECTS, BOTH CAUGHT BY ITS OWN VACUITY FLOOR RATHER THAN BY READING.** It required **double quotes**, and the two creates use different ones (`fetch("…")` for the repost, `fetch('…')` for the composer), so it found **one of two** and would have asserted on a corpus of one; and it required every call to name a `method`, which the feed's own **GET** legitimately does not. It covers all four calls now (GET · 2× POST · PATCH) and asserts it found each kind. ⚠ **The PATCH is deliberately exempt**: an edit targets a post that already has a channel, and stamping one there would rewrite where an existing post lives.
- ⚠ **AND THE RENDER HARNESS REPORTED THE FIX AS NOT LANDED BEFORE IT REPORTED ON THE PAGE.** Its stat-grid probe matched `\d+px \d+px \d+px` against a **computed** `grid-template-columns`, which resolves to **fractional** pixels — so it read `gridRows: 0` on every card, which is exactly what the defect looks like. It counts tracks instead. *A probe that cannot match a legal value reports an absence and it reads as a finding.*
- **Review:** Codex, **one front-loaded round** on `5b5fc1b`, head-pinned by its own *Reviewed commit* line **with the summary row naming the same head**, so no head-pinning trap. **Two findings, both real, both fixed, both threads replied to and resolved.** Per the standing one-round rule it was **not re-triggered** on the fix head, so the `a3bde72` diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not run** (owner's standing ruling); its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `a3bde72` and not a draft.
- **Verified on the MERGED tree** (`4a80f25`), re-derived rather than carried — and the merged tree was first proven **byte-identical** to the reviewed branch tree: `npm test` **3836/3836** · `tsc --noEmit` **0** · JSX parse · the newdesign precompile check **72 pages, 77 shared jsx, 0 errors** · **21/21 mutations killed across two rounds** (12 + 9), each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal**, and every file read and written as **bytes**.
- **Driven in Chromium at 1280 · 1024 · 800**, before and after the fix: no chip row (only `My posts` / `New post` survive), **8 cards**, a pill on every measured card and **none on the two notes**, the two-stat PR's grid back at **2 rows**, a nine-stat run capped at **6** starting at its first stat, `MY POSTS ONLY · 0` still right-aligned with its own empty copy, **zero page errors and zero horizontal overflow** at every width. The channel scope is driven too: a client reaches neither `TRAINER` nor `NUTRI`, by channel or through the author's-role fallback; a trainer reaches their own peers and not the nutritionists'; everyone keeps `COMMUNITY` and `SHAPE`.
- ⚠ **REGISTERED, NOT FIXED:** there is **no on-account pass** — every reading here is the signed-out preview over the demo feed, so the channel scope is argued from the route and the app's own filter and driven against stubs, never against a real member's rows; the honest check is a client signed in with a trainer in their community. The panel's eyebrow still reads **THE WALL** over a feed that no longer filters by chip, and that is **correct rather than an oversight** — the app applies `variant="wall"` to the **entire Feed segment** (`iosAppBroadsheetClient.jsx:21907`), so the chip was never what made it the wall. And `CfPill` still has no caller, which predates this and is documented at its site.

### 2026-09-15 — The chat bubble's feed draws the app's wall plate, and a guard that claimed an invariant it did not enforce

- **Owner, on a screenshot of the bubble's Feed tab: *"this needs to match same look of the wall feed on app"*.** The website drew its own card — a teal-tinted box with the figure set in Fraunces — where the app draws a **record plate**: a heat-tinted record pill, a 5×7 **dot-matrix** hero figure, the two facts that qualify it, a heat rule, the stat grid, the HR zones bar and the trace. #2099 → `665ec66`. `public/newdesign/` only; **no migration, no route, no mobile change.**
- ⚠ **`variant="wall"` IS APPLIED TO EVERY CARD IN THE APP'S FEED SEGMENT, NOT TO THE WALL CHIP ALONE** (`iosAppBroadsheetClient.jsx:21907`), so the plate replaces all six per-kind blocks here rather than one chip's. The six retired renderers are **deleted rather than left reading as live** — four carried unguarded reads (`p.duration.toUpperCase()`, `p.coach.split(" ")`, `p.earned.toLocaleString()`) that throw on a post missing the field, in a directory with **no error boundary**, where one throw blanks the page.
- ⚠ **IT IS A PORT, NOT AN IMPORT, AND THAT IS FORCED RATHER THAN PREFERRED.** `shareCard.mjs` already mirrors one of these helpers (`bsHeroStatIndex`) — and it loads on **3** pages while the bubble mounts on **35**. A plate reaching for `window.ShapeShareCard` would draw a figure on the app shells and **nothing on the other thirty-two, with no error anywhere**. So the helpers are copies, and a copy drifts: `tests/community-wall-plate.test.mjs` compares each against its original rather than pinning our own spelling, with the **glyph bitmaps asserted byte-for-byte against the app's** — *"the same number reads the same on both surfaces"* is a claim about those bitmaps and nothing else.
- ⚠ **THE PLATE'S WEIGHTS ARE FETCHED BY THE MODULE ON FIRST RENDER, NOT BY EDITING 35 HOST PAGES.** Measured: those pages ask for JetBrains Mono at max weight **500, 600 or 700 depending on the page**, so the same record would otherwise be **synthetically emboldened on some hosts and really bold on others**. One lazily-injected stylesheet link beats a 35-file sweep — which this file already records as churn the deploy does for you.
- **Heat is the author's role, which the website never did** — trainer rust, nutritionist gold, else teal, so a coach's record reads as a coach's at a glance. Derived from the app's own expression, which the guard re-reads rather than restates.
- **Three stated divergences from the app, each one line to reverse and each commented at its site:** the pill is a **record** claim, so a post with no record makes none (the app pills every card `measure · value`, which renders `TIME · 52 MIN` directly above a hero reading `TIME / 52 min`); the measure never stands in for the lift, because it is always `heroStat[0]` and therefore the eyebrow two lines below; and the stat grid holds what the hero and facts have **not** already said, which is how `bsShareCardModel` already treats these same rows.
- ⚠ **THE TAB STAYS `Feed`, AND THE OWNER REVERSED ME ON IT.** I renamed it to Wall; the owner clarified the app's structure (*"like under feed, subtab is wall"*) and ruled *"leave it as feed"*. **Measured from the catalog rather than the call site's `defaultValue`** — `feed:tab.feed = "Feed"`, `feed:tab.wall = "Wall"` — so the app's top segment is Feed and the Wall is the chip inside it. Labelling the tab Wall put **two controls reading "Wall" in one panel**, counted in a live render. The guard *derives* the expected label from that catalog now, so the day the app renames its own segment this fails rather than silently disagreeing.
- **TWO CODEX ROUNDS, BOTH AT THE OWNER'S REQUEST, AND EVERY FINDING REAL.**
- **Round 1 on `aab608c` — three findings, each a place the port silently lost data the app itself reads.** **P1:** the app writes `workoutStats` in **two** shapes and `mapPost` read one — `shapeBackend.js:3141` publishes `{ label, value }` while the Log-activity composer (`:14060`) and the Post-a-PR sheet (`:20166`) write `{ l, v }` — so **every row from a post made in the app by hand was dropped**: no hero, no facts, no grid, nothing failing anywhere. **P1:** a member's **first** record carries `metrics.pr: true` and **no delta** (`:20223`), because a delta exists only against a prior best; my port decided the pill on the delta alone, so a genuine first record drew as an ordinary load — the defect the app had already fixed for itself, re-introduced in the copy of it. **P2:** an incomplete zone distribution published a missing zone as a **measured 0%**, and because these are percentages of a sum, zero-filling one **inflates every other segment**.
- ⚠ **AND MY OWN FIXTURE HAD ALREADY HIT THE FIRST ONE AND I FIXED THE FIXTURE.** The first live render came back with no hero; I "corrected" the payload to `{ label, value }` because that is what `mapPost` **read** — matching the consumer instead of checking the producer, when the consumer was the thing that was wrong. *A fixture that agrees with the bug is not evidence.*
- **Round 2 on `657f5c7` (owner: *"run codex again"*) — one P2, and it caught a FALSE CLAIM rather than a weak test.** The round-one guard carried the sentence *"so the day a third shape appears this fails"*, and I repeated it in `657f5c7`'s commit message, in the PR body and in my own reply on the round-one thread. **It would not.** Asserting that the two **known** writers still exist is silent about a third: both keep existing when it arrives, so a producer emitting `{ name, val }` would be dropped by `mapPost` **with the suite green** — round one's own defect, able to come straight back.
- ⚠ **AND DERIVING IT FOUND MORE WRITERS THAN THE FINDING DID: SIX, ACROSS FIVE FILES, NOT TWO.** `shapeBackend.js:3137` (the live session), the app's composer (`:14060`) and Post-a-PR sheet (`:20112`), and the **Garmin** (`:207`), **Whoop** (`:92`) and **Strava** (`:195`) sync routes. **Four of the six are server-side and no guard had ever looked at them.** No live defect — every one emits `{ label, value }` or `{ l, v }`, both of which `mapPost` reads — so what was wrong was the guard and the sentence above it, corrected at the source. *A claim about "every writer" is worth deriving once before it is written down.*
- ⚠ **THE NEW GUARD DERIVES RATHER THAN ENUMERATES, BECAUSE AN ENUMERATION GOES STALE THE SAME WAY THE OLD ASSERTION DID.** It walks every production file mentioning `workoutStats`, every identifier that flows into it — **including the local accumulator of a function *called* to build it**, which is the only way Strava's is reached — and every two-key literal written into one of those. Then it **closes the loop**: each derived shape is fed to the **shipped** normaliser and must reach the plate, so an unrecognised producer **fails here**, naming the file and line that wrote it, rather than being compared against a list somebody has to remember to update. A writer added in a new integration route is covered with nobody remembering this file exists.
- ⚠ **AND MY OWN EXTRACTOR WAS A BROKEN INSTRUMENT TWICE BEFORE IT WAS A GUARD — both caught by DRIVING it rather than reading it, and both failing in the direction that PASSES.** (1) A `[^{}]*` literal body cannot cross a template literal, and a stat value is routinely `` `${mins} min` `` — it found **11 of 15** and skipped `shapeBackend`'s writer **entirely**, i.e. it reported a clean sweep while blind to the single most important producer. (2) A `pat.exec(src) ?` probe before the `matchAll` loop advanced `lastIndex`, **which `matchAll` inherits**, so it silently ate the **first match per sink** — losing Garmin's first push and both the Whoop and Strava writers outright. *A sweep that under-reports is worse than one that over-reports, because it passes.*
- ⚠ **THE TWO VACUITY FLOORS ARE MEASURED TO BE MUTUALLY REDUNDANT, NOT ASSUMED — which is what turns a survivor into a fact rather than a number.** Removing **either** alone is a no-op and its mutation survives, because the other fires. Removing **both**, with the extractor degraded, goes **green** on a sweep that has silently lost a third of the writers. So neither is decoration: together they are the only thing between a broken extractor and a passing suite, and that reasoning is written at the site.
- **Review:** Codex, **two rounds, four findings, every one real and every one fixed**, each verified against the source before acting and each replayed as its own mutation. It was **not triggered a third time** — the owner asked for the second round specifically — so the `5c8a5cd` diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not run** (owner's standing ruling); its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `5c8a5cd` and not a draft.
- **Verified on the MERGED tree** (`665ec66`), re-derived rather than carried: `npm test` **3829/3829** (the branch read 3828; ⚠ **the +1 is #2097's two radio test files, not this work's** — stated so the move does not read as mine) · `tsc --noEmit` **0** · JSX parse · the newdesign precompile check **72 pages, 77 shared jsx, 0 errors** · the writer sweep still reading **15 literals across 8 files at exactly 2 shapes** · **28/29 mutations killed across both rounds**, each **proven to land** (anchors occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status — a pipeline reports its LAST command), sanity green at both ends, the tree restored in a `finally` **and on a signal**, and every file read and written as **bytes** so a CRLF-stored file cannot be silently normalised. The decisive mutation adds a third producer emitting `{ name, val }` to a real route and the guard **fails**.
- **Driven in Chromium** with the real faces served locally, on **both populations** and — the part that makes it evidence — **using the shapes the app actually writes**: a `{ l, v }` post renders its full plate (`NEW PR · +15 LB` / `TOP SET 315 lb` / `REPS 5` / `VOLUME 7,880 lb` / `RPE 9`), a first record reads **`NEW PR`** with no invented gain beside it, and a WHOOP-shaped row missing `zone_five_milli` renders **no zones bar at all** rather than publishing `Z5 0%`. Zero page errors, zero horizontal overflow at 1280 / 1024 / 800 — the bubble does not mount below 761px, so there is no phone layout to check.
- ⚠ **AND THE RENDER HARNESS REPORTED NOTHING TWICE BEFORE IT REPORTED ON THE PAGE.** Its Chromium path was wrong (`/opt/pw-browsers/chromium/…`; the real one is `chromium-1194/`), and it looked for the panel by an id the rich widget does not use — so it walked up from the feed body to the `position:fixed` ancestor instead. And a font-width probe was **inconclusive by construction**: a real monospace face and its fallback measure nearly identically, so the check that actually settles it is **which woff2 files the page fetched**, not how wide a string came out.
- ⚠ **REGISTERED, NOT FIXED:** there is **no on-account pass** — the signed-in path is a stubbed `shapeDb` and a scripted API response, so no real member has loaded this; and `CfPill` now has **no caller**, kept deliberately with the reason at its site because `chat-feed-tab.test.mjs` asserts this file carries its own `Card` and `Pill` rather than reaching for the dashboard globals (`Pill` is declared **twice** in `public/newdesign` with different defaults, so which one it rendered was a property of tag order). ⚠ Writing that explanation **tripped that very guard** — its ban ran against **raw source**, so naming the tag in prose read as rendering it. Fixed at the guard (it strips comments first) and **proven still to catch a real render**, rather than by rewording the comment to appease it.
...
- ⚠ **AND I RE-PAID THE "A WAIT THAT DOES NOT WAIT" LESSON THIS FILE ALREADY RECORDS — WITH A SHARPER
  ENDING.** A `run_in_background` bash `sleep` **returns immediately**, so four consecutive "waits"
  elapsed nothing and a CI run **51 seconds old** read as **eight minutes stalled**. The 2026-09-14
  entry post-mortems exactly this, and I had read it. ⚠ **What is NEW is that the obvious fix fails the
  same way:** pointing a `Monitor` at an `until [ -s <the sleep's output file> ]` loop watches a file
  that is written the instant the sleep "completes", so the monitor returns at once too — **the
  instrument inherits the defect it was brought in to cover**. What actually blocks is a `Monitor`
  whose own command is the `sleep`. The tell was cheap and I should have reached for it first:
  `date -u` disagreed with my assumption by eight minutes. *When an elapsed time is load-bearing,
  measure it; do not infer it from how many calls you have made.*
...
### 2026-09-15 — The Coaches page stops selling a retired tab, the journey cloud gets bounded rather than sampled, and a manifest nobody could see break

- **Four owner-reported items, one PR** (#2089 → `da51006a4`). *"updated snapshot here on coaches page, still showing community tab"* → *"also here"* on the hero · *"also increase the shape logo here more and center it over the subtext"* · *"also need more spacing here between the phone and the images appearing as you scroll"* · plus a guard for the chat bubble's module manifest. `public/newdesign/` and two test files; **no migration, no route, no mobile change.**
- ⚠ **THE PAGE SELLING THE DASHBOARD WAS THE LAST PLACE COMMUNITY EXISTED.** The twenty `coaches-dash-*.jpg` captures were shot **before** #2077 removed that tab from all three dashboards, so a prospect reading the Coaches page was shown a sidebar the product no longer has — in **every one of the twenty frames** and in the hero, which renders `CO_TOUR[role].tabs[0]` and is therefore the same file again. The live sidebars were driven in a real render **first**: they already read eleven items with no Community, so this is stale assets and nothing else. All twenty re-shot at **1440×900**, `?v=20260915`.
- ⚠ **AND THE CAPTURE RECIPE HAD TO BE RE-DERIVED FROM THE SHIPPED PIXELS, so it is written down this time.** A frame is the DASHBOARD, not the chrome around it: the capture suppresses `.shape-header`, `#shape-global-chat-button` and the preview banner — a fixed `div` with **no id and no class**, found by its own *"Preview · demo data"* text — and reclaims the header spacer with `main{margin-top:-72px}`. **That number is measured, not chosen**: against the shipped `trainer-business` frame, `-72` lands at **0.155%** differing pixels where `-60..-68` land at **6.2–7.8%**. The sidebar is deliberately **not** moved, which is why every frame keeps its dark band above *Today*.
- ⚠ **AND "THE NEWDESIGN PAGES CANNOT BE DRIVEN IN THIS CONTAINER" IS FALSE — it is recorded as fact in THIS file and it cost a round to refute.** React, ReactDOM and Babel come from **unpkg**, which the proxy answers `403 CONNECT` for, and the tags carry **SRI**, so a hand-rolled substitute is correctly rejected. But **unpkg serves the npm tarball's bytes verbatim**: `npm pack react@18.3.1` (and `react-dom@18.3.1`, `@babel/standalone@7.29.0`) yields files whose **sha384 equals the pinned `integrity` attribute exactly**, checked for all three. Route the unpkg URLs at those bytes and SRI validates, because they **are** the bytes. *A blocked host is not an unobtainable file.* Corrected at the source in `coaches.jsx`'s header, and every browser figure in this entry exists because of it.
- **The homepage footer mark was the smallest on the site.** `pageShell.jsx`'s `Footer` takes `logoHeight = 64` and ~70 pages render it; the homepage's **hand-written copy** had been left at **40**.
- ⚠ **AND THE BLOCK SHRINK-WRAPS ITS TAGLINE RATHER THAN CARRYING A `max-width`, BECAUSE THE OBVIOUS VERSION DOES NOT CENTRE.** `ch` is **font-relative**: `.tag2` caps at `34ch` **at 13px**, which measures **281px**, while the same `34ch` written on the wrapper resolves in the wrapper's own larger font and measures **346** — so `margin:0 auto` inside it put the mark **32px right** of the sentence it belongs to. `width:fit-content` lands both boxes on **281** and both mid-points on **173** (157/157 at 390).
- ⚠ **THE JOURNEY'S CLOUD RAN THROUGH THE PHONE, AND THE CANVAS IS WHY.** `#jcanvas` is `inset:0` on `.jpin`, so it spans the **phone column** as well — and `cx = W*0.62` with `R` up to `W*0.22` drove the orbs straight through the card. **Measured off painted pixels rather than eyeballed**: the cloud reached **x 1205 at 1440×900** and **1318 at 1920×1080** against a phone whose left edge is **1144** at both — **61–174px inside it**. It reads the phone's own rect now and fits itself to the room, **shifting left before it shrinks**, because a cloud that cleared the phone purely by getting smaller would lose a quarter of its radius on a tall screen for nothing.
- ⚠ **AND THE PHONE-HIDDEN LAYOUT IS PROVABLY UNTOUCHED, WITH THE CHECK READING THE STYLESHEET INSTEAD OF RESTATING IT.** `offsetParent` is null **exactly** when `.jph` is `display:none`, so `phoneL` is 0 and the clamp never runs — measured at 820 and 760, centre and radius unchanged. That also closes a real mismatch nobody had named: **CSS hides the phone at 860 while the canvas's own `wide` is 900**, so between those two the cloud used to centre straight through a *visible* phone.
- **THE CODEX ROUND RETURNED ONE P2, IN EXACTLY THE AREA THE TRIGGER COMMENT ASKED TO BE ATTACKED, AND IT WAS REAL ON BOTH HALVES IT NAMED.**
- ⚠ **THE BOUND WAS A SAMPLE, SO THE CLEARANCE BELONGED TO THE PAGE LOAD RATHER THAN TO THE CODE.** `ORB_R` read `max(orb[i].r)` — and `rnd()` is bare `Math.random()`, so that is the largest of **eleven UNSEEDED draws from [1.00, 1.35)**. Measured on one load: **1.3429** against a bound of 1.35. **So the six-viewport sweep in the first head passed on whatever that run happened to draw** — which is the whole shape of the defect: a measurement can pass by luck, and nothing in its output says so.
- ⚠ **AND THE CENTRE GLOW ESCAPED AN ORB-DERIVED BOUND OUTRIGHT, sampled or not.** `grr = R*(1.25 + 0.1·sin)` reaches **1.35·R whatever the orbs drew**, so no bound taken from the orbs could ever have covered it. *A bound derived from one effect is not a bound on the others.*
- ⚠ **AND WRITING THE FIX TURNED UP TWO MORE EFFECTS OUTSIDE THE OLD BOUND, ONE OF THEM ON NEITHER OUR LISTS.** The full set, with true bounds: the **orbit** 1.35 + an 8px glow box · the **centre glow** 1.35 + 0 · the **point cloud** 1.0311 + a 3.3px dot · the **ECG sweep head** 0.92 + a **28px** box · the **score arc** 0.81 + a **16px** head glow. The arc is the one neither of us named. ⚠ **AND "UNBOUNDED" IS THREE DIFFERENT THINGS HERE, which is worth more than the headline count.** Driven against the old rule rather than asserted: the **centre glow escaped at EVERY Rb**, because 1.35 exceeds any sample — that one was simply not bounded. The **ECG head** escaped only below `Rb 45`, and the old clamp *did* reach there — **42.0 at 1920×600**, an overshoot of **1.4px**, in a band of **8 widths out of 1060** — so it was real and marginal. The **arc**'s crossover is `Rb 14.3`, which the real range never reaches, and the **cloud** never escapes at all: both were covered *incidentally*, by an expression that made no claim about them. *A bound that happens to be large enough is not a bound.*
- **Every draw-site literal is a named constant read by BOTH the draw and the bound now**, so they cannot drift apart — and the cloud's own reach is **measured off the shapes that were built** (`CLOUD_R` walks `SH`) rather than restated from their constructors, so a new stage carries on its own. It computes **1.0311**, which independently matches the hand-derivation from `SH[1]`'s `0.42 + 0.6×1.02`.
- ⚠ **EACH EFFECT IS SOLVED ON ITS OWN TERMS, NOT AGAINST ONE WORST-CASE PAIR — and the cost is what decides it.** Pairing the widest reach with the largest pad is also provably safe and is two lines shorter; it costs **~9% of the radius at 1440** (Rb 224.6 → 210.5), to a **28px pad belonging to an effect that only reaches 0.92R**. That is a real regression in the one thing the owner asked for. *Conservative-by-pairing is safe and is not free.*
- ⚠ **AND THE GUARD MUST NOT TRUST THE TABLE IT IS CHECKING.** `tests/homepage-journey-clamp.test.mjs` drives the **shipped** code — the constants are built by running the page's own IIFE under a stub DOM and the geometry block is lifted **verbatim**, so nothing is restated — and it asserts the invariant against extents **rebuilt from the DRAW SITES**, never against `PAINT`. Checking a clamp against the table it was fed proves only **self-consistency**: revert one row to a sample and it still "clears" the smaller number it was handed, with the suite green. Measured: the sweep covers **2,000+ layouts**, including the 861–900px band where `wide` is false and a phone is still on screen.
- ⚠ **AND BOTH MUTATION SURVIVORS ARE NAMED IN THE TEST HEADER RATHER THAN LEFT TO READ AS COVERAGE.** **11/13 killed** — the sampled bound restored (the defect itself), each of three rows removed, each clamp loop dropped, the breathing amplitude and the pad dropped, the floor ignored, and a **new** glow box added without joining the table. One survivor is a **proven no-op** (`PAINT` reads the very same constant, so a widened pad widens the bound with it — drift is unrepresentable by construction); the other is the honest **coverage boundary**, because `phoneL` is handed in, so *how* it is measured is the browser's half. *A survivor is either a gap or a fact about the design, and saying which is the difference between a guard and a number.*
- ⚠ **AND TWO OF MY OWN MUTATIONS WERE THE BROKEN INSTRUMENT BEFORE THE GUARD WAS.** Two anchors occurred **zero** times (a trailing comma on the table's last row; a four-space indent read as eight) and the runner **reported the skip rather than relocating the edit** — which is the right failure, since a runner that silently moves an edit reports a kill for a mutation that never landed. A third "mutation" widened a pad at its draw site and **survived**, and that was the design working rather than a hole: it is now labelled as the no-op it is, and replaced by the case that actually drifts — a new glow box.
- **The fourth item is a guard for a break nobody could have seen.** `window.__ndCompiled` is **derived from the pages** by `scripts/build-newdesign.mjs`, built from the `<script type="text/babel" src="…jsx">` tags across `public/newdesign/*.html` — so a module `bootCompiledChat` asks for that **no page references** has no entry at all: `map[name]` is `undefined`, `sc.src` becomes the string `"undefined"`, the browser 404s, and `onerror` drops the member into the plain fallback panel. **The rich bubble never opens on any page that lazy-boots, and nothing fails anywhere a build or a suite would see it.**
- ⚠ **BOTH HALVES ARE DERIVED, AND THAT IS THE WHOLE GUARD.** The ask is parsed out of `globalChatButton.js`; the manifest is rebuilt with **the precompile's own two regexes**, so it asks the question the build answers rather than a paraphrase of it. Naming today's four modules would let the ask and the manifest drift apart with the test still green — which is precisely the failure being guarded. Each half carries a vacuity assertion, so a parse that stops matching **fails** rather than passing on an empty set. **4/4 mutations killed**, including the two that make each parse stop matching.
- ⚠ **AND THE MUTATION RUNNER REWROTE A FILE IT NEVER MEANT TO TOUCH.** Reading and writing in **text mode** applies universal newlines, so it silently normalised `ClientProfile.html` — the **one CRLF-stored page** in that directory — to LF; its "restore" then compared equal and left a 29-line diff behind. Caught by reading `git status` **after** the run, not by the runner. It reads and writes **bytes** now. *An instrument that edits the tree owes it a byte-FAITHFUL restore, not merely a restore.*
- ⚠ **AND THE MERGE-SAFETY CHECK CAUGHT ITS OWN HARNESS FIRST, WHICH IS WHY THE CONTROL MATTERS.** The merged tree reported **2 failures** on `@sentry/capacitor` — and the cause was my `ln -s` nesting **inside** `mobile-app/node_modules`, which is a real directory in the worktree because `mobile-app/node_modules/@capacitor/android/**` is **TRACKED IN GIT** (vendored for the Android build). So `ln -s A B` with `B` an existing directory created `B/node_modules` instead of a link, and the deps were unresolvable. *A red run is a claim about the code only once the instrument is ruled out.*
- **Verified on the merged tree** (`da51006a4`), re-derived rather than carried: `npm test` **3796/3796** · `tsc --noEmit` **0** · `coaches.jsx` parses and **all 9** inline `<script>` blocks in `index.html` parse · the newdesign precompile check **72 pages, 77 shared jsx, 0 errors** · **20** captures at 1440×900 · **17 mutations across two guards, every one proven to land — 15 killed and the 2 survivors named above** · the journey **re-measured at six viewports, 45–104px clear everywhere** with zero page errors and the phone-hidden path inert at 820/760 · the footer measured at 1440 and 390 with `logoMid === tagMid` · and both journey stages **looked at, not only measured**.
- ⚠ **AND ONE FIGURE IN MY OWN PR BODY WAS WRONG AND IS CORRECTED HERE RATHER THAN LEFT IN A MERGE COMMIT NOBODY CAN EDIT.** It said *"all 8 inline `<script>` blocks in `index.html` parse"*. Re-derived independently: **17 script tags, 9 inline, 8 external** — I had published the **external** count as the inline one. It changes no conclusion (all of them parse either way), and it is exactly the class this file post-mortems: *a count nobody re-runs is a claim.*
- **Review:** Codex, **one front-loaded round** on `798742a` as the standing rule requires — **1 finding, real, fixed, thread replied to and resolved**. Per the one-round rule the fix head `da951164d` was **not** re-triggered, so it is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. **CodeRabbit was not triggered** (owner, this session: *"dont run coderabbit for PR"*); its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `da951164d` and not a draft.
- ⚠ **AND THE SQUASH-MERGE WAS PROVEN, NOT ASSUMED — TWICE, BECAUSE `main` MOVED UNDER IT TWICE.** #2087 landed during the round, then #2088 and #2090; all three touch `mobile-app/**`, `docs/**` and mobile tests, with **zero file overlap**. Merged locally against each: **3781/3781**, then **3796/3796** against `fb322c471`.
- ⚠ **REGISTERED, NOT FIXED:** the Coaches captures are the **signed-out demo practice** — invented figures, each frame carrying *"EXAMPLE ACCOUNT"* inside its own frame — which is the pre-existing convention and still the owner's open preview-cast ruling from 2026-09-02; the cloud still passes **behind** the rail text on the left, which is the shipped design (text is `z-index:2` over the canvas) and did not get worse; and there is **no on-account pass** on any of this — every browser figure here is the signed-out preview with `/api/me` answering the route's own `503`.
...
- ⚠ **THE TWO ASKS WERE ONE DEFECT.** The form rendered INLINE on the Passport root, under the root's own `← Back` — which is the shell's `onBack` and closes Settings — so backing out of it landed on Home with the draft discarded. It is its own page now, with the same masthead-and-← row as every drill-in pane, and that row returns to the SETTINGS root: `DetailBack` takes a back target defaulting to `setDetail('')`, the edit page passes `cancelEdit`, and the root, its own Back and the doors are gated on `!editing`, so the shell's Back is unreachable from the form.
- ⚠ **KEYED ON `editing`, NEVER A `detail` VALUE.** `toRoot` clears every `detail` on `shape:openProfile`, and this page holds an unsaved draft it must not discard — the invariant already recorded beside `toRoot`. The scroll reset the tile panes got from #2085's Codex round runs on `editing` too, through one shared `scrollPaneTop`.
- **The page is the Passport's own grammar:** mono eyebrows over each field, squared boxes at radius 3, the house chamfer on every control cell (the Photo/Initials pair as a radiogroup, the three pronoun cells), the theme accent for focus and selection with the tier colour staying on the avatar as the card keeps it, the avatar card as the identity card's own box with the ✎ badge as its one control, and Cancel · Save as **`PassportBtn`** — hoisted from the identity card so the two cannot drift — at the review's 44px row.
- ⚠ **A DEFECT IN MY OWN #2085 FIX, FOUND BY READING.** `saveEdit` dropped the handle under `if (!handleKnown) delete patch.handle` and a comment claiming *"this form has no handle field"*. **It has always had one** — the owner's screenshot shows it — so a member whose account carried no username typed a handle, tapped Save, and had it silently discarded. *A comment asserting a false premise is an instruction to fix the wrong thing*, and this one was mine, one day old.
- ⚠ **AND THE SAME LAUNDERING WAS OPEN ON THE CITY AND THE BIO.** `identity` seeds to the demo persona (Brooklyn, NY; *"Cutting for summer…"*) for every account and the form presented both as the member's own; saving any other field persisted them, and the card's `locationKnown` guard lasted exactly one save. `seedDraft` withholds the three unless saved or account-derived, `bioKnown` joins the two flags, and `goal` (the Goal page's own field) and `accent` (the persona's) are never written by this form.
- ⚠ **TWO TAP TARGETS UNDER THE 24px FLOOR, MEASURED ON THE NEW PAGE.** Every drill-in pane's `← Settings` row was **78×13** — the root's own ← Back got padded in #2085 and the panes never did (98×29 now, pixel-identical via the negative margin). The avatar's ✎ badge was **22px** at the card's size (24 now; the 86px avatars already got 26).
- **THE CODEX ROUND ON `76ea63b` RETURNED TWO FINDINGS, BOTH REAL, AND THE FIRST IS THE ONE WORTH THE WORDS.** **P1:** the draft is seeded from the `*Known` flags, which are false until the `client_identity` read lands — and stay false when it fails — so a member who tapped Edit inside that window and saved an unrelated field wrote `''` over their stored city and bio, and on a bad network every save did. ⚠ **The old seed-from-`identity` had the SAME window with Brooklyn as the wrong value; the Passport swapped the value and kept the hole.** **P2:** clearing a saved handle was silently ignored — the empty value was omitted from the patch, so the merge kept the old handle stored and shown. I had written that reading up in the PR body as *"the right one?"*; it wasn't.
- **ONE RULE CLOSES BOTH: ONLY THE KEYS THE MEMBER TOUCHED ARE WRITTEN.** The form records what it edits (`touchedRef`, every control through `edit()`), `saveEdit` writes only those, and when the read lands the untouched keys of an open draft refresh from it — on the root as well as the deep link, which was the only page that refreshed before. A cleared handle is a touched `''` and is written (the hydrate already reads an empty stored handle as not-known); a handle that was never theirs is an untouched `''` and never is.
- ⚠ **AND THE DRIVE FOR IT FOUND A THIRD, MINE.** The *"nothing edited, nothing to write"* check asked of the PATCH, which the cached photo always fills, so an untouched Save still wrote one document — **driven: 1 where 0 was expected.** It asks of what the member edited now. *A guard that can never fire reads as a guard.*
- ⚠ **SIX OF MY OWN #2085 GUARDS PINNED SPELLINGS AND FAILED THE CORRECT TREE**, re-anchored on their invariants: the scroll reset resolves through its helper for every pane key, the handle guard asserts the seed rather than the retired `delete`, the seed guard asserts `startEdit` seeds through `seedDraft` rather than the line it happens to be. And a first vacuity floor of mine demanded four radii where the pane sets three — *a vacuity floor is a claim too.*
- ⚠ **CODEX WAS NOT RE-TRIGGERED ON THE FIX HEAD** — one round per PR. The fix diff is covered by my own adversarial read, the two thread replies and the mutation round, and the PR says so. **CodeRabbit was not run**; its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `a321a9f` and not a draft.
- **Verified on the final head:** `npm test` **3780/3780** · `tsc --noEmit` 0 · JSX parse · mobile build clean · **33/33 mutations killed** across two rounds, each proven to land, sanity green at both ends, the tree restored in a `finally` **and on a signal** — both Codex findings replayed as their own mutations (each field written untouched, the refresh gated on the deep link again, the refresh overwriting typed keys, a control bypassing `edit()`, a cleared handle omitted, a cleared handle left known), plus the empty-check asking the patch and the retired `delete patch.handle` restored. ⚠ Two anchors went stale across the fix round and one *insertion* mutation was misread as *"did not land"* by the runner's own check — its anchor survives by construction — each re-run once corrected.
- **Driven in Chromium at 320 · 390 · 430:** the page opens at `scrollTop 0` from a scrolled root, `← SETTINGS` lands on the Settings root and never Home, the draft re-seeds on re-entry, **0 horizontal overflow, 0 page errors, 0 controls under 24px**. In German zero English leaks. Signed in against a stubbed store, `"@@quinnh"` + `"Austin, TX"` save as exactly `{ location, handle: "@quinnh" }` and the card reads **`Quinn Harper · @quinnh · Austin, TX · BASE TIER`**. Against a store whose `client_identity` read lands **2.5 s late**: tapping Edit inside the window shows `''` for the city; the read refreshes the open form to `@saved · Brooklyn, NY · saved bio` while the typed name stays; Save carries only the name and the merge keeps all three; clearing the handle writes `handle: ""` and the card withholds it; an untouched Save writes **0** documents. The `shape:editProfile` deep link opens the page with its own back row.
- ⚠ **THREE HARNESS LESSONS.** A signed-in patch applied *before* Settings opens meets the shell's real first-run gates (goal onboarding, then the required PAR-Q) — the patch lands after Settings has mounted. After a signed-in save the **radio ask prompt** rises over the root, and its decline is a React `onClick` on a plain element no attribute selector sees; the card is read by *content* (the `.bs-scroll` carrying the quick switches), not by the topmost element. And a wait loop written as `until ! pgrep -f "node --test"` **matched its own command line** and never exited — the `pkill` self-match this file already records, in `pgrep`'s clothes.
- ⚠ **NO ON-ACCOUNT PASS.** Every signed-in reading is a patched `ShapeAuth` and a stubbed `shapeDb`. The honest check is a real legacy member — no username, no saved handle — typing one and seeing it on their card, and a real member on a slow network editing their name and keeping their city.
- ⚠ **REGISTERED, NOT FIXED:** native/hardware back from the edit page pops the Settings entry itself, exactly as it does from every `detail` pane today (`startEdit` does not `navPush`); the `PREF_OPTIONS` table's stored English tokens; the three unread preference rows; the Home ticker's storage-key sub-labels; search on the root.

### 2026-09-15 — Settings becomes the Passport, and four claims the new identity card could not support

- **The owner's pick, built** — *"i like the passport"* → *"give the boxes a more square edgy look"* → *"go with passport"* → *"yes make new settings page the passport design you just created"*, off [`REVIEW-2026-09-14-settings-page.md`](REVIEW-2026-09-14-settings-page.md) §5. #2085 → `d2f3a7a`, after #2084 → the avatar and the build number. Mobile only; **no migration, no route, no data change.**
- ⚠ **THE ROOT WAS 2.8 SCREENS AND THE MEMBER'S OWN SETTINGS BEGAN 1.7 SCREENS DOWN.** Measured in Chromium at 390×844 on the shipped build: the root scroller was **2,376 px** and the section list — Account, Privacy, Billing, Notifications — sat at **1,420 px**, under the theme picker, the radio toggle, the light-effects picker and the Home ticker editor. It is **1,196 px** for a member now (**1,290** for a coach), which is the customization section moving behind one door and the identity card taking the top.
- ⚠ **AND THAT FIGURE IS RE-DERIVED ON THE MERGED TREE, NOT CARRIED — IT READ 1,198 IN THE PR BODY AND THE MERGE COMMIT, WHICH CAN NO LONGER BE EDITED.** Two pixels change nothing about the finding; what the re-run was for is that **the number depends on a state nobody had named**. Every figure above is the **signed-out preview**, which is the state the 2,376 baseline was taken in, so the comparison is like-for-like. Signed IN the root is **1,026 px**, because the preview's own *Vorschau als · PROFILTYP* strip is ~170 px and is not there. *A scroll height is a measurement of a screen in a state, and a comparison across two states is not a comparison.*
- ⚠ **AND THE HARNESS REPORTED THE COACH ROOT AS 1,961 px WITH ZERO TILES BEFORE IT REPORTED ON THE PAGE.** The PROFILTYP switch navigates away from Settings, and a topmost-scroller finder then measures whatever page is underneath — **the zero tile count is the tell**, not the height, which was perfectly plausible. It re-opens Settings when the root is not on screen now. *An instrument that measures the layer under the one you asked about reports a number, and nothing in that number says it is the wrong page* — the same defect the 2026-09-14 review paid for with a finder that reported on Home while the screenshots showed Settings.
- ⚠ **AND THE WHOLE SETTINGS TREE LIVED UNDER A WORD MEANING "THE REST".** The page was headed **"More · 12 sections"**, and one of the twelve cards was itself called **More**. The grid IS the settings now; More is one row in an **Also** list beside About and Account actions.
- **Thirteen hub cards for a client and ten for a coach become six tiles (seven for a coach), and NOT ONE DESTINATION IS LOST** — because each pane composes **whole** sections through `findSec` + `renderRows` rather than a hand-picked subset: Billing rides inside Account, Nora's voice inside Notifications, Training + Nutrition + Preferences behind one door, Cycle inside Health. A guard asserts every section the app declares is reachable **per role**.
- ⚠ **PER ROLE, NOT UNIONED, AND THE MUTATION ROUND IS WHY.** `passportTiles` is a role ternary whose two arms differ; a union passed on a tree where the CLIENT's Training & nutrition door had **lost the Preferences section**, because the COACH's Preferences tile still reached it. *A door only one role has is not a door for the other.*
- ⚠ **A COACH GETS PREFERENCES, NOT "TRAINING & NUTRITION".** The coach card list has never carried the Training or Nutrition sections — those are a member's own training and eating preferences — so composing them into a coach's tile would have ADDED two sections their settings have never had.
- ⚠ **AND THE PLAN CARD GAINED THE ROLE GATE IT HAD NEVER HAD.** It rendered *Shape Membership · Become a member to join the community · Join now →* for **every** role — captured on the Trainer preview during the review — against the owner's standing ruling that *"Its free to join for coaches. Its $5 a month for members/clients."* The composed **billing rows** are gated too: every one opens the Stripe CUSTOMER portal, so for a coach they would have been three dead controls added by a composition meant to lose nothing.
- **The boxes are squared and chamfered on the owner's note** — the house `polygon(0 0, calc(100% - 11px) 0, 100% 11px, 100% 100%, 0 100%)` that thirteen other surfaces already clip with, so the Passport looks like the app rather than like a new idea. The whole quick-switch cell is the control, so the tap target is the box rather than the 26 px pill inside it.
- ⚠ **ONLINE IS `onlineVisible`, NOT `onlineRail`, AND THE TWO ARE OPPOSITE CLAIMS.** `onlineRail` is the member's own *view* of Community's online row — its own description says *"Hiding it changes only your view — it never changes whether others can see you online"*. A switch labelled **Online** wired to that would tell a member they were hidden while they were still visible to everyone. All three switches write through `setPref`, the module's one writer, so the mirror, the live re-render and the presence broadcast still happen.
- ⚠ **THE CITY PRINTS ONLY WHEN THE MEMBER SUPPLIED ONE.** `identity.location` seeds to **Brooklyn, NY** — the demo persona's — for every account, and until the Passport it rendered nowhere but inside the edit form, where a value you can overwrite is not a claim. On the first line of a real member's own settings it would be one.
- ⚠ **AND `← BACK` WAS A 47×13 PX TAP TARGET.** Padded with a cancelling negative margin, so the row is pixel-identical and only the hit area grew — past this repo's own documented floor, **WCAG 2.5.8 AA at 24 px**, never Apple's 44 pt suggestion.
- **CODEX FOUND A P2 ON THE CUSTOMIZE DOOR THAT ONLY THE TALL PANE COULD EVER HAVE EXPOSED.** `BSPage` keeps ONE `.bs-scroll` across every value of `detail` and never resets it — so a member who scrolled to the bottom of the root to reach the card opened a pane whose `← SETTINGS` row and whole Appearance tab bar were **already above the viewport**. Driven: `scrollTop_before_tap=685 → scrollTop_in_pane=685` without the reset, `→ 0` with it.
- ⚠ **EVERY PANE THAT SHIPPED BEFORE THIS ONE IS SHORTER THAN THE VIEWPORT, WHICH IS WHY NOBODY COULD SEE IT.** Measured on the unfixed build: the **Account** pane reports `max 0` — the browser clamps 685 → 0 by itself — where **Customize** reports `max 813`, so 685 survives untouched. *Customize is the first pane tall enough not to self-correct.*
- ⚠ **AND THE SCROLLER IS FOUND BY WALKING UP FROM OUR OWN TREE, NEVER `document.querySelector('.bs-scroll')`.** That class marks many scrollers and Settings renders as an overlay **above a still-mounted tab tree**, so the first match in the document is the page underneath — the wrong-layer trap the review itself paid for with a finder that reported on Home while the screenshots showed Settings.
- **THEN CODEX RETURNED FOUR MORE P2s ON THE PASSPORT ROOT, EVERY ONE REAL, AND EVERY ONE A CLAIM THE NEW IDENTITY CARD MAKES THAT THE CODE BEHIND IT DID NOT SUPPORT.**
- ⚠ **THE TIER WAS THE CLIENT LADDER, EVEN FOR A COACH — AND THE SECOND READING WAS DEAD CODE THIS PR WOKE UP.** `BSSettings` declared `settingsScore = _bsUseLiveScore(SHAPE_SCORE_PROFILES.client)` under a comment claiming it reflected *"the signed-in client / trainer / nutritionist's current tier"*, which it could not: `_bsUseLiveScore` derives the ladder from the profile's own `roleLabel`. It had **zero readers on `main`**, and the identity card is its first consumer. The same component already computes a role-aware `scoreProfile` — **and hands THAT one to `BSShapeScorePage`** — so a coach would have read one tier on the card and a different one on the page that card opens. **Deleted rather than corrected in place:** two score readings in one component is the defect, and one of them was already right. Measured signed in: a trainer reads **CERTIFIED TIER**, a member **BASE TIER**.
- ⚠ **AN UNREAD SUBSCRIPTION WAS REPORTED AS AN INACTIVE MEMBERSHIP.** `plan` is null while `/api/stripe/subscription` is in flight **and permanently after a failed read** — its non-ok arm sets nothing and its `.catch` swallows — and the state's own declaration draws the distinction in as many words (*"null until loaded; `{ active:false }` when there's no active subscription"*). A paying member read **MEMBERSHIP INACTIVE** under their own name on every open, and indefinitely on a bad network. Fixed on the identity line **and** on the plan card's corner label behind it, because those two sit one tap apart and are deliberately composed from the same keys.
- ⚠ **AND SIGNED-OUT STILL SETTLES AS "NOT A MEMBER", WHICH THE OBVIOUS FIX WOULD HAVE LOST.** You cannot hold a subscription with no account, so that reading is a fact about the **session** rather than about the unread plan. Treating `null` as unknown everywhere would have traded a false claim for a true one.
- ⚠ **A SYNTHESIZED HANDLE WAS PRESENTED AS THE MEMBER'S OWN — AND THE WRITE HAD TO MOVE, OR THE GUARD LASTS ONE SAVE.** With no account `username` and no saved handle the initializer slugs one out of the display name, and `bsMyName()` itself falls back to the **email local-part** — so a legacy account's card offered `@chris.perry` as a public handle nobody had claimed and which may belong to someone else. `handleKnown` is the `locationKnown` rule applied to the handle. But this form has **no handle field**, so the draft carries whatever was seeded and `saveEdit` persists the whole draft: saving any *other* field used to write the synthesized handle into `client_identity`, after which the hydrate reads it back as **claimed**. `bsSaveIdentity` merges, so omitting the key cannot blank a real saved handle.
- ⚠ **THE NORA ROWS WERE HARDCODED ENGLISH, AND THIS PR IS WHAT MADE THEM REACHABLE.** *Nora's voice* was defined in `sections` and opened by **no card at all** — five rows nothing could reach, which the review found; routing Notifications into it is the fix, and routing it as it stood would have turned a dormant gap into a **reachable** English-only block in twelve locales. Nine new keys × 13 locales, each composed from that catalog's **own** vocabulary rather than translated fresh (the word for *voice* lifted out of its own `section.nora` — Stimme · voz · voix · Murya · Suara · voce · Голос · ses · Giọng), plus `segLabels` on both segmented rows, which is the mechanism the Units row already uses for exactly this: *a keyed label above an English ON/OFF pair is half a fix.*
- ⚠ **THE SECTION TITLE STAYS AN ENGLISH LITERAL, BECAUSE IT IS AN ADDRESS AND NOT COPY.** `findSec('Nora's voice')` matches on that exact string, so *"translate every string in the section"* is the wrong generalisation — it would make the pane **unfindable** and render nothing at all. A guard pins it and the mutation that translates it is killed.
- ⚠ **AND THE OPTION *VALUES* ARE DELIBERATELY NOT TOUCHED.** `prefs[key]` holds the raw English token (`On`, `Supportive`, `Auto`) and is what `setPref` stores and `renderRows` prints for a dropdown — the same for **every already-reachable** segmented row in Notifications, Preferences and Privacy. Localizing a stored token needs a display mapping across the whole `PREF_OPTIONS` table: this module's own pre-existing gap, **registered rather than half-closed behind one door**. The contrast is visible in the render — the new Nora segments read **AN/AUS** and **UNTERSTÜTZEND/DIREKT** while the rows above them still read `ON · 30M BEFORE`.
- ⚠ **THE i18n RATCHET CANNOT SEE THAT FIX, AND ITS COLUMNS BEING UNMOVED IS EVIDENCE OF NOTHING.** `BSSettings` is PARTIAL to that walk over exactly **one** string it can see — the `'AB'` initials placeholder — and these labels live in a local `const sections` object literal, that walk's **own documented blind shape**. So 220/757/38/91 reads the same before and after, and a session that reported it as the certification would be quoting an instrument that never counted the strings. *The wrong instrument returning the expected answer is the most convincing way to be wrong* — the five new guards are what measure this, and a comment above them says so.
- ⚠ **TWO OF MY OWN GUARDS WERE BROKEN INSTRUMENTS FIRST, BOTH CAUGHT BY FAILING ON A CORRECT TREE.** The `BSSettings` extractor scanned to the next `\nfunction ` — and this module declares components **both** ways, so it ran **181,306 characters** past the end and every count taken over it was a count of half the file. Fixed to an AST span, it then counted its **own rationale comment** as a second score reading; it goes through the shared `stripComments` now, and a mutation proves that is load-bearing. A third pinned a `[^:]*` character class that can never cross `tr('settings:plan.inactive'`.
- ⚠ **AND A SIBLING GUARD FAILED THE CORRECT FIX AGAIN.** `tests/pref-options-token.test.mjs` pinned the retired hub card's exact spelling, so the Passport broke a test about something else entirely; re-anchored on the invariant (every reference to those preference keys is inside a `bsPrefOptionLabel(…)` call or used as a condition) and **proven to still fail** on a raw-token mutation.
- **Verified on the final head:** `npm test` **3773/3773** · `tsc --noEmit` **0** · JSX parse · the full pre-commit gate · mobile build clean with all **9 keys × 13 locales confirmed in the emitted bundle** behind a negative control · i18n parity 13/13 at **519 keys**, one orphan removed · **32 mutations killed across three rounds** (13 Passport + 4 scroll + 15 review-round), each **proven to land** (occurrence-counted before the edit, the suite's own `# pass`/`# fail` **parsed** rather than read off a pipeline's exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal** — every finding replayed as its own mutation, plus **two aimed at the INSTRUMENTS** (the extractor stripped of its comment-stripping, and its span truncated).
- **Driven in Chromium at 320 · 375 · 390 · 430 px**: zero horizontal overflow, zero page errors, **zero controls under the 24 px floor**, every tile opening a non-empty pane **at its top**. In German the Notifications pane renders *Noras Stimme · Antworten vorlesen · AN/AUS · Tonfall · UNTERSTÜTZEND/DIREKT · Stimme anhören → ANHÖREN · Was Nora sich merkt → ANSEHEN* with **zero English leaks**. Signed in, a member reads **BASE TIER** and a trainer **CERTIFIED TIER**, neither claims MEMBERSHIP INACTIVE while the plan is unread, and the `@handle` is **withheld** with no account username and **shown** (`@quinn.h`, `@coach.q`) when there is one.
- ⚠ **THE REVIEW DOC STILL READ "Not built" OVER SHIPPED CODE, AND IT IS MARKED RATHER THAN REWRITTEN.** A review says what was read on the day it was read — but a status line left uncorrected is what the next reader acts on, and *"Not built"* over a shipped page is an instruction to build it twice. Its §6 rulings that this PR answered by building are marked; the ones still genuinely open say so.
- ⚠ **CODEX WAS NOT TRIGGERED A THIRD TIME, AND THE RECORD SAYS SO RATHER THAN IMPLYING A LAYER RAN.** Two rounds, seven findings, all real and all fixed. Per the standing one-round-per-PR rule the fix diff is covered by my own adversarial read plus the mutation round. **CodeRabbit was not run** (owner's standing ruling); its skip-review notice is automatic and is not a review. The merge gate was the four required checks green on `c7b7988` and not a draft.
- ⚠ **NO ON-ACCOUNT PASS, AND IT IS THE STANDING CAVEAT.** Every signed-in reading above is a **patched `ShapeAuth`**, not a real session — and an init-script stub is overwritten by the app's own bootstrap, which this harness had to learn again (patch after boot, or you measure the signed-out path and read it as the feature missing). The honest check is a real coach at capacity opening Your practice, and a real member whose subscription read fails.
- ⚠ **REGISTERED, NOT FIXED:** `timeZone`, `weekStarts` and the Preferences `language` are still **read by nothing** and two are printed on a coach's Preferences tile; the Home ticker still prints each metric's **storage key** (`CAL · PRO · HAB`) as its member-facing sub-label with `BS_TICKER_METRICS` hardcoded English in 13 locales; the whole `PREF_OPTIONS` table still stores and prints raw English tokens; and **Edit profile as its own page** and **search on the root** are later steps.

### 2026-09-14 — The chat bubble reads the member's real conversations, and three findings about who a thread belongs to

- **The second half of the owner's ask** — *"the chat bubble pop up messages need to be synced with app as well. Like message history"* — after #2077 moved the feed in. #2082 → `edac25a`. `public/newdesign/` only; **no migration, no new route, no mobile change.**
- ⚠ **IT HAD NO MESSAGE HISTORY AT ALL, SO THIS IS NOT A SYNC FIX.** `clientChatThreads.jsx` types Maya Okafor, Marcus J. and `# running-club` into the file, and a thread only became DB-backed when something handed `__openChat({ conversationId })` an explicit id. A member who messaged their coach in the app opened the bubble on the web and saw invented messages from people who do not exist. **No new route is needed** — `conversations` is readable by its participants (`participants read conversations` → `can_access_conversation(id)`), the same path the app takes — and the shapes already match, so this maps rather than adapts.
- ⚠ **AND A CLAIM I WROTE IN THE PR BODY WAS FALSE, WHICH IS WHAT REMOVED A WHOLE READ.** It said the poll *"fetched only messages newer than `since`"*. `fetchOnce` runs with **no** `since` the first time a conversation is opened (`lastSeenRef` is empty), which is the route's own cold-load path, and that arm **replaces** the thread's messages. The gap was never the messages; it was that the thread **list** was invented, so no thread carried an id for the poll to read. *A comment asserting a false cause is an instruction to fix the wrong thing* — and this one made a bulk `messages` read look necessary.
- **CODEX RETURNED THREE FINDINGS ON THE FIRST HEAD, EVERY ONE VERIFIED AGAINST THE SOURCE BEFORE ACTING AND EVERY ONE REAL.**
- ⚠ **P1 — THE READ ASSUMED `window.shapeDb`, AND IT IS WIDER THAN THE ONE PAGE IT NAMED.** Derived rather than estimated: **25 of the 35 hosts that carry this widget do not load `/supabase.js`** — `chatPopout.html`, every marketing page, and every `Client*.html` dashboard stub. So a bare `if (!db) return;` in an empty-dependency effect is **permanent**: the pop-out opened from a signed-in dashboard would have shown the sample cast forever. It goes through `pageShell.jsx`'s `ssEnsureDb` now, exported as `window.shapeEnsureDb` and **reused rather than copied**, because a second copy of the vendor bundle's SRI hash in another file is the thing worth avoiding. `pageShell.jsx` precedes `chatWidget.jsx` on all 35 hosts and in both of `globalChatButton.js`'s boot lists, which is asserted rather than assumed.
- ⚠ **AND THE BUNDLE LOADS ONLY FOR A SESSION, WITH "CANNOT TELL" TRYING.** The gate is the `/api/me` answer the hydrate already fetched, so it costs no extra request — and it reads **three** states, not two: that route answers `200 {user:null}` for a **measured** signed-out visitor and `503` for a read that did not complete, and only the first skips the load. A probe that skips on a blip costs a signed-in member their inbox; one that tries costs a download, and `getUser()` settles the truth either way.
- ⚠ **P1 — A COACH SAW THEIR OWN NAME ON EVERY THREAD.** RLS returns every conversation the viewer participates in, and for a trainer or nutritionist that includes all of **their clients'** threads — rows created with `title` set to the **provider's** name (`2026-05-02-conversations-messages.sql:205-206`). So a coach opening the bubble on `TrainerApp.html` got N threads all labelled with their own name, indistinguishable, under a tab reading *"Your coaches"*. Fixed as a property of the **row** rather than of the account: `.eq("client_id", me)` is exactly what that tab means, needs no role resolution and no extra read, uses the existing `conversations (client_id, last_message_at desc)` index, and stays right for a **dual-role** account where a role gate would not — a trainer who is also a member still gets their own nutritionist here.
- ⚠ **AND WRITING THAT FIX TURNED UP A THIRD CASE, FOUND BY MY OWN READ RATHER THAN BY THE REVIEW.** There is a **third writer** of `kind='direct'` rows with no `dm_key`: `get_or_create_coach_coach_conversation` (`2026-05-26-shared-clients.sql:181`) opens a **coach↔coach** thread titled `Care team` with **no provider** and `client_id` set to the client it is *about* — and the member is **not a participant**. `can_access_conversation` grants on `c.client_id = auth.uid()` (`:71`) all the same, so the row comes back to them, and this widget would have rendered two coaches' private thread about them under *"Your coaches"* as `Care team · Trainer · Shape coach`. `.not("provider_role", "is", null)` closes it, and also makes the `roleLabel` fallback unreachable rather than wrong.
- ⚠ **P2 — THE BULK `messages` READ IS DELETED RATHER THAN REORDERED, AND ITS ABSENCE IS THE FIX.** One combined `.in("conversation_id", […])` ordered **ascending** shares PostgREST's row ceiling across *every* conversation, so a long thread opened on its oldest messages while its own preview advertised a newer one — and a busy thread starved the quiet ones. It was also **dead weight**: the cold-load arm above discards every row it fetched on the first look at the thread. History comes from `/api/conversations/[id]/messages`, per thread, on open — the one place that gets the cap right (newest-first, capped, re-reversed at `route.ts:55` and `:60`, with the incremental poll deliberately staying ascending). **The list needs no messages at all**: `conversations.last_message` is maintained by a trigger on every insert (`:146`) and the DM RPC returns its own, so the previews are real without reading a single row, and the threads hand over `messages: []`.
- **Also load-bearing, not tidy:** the cookie session is bridged **before** any read (a session living only in the Next.js HTTP cookies leaves this client **anon**, so every read returns nothing and a signed-in member is shown an empty inbox — this file has paid for that twice); a **failed read is never an empty inbox** (falling through to `[]` replaces real threads with *"you have no conversations"*, a claim rather than an absence — the hard leg bails, the DM leg degrades alone); member DMs go through `list_member_dm_threads`, because the other member's *name* is not on the conversation row; every thread carries its `conversationId`, which is why send, the cold load and the poll all work with no further change; the install is **by tab id**, not index; and it **waits for the localStorage hydrate**, because both are async and neither is ordered against the other — on the run where the hydrate lands second it restores the sample threads over the real ones.
- ⚠ **AND A CITATION IN MY OWN COMMENT WAS OFF BY ONE, CORRECTED HERE RATHER THAN LEFT.** It read `…messages.sql:204`; the `insert into` is `:205` and the `values` carrying `v_provider_name` is `:206`. Re-derived from the merged tree while writing this entry rather than carried from the PR reply. *A `path:line` nobody re-runs is a claim*, and this file post-mortems eighteen of them at once on 2026-09-10.
- ⚠ **SCOPE: CHANNELS ARE DELIBERATELY NOT HERE.** They are a separate `channels` table with their own rows and their own message store, so they are a different read rather than one more bucket; the Channels tab keeps its sample set until that lands.
- **Review:** Codex, one round on `81768b4`, **3 findings — 2×P1, 1×P2, all real, all fixed** and all three replayed as their own mutations. Per the standing conventions it was **not re-triggered** (one round per PR, everything front-loaded) and **CodeRabbit was not run** (owner, this session: *"dont run coderabbit for PR"*) — its skip-review notice is automatic and is not a review. The fix diff is covered by my own adversarial read plus the mutation round, and the PR says so rather than implying a layer ran. The merge gate was the four required checks green on `83e7cfb4` and not a draft.
- **Verified on the final head:** `npm test` **3735/3735** · `tsc --noEmit` 0 · JSX parse on both changed modules · the newdesign precompile check (**72 pages**, 0 errors) · the full pre-commit gate including the mobile build · **24/24 mutations killed**, each proven to land (anchor occurrence-counted before the edit, the suite's `# fail` **parsed** rather than read off an exit status), sanity green at both ends, the tree restored in a `finally` **and on a signal** — the loader hop reverted · the export dropped · a host and a boot list reordered · the probe collapsed to two states · an unreadable `/api/me` read as signed out · the read stopped depending on the probe · the `client_id` scope dropped · the `Care team` leak · the bulk ascending read restored · the route's cold load broken both ways · the poll's replace arm folded back · both preview reads · the trigger · and each of the three findings' own premises moved in the migrations, so a guard that has stopped describing the schema fails rather than passing vacuously.
- ⚠ **NO ON-ACCOUNT PASS, AND IT IS THE STANDING CAVEAT.** Every path here is argued from the schema, the RLS policy and the app's own reads and driven against stubs; none has run against a real signed-in member. The honest check is a member with a coach thread opening the bubble. **And no browser render** — these pages load React 18 UMD and Babel from **unpkg with SRI**, and this container's proxy answers `403 CONNECT` for that host, so a substitute fails SRI and the pages cannot be driven at all.
- ⚠ **REGISTERED, NOT FIXED:** whether a member should be able to **read** the coaches' `Care team` thread at all is a question about `can_access_conversation`, i.e. a migration and somebody's ruling — this stops surfacing it and revokes nothing; and `public/mobile/` keeps its own copies of the community pages, so neither this change nor #2077 reaches that legacy surface.
...
- ⚠ **THE OWNER'S READING BEAT MINE, AND THE OBJECTION I HAD BUILT MINE ON WAS MEASURABLY FALSE.** I had recommended the Wall go on the Community page, arguing a ~400px chat popup would squeeze a card carrying a stat grid, HR zone bars and split tables. `chatWidget.jsx:421` reads `const DEFAULT_SIZE = { w: 1180, h: 860 }` and it is **resizable** — wider than the dashboard's own content column. *I reasoned from the words "chat bubble" instead of measuring the container*, and that was the load-bearing half of the recommendation.
- ⚠ **AND THE BETTER ARGUMENT IS THE ONE THE APP ALREADY MAKES.** Its Chat is four segments — **Feed · Team · Channels · Support** — and the bubble already carried three of them. The Wall is a **CHIP INSIDE Feed**, not a surface beside it: #2036 retired the separate Wall segment on the owner's own ruling (*"don't need 2 wall tabs"*). So the website was splitting the app's ONE page across two surfaces, and my plan would have kept it that way while leaving three card implementations alive to drift. One surface on both now.
- **It is a MOVE, not a port, which is what made it tractable.** `dashboardCommunity.jsx` → `communityFeed.jsx`: the card renderers, the eight stat rows, the session-details charts and the composer are **unchanged** — **361 lines of a ~1600-line file, net negative**. What changed is the frame and the filter grammar. A faithful port of the app's own `BSActivityCard` was the alternative and was measured first: **708 lines reaching 35 app-local helpers, 8 sub-components and 29 `tr()` sites**, none of which exist on the website.
- ⚠ **THE DATA ALREADY MATCHED — ONLY THE RENDERING DIVERGED, AND THAT IS THE FINDING THAT SIZED THE WHOLE JOB.** The app reads `community_posts` through `COMMUNITY_POST_SELECT` (`shapeBackend.js:1158`); `/api/community/feed/route.ts:64` selects the **byte-identical string**. Same rows, same `universal`/`following` modes. So "make the website match the app" was never a data-sync project.
- ⚠ **THE CHIPS FILTER ON CHANNEL, WHICH IS NOT A COLUMN.** `community_posts` has no `channel`; the app stashes it in the **metrics jsonb** (`shapeBackend.js:3694`) and lifts it back out in its own row mapper. Reading `p.channel` off a raw API row returns `undefined` for every post, so **the Wall renders empty with nothing failing anywhere**. The Wall is `metrics.channel === 'COMMUNITY'` — what the app's composer stamps on every logged session, PR and workout (`:13980`, `:20102`, `:23432`).
- ⚠ **AND THE DEMO CARDS CARRY A TIER STRING WHERE THE ROLE FALLBACK LOOKS.** The app resolves a channel-less post from the author's role; these cards hold `role: "Tempo · 1,284"`, which matches none of the role words — so **all eight resolved to CLIENT and the Wall opened EMPTY**, the same failure from the other side. Fixed by stamping the demo DATA rather than bending the rule, so the derivation stays byte-faithful to the app's; a first cut added a kind-based fallback and was reverted, because every live post is mapped `kind: 'post'` so that branch could only ever fire for demo rows — **demo-only logic inside a rule whose whole job is matching the app.** Driven against the shipped functions: **6 on the Wall, 2 on Community, 0 stranded**.
- ⚠ **THE FEED CARRIED TWO DASHBOARD GLOBALS INTO A WIDGET THAT MOUNTS EVERYWHERE.** `Card` and `Pill` are file-scope declarations in `trainerDashboard.jsx` — classic scripts, so they land on `window`. As a dashboard page the feed sat beside them; the bubble mounts on pages that load no dashboard module at all, and there is **no error boundary anywhere in `public/newdesign`**, so that is a blank page rather than a missing card. Both inlined. ⚠ **`Pill` is additionally declared TWICE in this directory with DIFFERENT defaults** (`trainerDashboard.jsx:217` tone `"mute"`, `publicProfile.jsx:203` tone `"default"`), so which one the feed rendered with was a property of tag order; the inlined copy is the one the Community page actually loaded.
- **The thread pane is hidden, never unmounted, behind the Feed tab** — that subtree owns the message poll, every open thread's scroll position and its unsent draft. And the Feed tab is **owned by the widget rather than by either tab source**: two files supply `tabs`, so adding it to one would have given half the site a Feed tab and the other half none, silently.
- ⚠ **THE BUBBLE'S CHANNELS TAB WAS LABELLED "Community", ONE CLICK FROM A CHIP LABELLED "Community" — the owner caught it** (*"channels is in the community tab on the chat bubble"*). Relabelled **Channels**, which is what the app calls that segment; **the id is untouched**, being the tab's address (`__openChat(who, "community")`, the persistence key, globalChatButton's own fallback set).
- **What came out with the page:** three shells, three nav entries and three shell routes across all three roles; both dashboard-tour steps, **folded into the Chat step that already pointed at the bubble**; the war-room rows; the Coaches page's **derived** tour tab and its two orphaned captures; and ~10.7KB of dead channels card and demo arrays. The hardcoded **`4,218 MEMBERS · 128 ACTIVE NOW`** eyebrow is not carried over — two figures nobody measured. The marketing `Community.html` is untouched and stays in the nav and the footer.
- ⚠ **A `feed` NOTIFICATION POINTED AT A FILENAME THAT NOW 404s.** `DASH_INBOX_ROUTES.feed` was `ClientCommunity.html`; the feed is not a URL any more, so that notice opens the bubble on its Feed tab instead. The two *"message this member"* fallbacks in `livingProfilePage.jsx` and `memberProfile.jsx` were repointed the same way.
- ⚠ **AND THE CODEX ROUND RETURNED THREE P1s, EVERY ONE REAL AND EVERY ONE INTRODUCED BY THE MOVE.** (1) The channel was stamped onto the **request** only, so the optimistic row had none and fell back to the hardcoded `ME.role` tier string — **a post made on the Wall vanished the instant it was published**. That is the demo-card defect again, *in the one place I had not looked*. (2) As a dashboard page the composer was already behind a signed-in shell; in the bubble it renders on **Landing and Login**, `New post` was unconditional, and the response arm read `if (!isMs || !r || !r.ok) return` — so an ordinary post's **401 was swallowed and the optimistic row stayed**. Gated on a MEASURED signed-in (three states — `null` unresolved is not signed-out, or a member cannot post while the read is in flight) **and** rolled back on every refusal, because the gate cannot cover a race. (3) **Prepending the Feed tab silently destroyed every existing member's chat state**: `threadsByTab`/`activeByTab` are positional and the hydrate accepted a record only on an exact length match, so every `shape.chat.v2.*` record failed, was dropped, and was overwritten with defaults on the next keystroke — losing sent messages and every channel they had created. Migrated rather than version-bumped, since a bump discards the same data deliberately.
- ⚠ **ONE TEST WAS RE-ANCHORED, AND IT IS THE CLASS THIS FILE KEEPS PAYING FOR.** `community-split-strip` required `kind` to be **immediately followed by** `who`, so adding a field between them made it match nothing and fail with *"the run post was not found"* on a card that was right there. *A guard that pins a layout pins whatever that layout is wrong about.*
- **Verified on the merged head:** `npm test` **3717/3717** · `tsc --noEmit` 0 · JSX/JS parse on every changed module · the newdesign precompile check (**72 pages**, down from 75 with the three stubs, 0 errors) · the full pre-commit gate including the mobile build · **16/16 mutations killed** across two rounds, each **proven to land** (occurrence-counted before the edit), sanity green at both ends, the tree restored in a `finally` and on a signal — the six replaying Codex's three findings among them · and the load order proven on **all 36** widget hosts, which is the property that matters (the widget reads `window.CommunityFeed` at FIRST RENDER, so a module loaded after it is a tab that silently never appears).
...
### 2026-09-11 — Four orderings for one record, and the fourth is the one that cannot lose it

- **The Wall's "Post a PR" went through FOUR write orderings in one PR, three of them fixed for a
  real defect and each producing the next.** Codex found the first two, CodeRabbit the third; the
  owner's ruling mid-way restored CodeRabbit as the fallback when Codex declines (*"then run
  coderabbit if codex is not working"*), which is why the last round had a reviewer at all. **No
  migration** — and the absence of one is a finding, not a shortcut.
- ⚠ **THE PROBLEM IS TWO STORES AND ONE BUTTON.** A record is a `pr_wall_posts` row written by
  `post_my_pr_to_wall`; a plate is a `community_posts` row. Since #2036 the Wall **is** the activity
  feed, so a member needs both — and no ordering of two unsynchronised writes is safe. Every round
  below is a different answer to *which half may be left behind when the other fails*.
  1. **Ledger only** (the restored sheet). A ledger row rendered NOTHING; the toast said *"On the
     wall"* over a plate that existed nowhere.
  2. **Post, then announce.** Fixed the plate. A refused announce then left an orphan post with its
     own **+5 award**, and a retry inserted a second one.
  3. **Announce, then post.** Fixed the orphan post and created a far worse orphan, because
     `post_my_pr_to_wall` refuses anything that does not **beat** the stored best: once the ledger
     had moved, a failed insert left a record **the server itself would refuse to publish forever**.
  4. **Post as an ordinary workout, announce, then stamp the record from the verdict.**
- ⚠ **(3) IS THE ONE WORTH THE MOST WORDS, BECAUSE IT LOOKED LIKE A FIX AND WAS A REGRESSION.** An
  orphan POST is visible, ordinary and the member's own to delete; an orphan LEDGER ROW is invisible,
  unrecoverable, and — driven rather than argued — **`bsWallYourBest` reported it as posted**:
  with the ledger at 245 and no plate, the row comes back `unposted: false, postable: false` in both
  the manual-only and the also-logged case. So *Your best* actively told the member their record was
  on a wall it had never reached. **Trading a visible failure for an invisible one is not a fix.**
- ⚠ **(4) IS ONLY POSSIBLE BECAUSE THE POST IS MUTABLE, WHICH NOTHING IN THE EARLIER ROUNDS USED.**
  `updateCommunityPost` merges into `metrics` and treats `''`/`null` as removal — so a retry can
  **update the post it already made** rather than insert a second one with a second award, and the
  record fields can be written **after** the verdict instead of optimistically. Nothing can now
  advance the ledger for a plate that does not exist, so every failure stays retryable forever; what
  a failure can leave is a community post, which is visible and deletable.
- ⚠ **AND THE MARKER IS WRITTEN ONLY BEHIND AN ACCEPTED RECORD — a reviewer's remedy adopted, and it
  made the code SIMPLER.** The post is published as the ordinary workout it honestly is; `pr` and
  `delta` are stamped from the verdict. A refusal then needs **no repair at all**, because there was
  never a claim to take back — and an announce we cannot read leaves the post **under**-claiming
  rather than over-claiming, which is the only honest direction for an unresolvable case.
- ⚠ **THE GAIN COMES FROM `res.prev`, NEVER FROM THE READ THAT PRECEDED IT.** The client pre-check
  exists to refuse before anything is published (or "Post a PR" with a number that beats nothing
  publishes a post nobody asked for); it is **not** a source for the delta, because a concurrent
  winner between the read and the announce makes a read-derived gain wrong — by 40 lb in the fixture.
  It compares in **pounds**, since the ledger keeps a unit per row; an unreadable ledger neither
  refuses nor guesses.
- ⚠ **`pr: true` EXISTS BECAUSE A FIRST RECORD HAS NO DELTA — Codex's P1, and it crosses three files
  on the way back.** `delta` is only defined against a PRIOR best, and both PR consumers on a real
  post read it (`kind: 'pr'` is demo-only), so a member's genuinely-first accepted best was filtered
  out of the PR chip and drawn as an ordinary load. **My first round of guards proved it was WRITTEN
  and never that it was READ**: three mutations — dropping it at shapeBackend's row mapper, at the
  client's mapper, or at the filter — left the whole suite green while the defect returned. The chain
  is driven end to end now, with a control and a legacy case.
- ⚠ **AND A LATER REWRITE OF THE SHEET'S TESTS DELETED THAT SECTION, WHICH ONLY THE MUTATION ROUND
  CAUGHT.** Four previously-killed mutations came back alive in one careless edit span. *A guard is
  as easy to lose to a bad boundary as to a bad rule* — it sits below the sheet's own tests now, with
  the reason written above it.
- ⚠ **A GAP WAS REFUSED FOR BEING IN KILOGRAMS, AND MY OWN RULE IS WHAT DID IT.** It read *"a lift
  logged in kg against a ledger row in lb is two different numbers, and subtracting them would invent
  a gap"* — which **conflates inventing with converting**. A conversion inside one family is exact,
  and refusing it hid the higher, unposted best behind the old post's date with no way to put it up.
  What the old rule was right about is the **family**, so that is all that survives.
- ⚠ **THE SECURITY FINDING WAS HALF RIGHT, AND THE REFUTED HALF IS THE PART TO REMEMBER.** CodeRabbit
  called client-settable `metrics.pr` an authorization bypass (CWE-602). Measured: `metrics.delta` is
  caller-supplied **on `origin/main`** and `bsFeedTypeMatch` has read it as a PR signal that whole
  time, so the marker adds **no capability**; and nothing privileged is reachable, because the ledger,
  the +5 award and the leaderboard are all definer RPCs that re-check the caller. **Registered, not
  half-fixed:** `community_posts.metrics` is client-written jsonb, so stripping `pr` while leaving
  `delta` closes nothing — a real fix is one server-side rule over the whole document, which is a
  migration and the owner's call. *CodeRabbit agreed on both points.*
- ⚠ **TWO GUARDS WERE DELETED RATHER THAN TESTED AROUND.** A `known` flag on the prior-best read was
  implied by `lb != null` at every return, so a mutation removing it from **both** call sites
  survived. And the `stored === 'supabase'` check is recorded as a **proven no-op** — every
  non-supabase path in `myPRLedger` answers `data: []` — kept only against that function ever growing
  a local cache, with the reasoning at the site rather than a fixture inventing a shape production
  cannot produce.
- **i18n:** `wall.postedNoFeed` → `wall.postedNoRecord` × 13 locales, each recomposed from that
  catalog's **own** "could not X — try again" pattern (`send.error`) and its own word for a record.
  The old copy said *"Record saved"*, which ordering 4 makes false. ⚠ The call site's `defaultValue`
  and the catalog disagreed on the first run — **the catalog is what renders**, so the JSX was the
  stale copy, and the resolution guard caught it.
- **Verified on the rebased head:** `npm test` **3394/3394** · `tsc --noEmit` 0 · JSX + JS parse ·
  the newdesign precompile check · **17/18 mutations killed** across the final round, the survivor
  proven a no-op above · and the whole change confirmed in the emitted bundle: the creation payload
  as `{kind:'workout',workoutStats,lift,load}` carrying **no `pr` and no `delta`**, the verdict stamp
  present, the announce carrying its `postId`, all 13 translations shipped, and **two negative
  controls reading 0** (the retired `postedNoFeed` key and the retired repair patch).
- ⚠ **REGISTERED, NOT FIXED:** an endurance record still cannot reach the wall by hand; the composer's
  own log affordance carries a hardcoded English `aria-label="Log activity"`; a server-side rule over
  `community_posts.metrics` is owed if PR presentation must be authoritative; and **no on-account
  pass** — production holds **0** `pr_wall_posts`, so not one of these four orderings has ever run
  against real RLS.
