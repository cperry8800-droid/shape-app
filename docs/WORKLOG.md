...
- ⚠ **AND THE MERGE COMMIT'S FOOTER IS MALFORMED, WHICH IS MINE AND CANNOT BE EDITED.** Its trailing `Co-Authored-By` line carries HTML entities (`&lt;`/`&gt;`) and a stray `</commit_message></invoke>` fragment from the tool call that wrote it. It changes nothing about the code and **a force-push to `main` to tidy a footer is far worse than the footer**, so it is recorded here rather than repaired. *A merge commit is the one message you cannot correct, so it is the one worth reading twice before sending.*
