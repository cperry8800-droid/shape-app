...
| **First screen** | **Two states, one instrument.** *Listening* (Y‑T): the station's live waveform sweeps the full width as one teal trace over a quiet dot field, the track is the hero at 32 px, the one wide key reads Match my BPM — nothing about the member's body until they ask. *Matching* (X‑Y, on tap): the heart's axis grows out of the centre in rust, the trace folds into the Lissajous figure (the measured beat drives the horizontal sweep, the strap the vertical), the field comes up, the lock state appears; × Listen only returns. The two sources are told apart by colour and axis — the station teal and horizontal, the heart rust and vertical — each axis flashing on its own pulse with its own reading at its end; the beam is cream until they agree, then teal. | A status rail (On air · session clock · Tempo · Signal) · the waterfall as the ground (64 log-spaced bands across, one row per frame down, black → teal → cream, a hertz axis) · the channel strip · the Now band over a dark strip · the lock as a readout line (You · a bar that fills as the rates close · Station) · the deck · one Match key. |
...
| **What reads "—"** | No strap → the key reads Connect monitor while listening; in matching the vertical axis goes dashed, its reading at the top reads "—", the figure collapses to a flat sweep and a line under it says the figure needs your pulse; Tempo until settled; no count, no scrubber. | You without a strap ("—", an empty bar, the key says Connect monitor); Tempo until settled; no count, no seek; the axis is labelled in hertz from the sample rate. |
