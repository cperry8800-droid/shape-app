          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', borderTop: `1px solid ${t.RULE}` }}>
            {stats.map(([label, value], i) => (
              <div key={label} style={{ padding: '9px 5px', borderLeft: i ? `1px solid ${t.RULE}` : 0, textAlign: 'center' }}>
                <div style={{ fontFamily: t.MONO, fontSize: 9, letterSpacing: '0.16em', color: t.INK50 }}>{label}</div>
            ...
            <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 6 }}>
              {metrics.map(({ label, value }) => (
                <span key={label} style={{ padding: '8px 5px', borderRadius: t.RADIUS_SM, background: 'rgba(255,255,255,0.035)', textAlign: 'center' }}>
                  <span style={{ display: 'block', fontFamily: t.MONO, fontSize: 9, letterSpacing: '0.16em', color: t.INK50 }}>{label}</span>
            ...
            <div key={entry.date} style={{ borderRadius: t.RADIUS_SM, border: `1px solid ${t.RULE}`, background: t.PAPER2, overflow: 'hidden' }}>
              ...
              >
                ...
                <span>
                  ...
                  <span style={{ display: 'block', fontFamily: t.MONO, fontSize: 9, color: t.INK50, letterSpacing: '0.1em', marginTop: 2, textTransform: 'uppercase' }}>{entry.meta}</span>
                </span>
          ...
          const chRow = (ch) => {
            ...
            return (
              <div key={ch.id} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 48px 12px 12px', borderRadius: 14, border: `1px solid ${hair}`, background: card }}>
                ...
                <button onClick={() => pinChannelNow(ch)} aria-label={ch.pinned ? 'Unpin' : 'Pin'} title={ch.pinned ? 'Unpin' : 'Pin to top'} style={{ position: 'absolute', top: 8, right: 8, zIndex: 2, width: 30, height: 30, borderRadius: 999, border: `1.5px solid ${ch.pinned ? TEALB : t.SURFACE_BORDER}`, background: ch.pinned ? `${TEALB}33` : t.SURFACE, boxShadow: '0 1px 4px rgba(0,0,0,0.35)', cursor: 'pointer', padding: 0, fontSize: 15, lineHeight: 1, filter: 'grayscale(1)', opacity: ch.pinned ? 1 : 0.92 }}>📍</button>
