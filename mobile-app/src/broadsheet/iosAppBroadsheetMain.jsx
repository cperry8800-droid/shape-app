...
function BSLogin({ onLogin, onBrowse, role, setRole, initialMode }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: t.PAPER, color: t.INK, padding: 'calc(140px + env(safe-area-inset-top, 0px)) 20px 32px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ fontFamily: t.MONO, fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: t.INK70, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `2px solid ${t.INK}`, paddingBottom: 10 }}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          {[['client','Client'],['trainer','Trainer'],['nutritionist','Nutri'],['shape_radio','Radio']].map(([k, l]) => {
            const on = role === k;
            return <button key={k} onClick={() => setRole(k)} style={{ borderRadius: t.RADIUS_SM, padding: 12, border: `1px solid ${on ? '#0ac5a8' : t.INK}`, background: on ? '#0ac5a8' : 'transparent', color: on ? '#031f1c' : t.INK, fontFamily: t.MONO, fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', cursor: 'pointer' }}>{l}</button>;
          })}
        </div>
      </div>
